...
//		}
	...
	private boolean execute(File[] files) {
		...
		File[] image_files = ( File[])ObjectsMessageDlg.execute(
			...
			String.format( ResourceManager.get( "image.files.message"), 0),
			files,
			this,
			MainFrame.get_instance());
		...
		if ( null == image_files || 0 == image_files.length) {
			JOptionPane.showMessageDialog(
				MainFrame.get_instance(),
				ResourceManager.get( "no.image.files.message"),
				ResourceManager.get( "application.title"),
		...
		if ( null == illegal_image_files || 0 == illegal_image_files.length) {
			JOptionPane.showMessageDialog(
				MainFrame.get_instance(),
				ResourceManager.get( "no.illegal.image.files.message"),
				ResourceManager.get( "application.title"),
				JOptionPane.INFORMATION_MESSAGE);
			return true;
		}
		...
		JOptionPane.showMessageDialog(
			MainFrame.get_instance(),
			String.format( ResourceManager.get( "illegal.image.files.message"), illegal_image_files.length),
			ResourceManager.get( "application.title"),
	...
	private boolean get_image_files(File file, List list, ObjectsMessageDlg objectsMessageDlg) {
		...
		if ( file.isDirectory()) {
			...
			objectsMessageDlg.update_message( String.format( ResourceManager.get( "image.files.message"), list.size()));
		}
