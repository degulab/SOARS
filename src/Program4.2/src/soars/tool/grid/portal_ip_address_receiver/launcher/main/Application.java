 ...
 * 
 */
package soars.tool.grid.portal_ip_address_receiver.launcher.main;
...
public class Application {
	...
	public boolean execute() {
		File current_directory = new File( "");
		if ( null == current_directory) {
			JOptionPane.showMessageDialog( null, "Could not start Grid portal IP address receiver!", "SOARS", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		debug( "Grid portal IP address", System.getProperty( "os.name"), System.getProperty( "os.version"), cmdarray);
	...
	private String[] get_cmdarray(File current_directory) {
		...
		if ( 0 <= osname.indexOf( "Windows")) {
			...
			list.add( "-X" + Constant._macScreenMenuName + "=SOARS Grid portal IP address receiver");
			//list.add( "-D" + Constant._mac_screen_menu_name + "=SOARS Grid portal IP address receiver");
		...
		list.add( "-jar");
		list.add( current_directory.getAbsolutePath() + "/" + Constant._portal_ip_address_receiver_jar_filename);
		list.add( "-language");
