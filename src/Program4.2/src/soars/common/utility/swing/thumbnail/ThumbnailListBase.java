...
public class ThumbnailListBase extends StandardList implements DropTargetListener, IProgressCallback, IObjectsProgressCallback {
	 ...
	 */
	protected TreeMap _imagePropertyManager = null;
	 ...
	 * @param overwrite_options the option strings of overwrite for the dialog box
	 ...
	 */
	public ThumbnailListBase(File imageDirectory, File thumbnailImageDirectory, TreeMap imagePropertyManager, String title, String[] messages, String[] overwrite_options, String cancel, Frame owner, Component parent) {
		super(owner, parent);
		...
		_title = title;
		_messages = messages;
		_overwriteOptions = overwrite_options;
		_cancel = cancel;
	}
	...
	protected boolean load(String filename) {
		...
		String[] filenames = ( String[])_imagePropertyManager.keySet().toArray( new String[ 0]);
	...
	private int select_page(String filename) {
		return select_page( ( String[])_imagePropertyManager.keySet().toArray( new String[ 0]), filename);
	}
	...
	protected boolean append(File[] files) {
		...
		changed( ( String[])overwrites.toArray( new String[ 0]));
	 ...
	 */
	private void on_yes_to_all(File[] files, List list, List overwrites) {
		for ( int i = 0; i < files.length; ++i) {
			if ( !files[ i].isFile() || !files[ i].canRead())
				continue;
	 ...
	 * @param list all image files which are appended
	 */
	private void on_no_to_all(File[] files, List list) {
		for ( int i = 0; i < files.length; ++i) {
			if ( !files[ i].isFile() || !files[ i].canRead())
				continue;
...
//				continue;
//
//			File file = new File( _image_directory, files[ i].getName());
//			if ( file.exists()) {
//				// 上書きしないものは省く
//				if ( no_to_all)
//					continue;
...
//					int result = JOptionPane.showOptionDialog( _parent,
//						files[ i].getName() + " - " + _messages[ 2],
//						_title, JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, _overwrite_options, _overwrite_options[ 0]);
//					switch ( result) {
//						case 0:
//							break;
//						case 1:	// No
...
//	}
	...
	private void setup_current_page() {
		...
		String[] filenames = ( String[])_imagePropertyManager.keySet().toArray( new String[ 0]);
		for ( int i = 0, j = _max * _page; i < _max && j < filenames.length; ++i, ++j) {
			ImageProperty imageProperty = ( ImageProperty)_imagePropertyManager.get( filenames[ j]);
	...
	private Data[] load(File[] files, ObjectsProgressDlg objectsProgressDlg) {
		...
		for ( int i = 0; i < max; ++i) {
			...
			ImageProperty imageProperty = ( ImageProperty)_imagePropertyManager.get( file.getName());
			if ( null == imageProperty) {
...
//		}
	 ...
	 * @param new_filename the new image filename
	 * @return true if we could rename the image file of the specified thumbnail panel successfully
	 */
	protected boolean rename(ThumbnailItem thumbnailItem, String new_filename) {
		String original_filename = thumbnailItem._file.getName();
		ImageProperty imageProperty = ( ImageProperty)_imagePropertyManager.get( original_filename);
		if ( null == imageProperty)
			return false;

		if ( !thumbnailItem._file.renameTo( new File( _imageDirectory, new_filename)))
			return false;

		File file = new File( _thumbnailImageDirectory, original_filename);
		if ( !file.renameTo( new File( _thumbnailImageDirectory, new_filename)))
			return false;

		_imagePropertyManager.remove( original_filename);
		_imagePropertyManager.put( new_filename, imageProperty);

		int index = select_page( new_filename);

		setup_current_page();

		if ( null != new_filename && !new_filename.equals( "") && 0 <= index && _defaultComboBoxModel.getSize() > index) {
			setSelectedIndex( index);
			on_select();
		}
