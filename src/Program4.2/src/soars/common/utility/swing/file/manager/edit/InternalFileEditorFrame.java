 ...
 *
 */
public class InternalFileEditorFrame extends MDIChildFrame implements IFileMenuHandler, IEditMenuHandler, ITextUndoRedoManagerCallBack {

	/**
	 * Default width.
	 */
	static public final int _defaultWwidth = 600;
	 ...
	 * 
	 */
	private MDIFrame _parentFrame = null;
...
//	private JButton _fileSaveAsButton = null;
	 ...
	 */
	public InternalFileEditorFrame(File file, boolean arg1, boolean arg2, boolean arg3, boolean arg4, MDIFrame parentFrame, EnvironmentBase environmentBase, String windowRectangleKey, String encoding, IFileManager fileManager, IFileManagerCallBack fileManagerCallBack) {
		super(( null == encoding || encoding.equals( "")) ? file.getName() : ( file.getName() + " [" + encoding + "]"), arg1, arg2, arg3, arg4);
		_file = file;
	...
	private void get_property_from_environment_file() {
		String value = _environmentBase.get( _windowRectangleKey + "x",
			String.valueOf( SwingTool.get_default_window_position( _defaultWwidth, _defaultHeight).x));
		_windowRectangle.x = Integer.parseInt( value);

		value = _environmentBase.get( _windowRectangleKey + "y",
			String.valueOf( SwingTool.get_default_window_position( _defaultWwidth, _defaultHeight).y));
		_windowRectangle.y = Integer.parseInt( value);

		value = _environmentBase.get( _windowRectangleKey + "width", String.valueOf( _defaultWwidth));
		_windowRectangle.width = Integer.parseInt( value);

		value = _environmentBase.get( _windowRectangleKey + "height", String.valueOf( _defaultHeight));
	...
	public boolean optimize_window_rectangle() {
		...
		if ( !_parentFrame.get_client_rectangle().intersects( rectangle)
			|| _parentFrame.get_client_rectangle().intersection( rectangle).width <= 10
			|| rectangle.y <= -_parentFrame.getInsets().top
			|| _parentFrame.get_client_rectangle().intersection( rectangle).height <= _parentFrame.getInsets().top) {
			Point position = SwingTool.get_default_window_position(
				_parentFrame.get_client_rectangle().getBounds().width,
				_parentFrame.get_client_rectangle().getBounds().height,
				_defaultWwidth, _defaultHeight);
			setLocation( position);
			setSize( _defaultWwidth, _defaultHeight);
			return true;
		}
		return false;
	}
...
//		new DropTarget( this, this);
		...
		addComponentListener( new ComponentAdapter() {
			public void componentResized(ComponentEvent e){
				...
				setSize( ( _defaultWwidth > width) ? _defaultWwidth : width,
					( _defaultHeight > height) ? _defaultHeight : height);
			}
		});
...
//		_fileSaveAsButton.setPreferredSize( new Dimension( imageIcon.getIconWidth() + 8, _fileSaveAsButton.getPreferredSize().height));
	...
	public void on_file_save_with_specified_encoding(ActionEvent actionEvent) {
		EncodingSelectorDlg encodingSelectorDlg = new EncodingSelectorDlg( _parentFrame, ResourceManager.get_instance().get( "encoding.selector.dialog.title"), true, _encoding);
