...
//			_parent = directory;
	...
	private void update_files(File[] files, DefaultTableModel defaultTableModel) {
		TreeMap treeMap = get_extensions( files);
		...
		while ( iterator.hasNext()) {
			...
			update_files( extension, ( File[])list.toArray( new File[ 0]), defaultTableModel);
		}
	}
	 ...
	 * @return
	 */
	private TreeMap get_extensions(File[] files) {
		TreeMap treeMap = new TreeMap();
		for ( int i = 0; i < files.length; ++i) {
			String extension = get_extension( files[ i]);
			List list = ( List)treeMap.get( extension);
			if ( null == list) {
				list = new ArrayList();
	...
	public void on_paste(ActionEvent actionEvent) {
		...
		int result = IntProgressDlg.execute(
			...
			new Object[] { files, _parent, Integer.valueOf( DnDConstants.ACTION_COPY), Boolean.valueOf( files[ 0].getParentFile().equals( _parent)), Boolean.valueOf( true)},
			this, _component);
		switch ( result) {
			case -1:
	...
	public void on_export(ActionEvent actionEvent) {
		...
		int result = IntProgressDlg.execute(
			...
			"copy",
			ResourceManager.get_instance().get( "dialog.cancel"),
			new Object[] { files, directory, Integer.valueOf( DnDConstants.ACTION_COPY), Boolean.valueOf( false), Boolean.valueOf( false)},
			this, _component);
		switch ( result) {
			case -1:
				Message.on_error_from_parent_to_child( _component);
	...
	private boolean paste(File[] files, int action) {
		...
		int result = IntProgressDlg.execute(
			...
			"copy",
			ResourceManager.get_instance().get( "dialog.cancel"),
			new Object[] { files, target_directory, Integer.valueOf( action), Boolean.valueOf( false), Boolean.valueOf( true)},
			this, _component);
		switch ( result) {
			case -1:
				Message.on_error_from_parent_to_child( _component);
