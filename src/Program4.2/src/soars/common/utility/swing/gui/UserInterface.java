...
public class UserInterface implements ActionListener {
	 ...
	 * 
	 */
	private HashMap _menuMap = new HashMap();
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		MenuAction menuAction = ( MenuAction)_menuMap.get( name);
		if ( null == menuAction)
			return;
	 ...
	 * @return
	 */
	public boolean this_menuItem_exists(List menuList, JMenuBar menuBar) {
		List list = ( List)menuList.get( menuList.size() - 1);
		if ( this_menuItem_exists( ( String)list.get( 0), menuBar))
			return true;

		JMenu menu = null;
		for ( int i = 0; i < menuList.size(); ++i) {
			list = ( List)menuList.get( i);
	 ...
	 * @return
	 */
	public JMenu create_menu(List menuList, JMenuBar menuBar, JLabel messageLabel) {
		JMenu menu = null;
		for ( int i = 0; i < menuList.size() - 1; ++i) {
			List list = ( List)menuList.get( i);
			String[] menu_elements = ( String[])list.toArray( new String[ 0]);
			menu = ( ( 0 == i) ? create_menu( menu_elements, menuBar, messageLabel)
				: create_menu( menu_elements, menu, messageLabel));
			if ( null == menu)
				return null;
		}
		return menu;
	 ...
	 * @return
	 */
	public JMenu create_menu(String[] menu_elements, JMenuBar menuBar, JLabel messageLabel) {
		for ( int i = 0; i < menuBar.getMenuCount(); ++i) {
			JMenu menu = menuBar.getMenu( i);
			if ( menu.getText().equals( menu_elements[ 0]))
				return menu;
		}

		JMenu new_menu = append_menu(
			menuBar,
			menu_elements[ 0],
			true,
	 ...
	 * @return
	 */
	public JMenu create_menu(String[] menu_elements, JMenu menu, JLabel messageLabel) {
		for ( int i = 0; i < menu.getItemCount(); ++i) {
			JComponent component = menu.getItem( i);
			if ( !( component instanceof JMenuItem && !( component instanceof JMenu)))
				continue;
			...
			JMenuItem menuItem = ( JMenuItem)component;
			if ( menuItem.getText().equals( menu_elements[ 0]))
