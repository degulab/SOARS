...
public class ImageManipulator {
	 ...
	 * @param image_property_map the image property hashtable(String[filename] - ImageProperty)
	 * @return true if the specified image is set to the specified object
	 */
	public static boolean get_image(Object object, File src, TreeMap image_property_map) {
		File dest = new File( ObjectManager.get_instance().get_image_directory(), src.getName());
		if ( !dest.exists()) {
			if ( !FileUtility.copy( src, dest))
				return get_default_image( object, src.getName(), image_property_map);
			...
			if ( null == bufferedImage) {
				...
				if ( AnimatorImageManager._thumbnail_size >= bufferedImage.getWidth()
					&& AnimatorImageManager._thumbnail_size >= bufferedImage.getHeight()) {
					if ( !FileUtility.copy( dest, new File( ObjectManager.get_instance().get_thumbnail_image_directory(), dest.getName()))) {
						dest.delete();
						return get_default_image( object, src.getName(), image_property_map);
					}
				} else {
				...
				image_property_map.put( dest.getName(), new ImageProperty( bufferedImage.getWidth(), bufferedImage.getHeight()));
			}
		}
	 ...
	 * Returns true if the specified image is set to the specified object.
	 * @param object the specified object(AgentObject, SpotObject, AgentProperty or SpotProperty)
	 * @param image_filename th image filename
	 * @return true if the specified image is set to the specified object
	 */
	private static boolean load_image(Object object, String image_filename) {
		if ( object instanceof AgentObject) {
			AgentObject agentObject = ( AgentObject)object;
			return agentObject.load_image( image_filename);
		} else if ( object instanceof SpotObject) {
			SpotObject spotObject = ( SpotObject)object;
			return spotObject.load_image( image_filename);
		} else if ( object instanceof PropertyBase) {
			PropertyBase propertyBase = ( PropertyBase)object;
			return propertyBase.load_image( image_filename);
		}
		return false;
	}
	...
	private static boolean make_thumbnail_imagefile(File file, BufferedImage bufferedImage) {
		...
		double ratio = ( ( double)AnimatorImageManager._thumbnail_size / ( double)Math.max( bufferedImage.getWidth(), bufferedImage.getHeight()));
	...
	private static boolean make_thumbnail_imagefile(File file, ImageIcon imageIcon) {
		...
		return Resource.write_image( thumbnail_bufferedImage, "png", new File( ObjectManager.get_instance().get_thumbnail_image_directory(), file.getName()));
	}

	/**
	 * Returns true if the default image is set to the specified object.
	 * @param object the specified object(AgentObject, SpotObject, AgentProperty or SpotProperty)
	 * @param image_filename the default image file
	 * @param image_property_map the image property hashtable(String[filename] - ImageProperty)
	 * @return true if the default image is set to the specified object
	 */
	public static boolean get_default_image(Object object, String image_filename, TreeMap image_property_map) {
		File dest = new File( ObjectManager.get_instance().get_image_directory(), image_filename);
		if ( !dest.exists()) {
			...
			if ( !Resource.write_image( bufferedImage, "png", new File( ObjectManager.get_instance().get_thumbnail_image_directory(), image_filename))) {
				dest.delete();
				return false;
			}
