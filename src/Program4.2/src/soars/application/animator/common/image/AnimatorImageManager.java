...
public class AnimatorImageManager extends ImageManager {
	 ...
	 */
	static public final int _thumbnail_max = 30;

	/**
	 * Size of ths thumbnail's maximum side.
	 */
	static public final int _thumbnail_size = 100;
	...
	public BufferedImage load_image(String filename) {
		...
		File file = new File( ObjectManager.get_instance().get_image_directory(), filename);
	...
	public void update() {
		...
		for ( int i = 0; i < filenames.length; ++i) {
			...
			if ( !ObjectManager.get_instance().uses_this_image( file.getName()))
				remove( filenames[ i]);
		}
	}
}
