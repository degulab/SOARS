...
//	}
	 ...
	 */
	public boolean write(File root_directory, String value, Writer writer) throws SAXException {
		...
		if ( null != root_directory && !_imageFilename.equals( "")) {
			File dest = new File( ObjectManager.get_instance().get_image_directory( root_directory), _imageFilename);
			if ( !dest.exists()) {
				...
				if ( !FileUtility.copy( new File( ObjectManager.get_instance().get_thumbnail_image_directory(), _imageFilename),
					new File( ObjectManager.get_instance().get_thumbnail_image_directory( root_directory), _imageFilename)))
					return false;
			}
		}
