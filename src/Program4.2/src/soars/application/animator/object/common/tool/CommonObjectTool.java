...
public class CommonObjectTool {
	 ...
	 * @param objectBases the objects
	 * @return the array of the name of the image file set to the objects
	 */
	public static String[] get_image_filenames(Vector objectBases) {
		List list = new ArrayList();
		for ( int i = 0; i < objectBases.size(); ++i) {
			ObjectBase objectBase = ( ObjectBase)objectBases.get( i);
			if ( objectBase._imageFilename.equals( ""))
				continue;

			if ( list.contains( objectBase._imageFilename))
				continue;

			list.add( objectBase._imageFilename);
		}
