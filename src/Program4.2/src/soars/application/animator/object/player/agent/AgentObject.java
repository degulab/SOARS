 ...
 * 2005/02/07
 */
package soars.application.animator.object.player.agent;
 ...
 */
public class AgentObject extends ObjectBase {
	...
	public AgentObject(String name) {
		super(name, new Color( ( int)( 255 * Math.random()), 0, ( int)( 255 * Math.random())), new Color( 255, 0, 0));
	}
	 ...
	 */
	public AgentObject(String name, Graphics2D graphics2D) {
		super(name, new Color( ( int)( 255 * Math.random()), 0, ( int)( 255 * Math.random())), new Color( 255, 0, 0));
	...
	public void draw(Graphics2D graphics2D, Rectangle rectangle, ImageObserver imageObserver) {
		...
		if ( null != bufferedImage) {
			...
			AgentPropertyManager.get_instance().draw_current_property_image( graphics2D, _position, CommonProperty.get_instance()._agentWidth, CommonProperty.get_instance()._agentHeight, _properties, rectangle, imageObserver, true);
		} else {
			if ( !AgentPropertyManager.get_instance().draw_current_property_image( graphics2D, _position, CommonProperty.get_instance()._agentWidth, CommonProperty.get_instance()._agentHeight, _properties, rectangle, imageObserver, false))
...
//	}
	...
	public void animate(Graphics2D graphics2D, Rectangle rectangle, ImageObserver imageObserver) {
		...
		Point position = ScenarioManager.get_instance().get_current_position( this);
		if ( null == position)
			return;
...
//		}
	...
	public boolean contains(Point point, String state) {
		...
		if ( state.equals( "animation"))
			position = ScenarioManager.get_instance().get_current_position( this);
...
//		return ( rect1.contains( point) || rect2.contains( point));
	...
	public boolean edit(Frame frame) {
		EditObjectDlg editObjectDlg = new EditObjectDlg( frame,
			ResourceManager.get( "edit.agent.dialog.title"),
	...
	/* (Non Javadoc)
	 * @see soars.application.animator.object.player.base.ObjectBase#select(boolean)
	 */
...
//			|| rectangle.intersects( new Rectangle( _position.x, _position.y + _imageDimension.height, _nameDimension.width, _nameDimension.height)));
	...
	/* (non-Javadoc)
	 * @see soars.application.animator.object.player.base.ObjectBase#get_tooltip_text(java.lang.String)
	 */
	public String get_tooltip_text(String state) {
		if ( state.equals( "animation"))
			return _name + ScenarioManager.get_instance().get_tooltip_text( this, AgentTransitionManager.get_instance(), AgentPropertyManager.get_instance().get_selected_properties());
		else if ( state.equals( "edit"))
			return get_tooltip_text( AgentPropertyManager.get_instance().get_selected_properties());
		else
			return null;
	}
