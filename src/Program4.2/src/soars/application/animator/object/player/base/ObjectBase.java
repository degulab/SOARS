...
public class ObjectBase {
	 ...
	 */
	public ObjectBase(String name, Color defaultImageColor, Color defaultTextColor) {
		super();
		_name = name;
	...
	public void update(ObjectBase objectBase) {
		...
		_visibleName = objectBase._visibleName;

		_imageFilename = objectBase._imageFilename;

		_position = objectBase._position;
		_dimension = objectBase._dimension;

		_imageDimension = objectBase._imageDimension;

		_nameDimension = objectBase._nameDimension;

//		_bufferedImage = objectBase._bufferedImage;
		
		_imageColor = objectBase._imageColor;

		_textColor = objectBase._textColor;

		_font = objectBase._font;
	}
...
//	}
	...
	public boolean write_graphic_data(File rootDirectory, AttributesImpl attributesImpl) {
		...
		if ( null != rootDirectory && !_imageFilename.equals( "")) {
			File dest = new File( ObjectManager.get_instance().get_image_directory( rootDirectory), _imageFilename);
			if ( !dest.exists()) {
				if ( !FileUtility.copy( new File( ObjectManager.get_instance().get_image_directory(), _imageFilename), dest))
					return false;
				if ( !FileUtility.copy( new File( ObjectManager.get_instance().get_thumbnail_image_directory(), _imageFilename),
					new File( ObjectManager.get_instance().get_thumbnail_image_directory( rootDirectory), _imageFilename)))
					return false;
			}
		}
