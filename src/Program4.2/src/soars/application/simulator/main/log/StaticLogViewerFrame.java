...
public class StaticLogViewerFrame extends LogViewerFrame {
	...
	public boolean update(String log_viewer_window_title, Rectangle window_rectangle, String console, List agents, List spots, String stdout, String stderr) {
		if ( null == _consoleLogPropertyPage) {
			...
			if ( !_consoleLogPropertyPage.create( ResourceManager.get( "log.viewer.console.title"), console))
				return false;
		} else {
			_tabbedPane.setTitleAt( _tabbedPane.indexOfComponent( _consoleLogPropertyPage), ResourceManager.get( "log.viewer.console.title"));
			_consoleLogPropertyPage._textArea.setText( console);
		}

		if ( null == _stdOutPropertyPage) {
			_stdOutPropertyPage = new SystemOutPropertyPage( _tabbedPane, ResourceManager.get( "log.viewer.stdout.title"));
			if ( !_stdOutPropertyPage.create())
				return false;
		}

		if ( null == _stdErrOutPropertyPage) {
			_stdErrOutPropertyPage = new SystemOutPropertyPage( _tabbedPane, ResourceManager.get( "log.viewer.stderr.title"));
