...
public class FileManagerFrame extends MDIChildFrame implements IFileManagerCallBack {
	 ...
	 * Default width.
	 */
	static public final int _default_width = 600;

	/**
	 * Default height.
	 */
	static public final int _default_height = 400;
	 ...
	 * 
	 */
	private boolean _setup_completed = false;
	...
	public boolean create(FileManagerData fileManagerData) {
		...
		if ( null == fileManagerData)
			...
			setLocation( fileManagerData._window_rectangle.x, fileManagerData._window_rectangle.y);
			setSize( fileManagerData._window_rectangle.width, fileManagerData._window_rectangle.height);
		}
	...
	private boolean setup_fileManager(FileManagerData fileManagerData) {
		...
		_fileManager.set_current_directory( ( null == fileManagerData) ? "" : fileManagerData._current_directory);

		_fileManager.setDividerLocation( ( null == fileManagerData) ? 100 : fileManagerData._divider_location);
	...
	public boolean optimize_window_rectangle() {
		...
		if ( !MainFrame.get_instance().get_client_rectangle().intersects( rectangle)
			...
			Point position = SwingTool.get_default_window_position(
				...
				_default_width, _default_height);
	...
	public void on_paste_completed() {
		warn( ResourceManager.get( "warning.dialog1.message4"));
	}
	...
	public void on_paste_and_remove_completed() {
		warn( ResourceManager.get( "warning.dialog1.message5"));
	}
	...
	private void warn(String message) {
		...
		WarningDlg1 warningDlg1 = new WarningDlg1(
			...
			ResourceManager.get( "warning.dialog1.title"),
			message,
			this);
	...
	public void on_select(File file, String encoding) {
		...
		InternalFileEditorFrame iInternalFileEditorFrame = new InternalFileEditorFrame( file, true, true, true, true, MainFrame.get_instance(), Environment.get_instance(), Environment._file_editor_window_rectangle_key, encoding, _fileManager, this);
...
//			fileEditorFrame.setIconImage( image);
	...
	public File get_export_directory() {
		...
		fileChooser.setDialogTitle( ResourceManager.get( "file.manager.export.files.title"));
