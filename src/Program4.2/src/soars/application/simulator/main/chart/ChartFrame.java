...
public class ChartFrame extends MDIChildFrame implements IFileMenuHandler, IMessageCallback {
	 ...
	 * Default width.
	 */
	static public final int _default_width = 510;

	/**
	 * Default height.
	 */
	static public final int _default_height = 360;
	...
	private void setup_menu() {
		...
		_file_save_as_menuItem = _userInterface.append_menuitem(
			...
			new SaveAsAction( ResourceManager.get( "file.save.as.menu"), this),
		...
		_file_save_image_as_menuItem = _userInterface.append_menuitem(
			menu,
			ResourceManager.get( "file.save.image.as.menu"),
			new SaveImageAsAction( ResourceManager.get( "file.save.image.as.menu"), this),
			ResourceManager.get( "file.save.image.as.mnemonic"),
			ResourceManager.get( "file.save.image.as.stroke"));
	...
	public boolean create(ChartData chartData, File directory) {
		...
		setLocation( chartData._window_rectangle.x, chartData._window_rectangle.y);
		setSize( chartData._window_rectangle.width, chartData._window_rectangle.height);
		...
		if ( !dataset._value.equals( "")) {
			...
			for ( int i = 0; i < values.length; i += 3) {
				...
				try {
					...
					double y = Double.valueOf( values[ i + 2]).doubleValue();
	...
	public void on_file_save_image_as(ActionEvent actionEvent) {
		...
		File file = CommonTool.get_save_file(
			...
			ResourceManager.get( "file.save.image.as.dialog"),
			new String[] { "png"},
		...
		if ( !MessageDlg.execute( MainFrame.get_instance(), getTitle(), true,
			"on_file_save_image_as", ResourceManager.get( "file.save.image.as.show.message"),
			new Object[] { bufferedImage, "png", file}, this, this)) {
			JOptionPane.showMessageDialog( this, ResourceManager.get( "file.save.image.as.error.message"), ResourceManager.get( "application.title"), JOptionPane.ERROR_MESSAGE);
			return;
		}
	}
