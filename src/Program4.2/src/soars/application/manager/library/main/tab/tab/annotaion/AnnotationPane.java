...
//		return MessageDlg.execute( _owner, ResourceManager.get( "application.title"), true,
//			"on_update_class_tree", ResourceManager.get( "file.open.show.message"),
//			new Object[] { file, Boolean.valueOf( save)}, this, _owner);
	...
	public boolean update(File[] files) {
		...
		boolean result = MessageDlg.execute( _owner, ResourceManager.get( "application.title"), true,
			"on_update_annotaion_files",
			ResourceManager.get( "dialog.cancel"),
			ResourceManager.get( "updating.annotation.files.message"),
			files, this, _owner);
		...
		if ( !WarningManager.get_instance().isEmpty()) {
			WarningDlg1 warningDlg1 = new WarningDlg1(
				...
				ResourceManager.get( "warning.dialog1.message1"),
				_parent);
