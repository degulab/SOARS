 ...
 * @author kurata
 */
public class WarningDlg2 extends Dialog {
	 ...
	 * @param parent
	 */
	public WarningDlg2(Frame arg0, String arg1, String message, Component parent) {
	...
	private Rectangle get_rectangle_from_environment_file() {
		String value = Environment.get_instance().get(
			Environment._warningDialogRectangleKey1 + "x",
		...
		value = Environment.get_instance().get(
			Environment._warningDialogRectangleKey1 + "y",
			String.valueOf( SwingTool.get_default_window_position(
				( null != _parent) ? _parent : getOwner(),
		...
		value = Environment.get_instance().get(
			Environment._warningDialogRectangleKey1 + "width",
		...
		value = Environment.get_instance().get(
			Environment._warningDialogRectangleKey1 + "height",
	...
	protected void set_property_to_environment_file() {
		...
		Environment.get_instance().set(
			Environment._warningDialogRectangleKey1 + "x", String.valueOf( rectangle.x));
		Environment.get_instance().set(
			Environment._warningDialogRectangleKey1 + "y", String.valueOf( rectangle.y));
		Environment.get_instance().set(
			Environment._warningDialogRectangleKey1 + "width", String.valueOf( rectangle.width));
		Environment.get_instance().set(
			Environment._warningDialogRectangleKey1 + "height", String.valueOf( rectangle.height));
	}
	...
	protected boolean on_init_dialog() {
		...
		setup_continue_button( southg_panel);

		insert_horizontal_glue( southg_panel);

		setup_abort_button( southg_panel);
	...
	private void setup_copy_to_clipboard_button(JPanel southg_panel) {
		...
		_copy_to_clipboard_button = new JButton(
			ResourceManager.get_instance().get( "warning.dialog2.copy.to.clipboard.button.name"));
	 ...
	 * 
	 */
	private void setup_continue_button(JPanel southg_panel) {
		JPanel panel = new JPanel();
		panel.setLayout( new BoxLayout( panel, BoxLayout.X_AXIS));
		...
		JButton button = new JButton(
			ResourceManager.get_instance().get( "warning.dialog2.continue.button.name"));
	 ...
	 * 
	 */
	private void setup_abort_button(JPanel southg_panel) {
		JPanel panel = new JPanel();
		panel.setLayout( new BoxLayout( panel, BoxLayout.X_AXIS));
		...
		JButton button = new JButton(
			ResourceManager.get_instance().get( "warning.dialog2.abort.button.name"));
		button.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				_continue = false;
				set_property_to_environment_file();
