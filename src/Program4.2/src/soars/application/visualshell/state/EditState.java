...
public class EditState extends StateBase implements IBasicMenuHandler1, IBasicMenuHandler2, IEditMenuHandler {
	...
	private void update_menu() {
		...
		MainFrame.get_instance()._editRemoveButton.setEnabled( !drawObjects.isEmpty());
	}
...
//		if ( null != _selected_drawObject) {
//			_selected_drawObject.select( false);
//			_selected_drawObject = null;
//		} else if ( null != _selected_connectOutObject) {
		if ( null != _selectedConnectOutObject) {
			...
			if ( null != drawObject && ( drawObject instanceof Role)
				...
				if ( role.isConnectInObject( mouseEvent.getPoint())
					...
					//&& !_selected_connectOutObject.equals( connectInObject._connectObject)) {
...
//					if ( drawObjects.contains( drawObject)) {
	...
	public boolean can_paste_objects() {
		...
		String system_temporary_directory = SoarsCommonTool.get_system_temporary_directory();
		File file = new File( system_temporary_directory + LayerManager._temporaryFilename);
