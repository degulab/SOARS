...
import soars.application.visualshell.common.image.menu.AppendAction;
import soars.application.visualshell.common.image.menu.EditAction;
import soars.application.visualshell.common.image.menu.RemoveAction;
...
public class ThumbnailList extends ThumbnailListBase {
	...
	public ThumbnailList(File image_directory, File thumbnail_image_directory, String open_directory_key, IThumbnailListCallback thumbnailListCallback, ThumbnailPanel thumbnailPanel, Frame owner, Component parent) {
		...
		_max = VisualShellImageManager._thumbnail_max;
		_side = VisualShellImageManager._thumbnail_size;
	...
	protected boolean update(String[] filenames, ProgressDlg progressDlg) {
		for ( int i = 0; i < filenames.length; ++i) {
			...
			File file = new File( LayerManager.get_instance().get_image_directory(), filenames[ i]);
			BufferedImage bufferedImage = Resource.load_image( file.getAbsolutePath());
			if ( null == bufferedImage)
				continue;

			VisualShellImageManager.get_instance().put( file.getAbsolutePath(), bufferedImage);
		}
		return true;
	}
...
//			scrollRectToVisible( getCellBounds( indices[ 0], indices[ 0]));
	...
	public void on_edit(ActionEvent actionEvent) {
		...
		Image image = ( Image)VisualShellImageManager.get_instance().get( file.getAbsolutePath());
		if ( null != image) {
			VisualShellImageManager.get_instance().remove( file.getAbsolutePath());
			file = new File( _imageDirectory, new_filename);
			VisualShellImageManager.get_instance().put( file.getAbsolutePath(), image);
		}

		LayerManager.get_instance().update_image( original_filename, new_filename);
	...
	protected boolean can_remove(int[] indices) {
		...
		for ( int i = 0; i < indices.length; ++i) {
			...
			if ( LayerManager.get_instance().uses_this_image( thumbnailItem._file.getName())) {
