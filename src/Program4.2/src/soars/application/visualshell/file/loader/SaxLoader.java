...
import soars.application.visualshell.object.player.base.object.exchange_algebra.ExchangeAlgebraInitialValue;
...
import soars.application.visualshell.object.player.base.object.map.MapInitialValue;
...
public class SaxLoader extends DefaultHandler {
	 ...
	 * 
	 */
	private boolean _playerBaseOthers = false;
	 ...
	 * Returns true if loading the specified file is completed successfully.
	 * @param file the specified XML file of Visual Shell data
	 * @param root_directory data's root directory
	 * @param graphics2D the graphics object of JAVA
	 * @param component the base class for all Swing components
	 * @return true if loading the specified file is completed successfully
	 * @return
	 */
	public static boolean execute(File file, File root_directory, Graphics2D graphics2D, JComponent component) {
		_result = false;
		_modified = false;
		...
		try {
			...
			SAXParser saxParser = saxParserFactory.newSAXParser();
			saxParser.parse( file, saxLoader);
			saxLoader.at_end_of_load( root_directory, component);
		} catch (Exception e) {
			e.printStackTrace();
			saxLoader.at_end_of_load( root_directory, component);
			return false;
	 ...
	 * @param component
	 */
	private void at_end_of_load(File root_directory, JComponent component) {
		if ( !_result)
			return;
		...
		if ( !LayerManager.get_instance().append( _layer, _expressionMap, _imagePropertyMap, root_directory, component)) {
			_result = false;
			return;
		}
	...
	private void on_layer(Attributes attributes) {
		...
		if ( name.equals( Constant._reservedLayerName))
			...
			if ( 0 == _layerCounter)
				_layer = ( Layer)LayerManager.get_instance().get( 0);
			else
				_layer = new Layer();
		}
	 ...
	 * @param type
	 */
	private void on_object_base(Attributes attributes, String type) {
		if ( null == _layer) {
			_result = false;
			return;
		}
		...
		String initial_role = attributes.getValue( "initial_role");
		if ( null == initial_role)
			initial_role = "";

		if ( !initial_role.equals( "")) {
			String[] roles = initial_role.split( ":");
		...
		if ( type.equals( "spot"))
			_drawObject = new SpotObject( id, name, new Point( x, y), number, initial_role, image_filename, gis, gisCoordinates, _graphics2D);
		else if ( type.equals( "agent")) {
			String initial_spot = attributes.getValue( "initial_spot");
			if ( null == initial_spot)
				initial_spot = "";

			_drawObject = new AgentObject( id, name, new Point( x, y), number, initial_role, initial_spot, image_filename, gis, gisCoordinates, _graphics2D);
		} else {
			_result = false;
			return;
		}
	...
	private void on_variable(Attributes attributes, String type) {
		if ( null == _drawObject || !( _drawObject instanceof PlayerBase)) {
			_result = false;
			return;
		}
		...
		PlayerBase playerBase = ( PlayerBase)_drawObject;
		...
		if ( !_initialData)
			playerBase.append_object( variableObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( variableObject, number);
		}
	...
	private void on_map(Attributes attributes, String type) {
		...
		if ( !_initialData)
			playerBase.append_object( mapObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( mapObject, number);
		}
	...
	private void get_map_initial_values(List<MapInitialValue> mapInitialValues, Attributes attributes) {
		int index = 0;
		while ( true) {
			String key_type = attributes.getValue( "key_type" + index);
			if ( null == key_type)
				key_type = "";
			...
			String value_type = attributes.getValue( "value_type" + index);
			if ( null == value_type)
				value_type = "";
			...
			mapInitialValues.add( new MapInitialValue( key_type, key, value_type, value));
	...
	private void on_object_probability(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( probabilityObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( probabilityObject, number);
		}
	...
	private void on_object_keyword(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( keywordObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( keywordObject, number);
		}
	...
	private void on_object_number_object(Attributes attributes) {
		...
		String initial_value = attributes.getValue( "initial_value");
		if ( null == initial_value)
			initial_value = "";

		PlayerBase playerBase = ( PlayerBase)_drawObject;

		NumberObject numberObject = new NumberObject( name, type, initial_value);

		if ( !_initialData)
			playerBase.append_object( numberObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( numberObject, number);
		}
	...
	private void on_object_role_variable(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( roleVariableObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( roleVariableObject, number);
		}
	...
	private void on_object_time_variable(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( timeVariableObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( timeVariableObject, number);
		}
	...
	private void on_object_spot_variable(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( spotVariableObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( spotVariableObject, number);
		}
	...
	private void on_object_file(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( fileObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( fileObject, number);
		}
	...
	private void on_object_extransfer(Attributes attributes) {
		...
		if ( !_initialData)
			playerBase.append_object( exTransferObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( exTransferObject, number);
		}
	...
	private void on_class_variable(Attributes attributes, String type) {
		...
		if ( !_initialData)
			playerBase.append_object( classVariableObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( classVariableObject, number);
		}
	...
	private void on_exchange_algebra(Attributes attributes, String type) {
		...
		if ( !_initialData)
			playerBase.append_object( exchangeAlgebraObject);
		else {
			String number = attributes.getValue( "number");
			if ( null == name || name.equals( "")) {
				_result = false;
			...
			playerBase.append_object( exchangeAlgebraObject, number);
		}
	...
	private void on_initial_data_file(Attributes attributes, String type) {
		...
		playerBase.append_object( initialDataFileObject);
	...
	private void on_role(Attributes attributes, String type) {
		...
		if ( type.equals( "agent_role")) {
			_drawObject = new AgentRole( id, names[ 0], new Point( x, y), _graphics2D);
			_agentRoleConnectionMap.put( ( Role)_drawObject, names);
		} else if ( type.equals( "spot_role")) {
			_drawObject = new SpotRole( id, names[ 0], new Point( x, y), _graphics2D);
	...
	private void on_rule_column_widths(Attributes attributes) {
		...
		for ( int i = 0; i < attributes.getLength(); ++i) {
			...
			if ( attributes.getQName( i).matches( "^width[0-9]+"))
				_columnWidthsMap.put( attributes.getQName( i).substring( "width".length()), Integer.valueOf( attributes.getValue( i)));
		}
	}
	...
	private void on_simulation_data(Attributes attributes) {
		...
		boolean export_end_time;
		if ( value.equals( "true"))
			export_end_time = true;
		else if ( value.equals( "false"))
			export_end_time = false;
		else {
			_result = false;
			return;
		}

		_simulationManager._exportEndTime = export_end_time;
	...
	private void setup_column_widths_on_experiment_data(Attributes attributes) {
		...
		Vector column_widths = new Vector();

		String name = "width";
		for ( int i = 0; i < column; ++i) {
			value = attributes.getValue( name + i);
			if ( null == value || value.equals( "")) {
				column_widths.add( Integer.valueOf( ( 0 == i)
					? ExperimentManager._defaultExperimentTableCheckBoxColumnWidth
					: ExperimentManager._defaultExperimentTableColumnWidth));
				continue;
			}

			column_widths.add( Integer.valueOf( value));
		}

		_experimentManager.setup_column_widths( column_widths);
	}
	...
	public void characters(char[] arg0, int arg1, int arg2) throws SAXException {
		...
		if ( null != _drawObject) {
			if ( _playerBaseOthers) {
				String others = new String( arg0, arg1, arg2);
				PlayerBase playerBase = ( PlayerBase)_drawObject;
				playerBase._others += others;
			}
		...
		if ( null != _otherScriptsManager) {
			String other_scripts = new String( arg0, arg1, arg2);
			_otherScriptsManager._otherScripts += other_scripts;
		}
	 ...
	 * 
	 */
	private void on_object_base() {
		if ( null == _layer || null == _drawObject
			|| !( _drawObject instanceof PlayerBase)) {
			_result = false;
			return;
		}
	...
	private void set_comment() {
		if ( null == _drawObject || !( _drawObject instanceof PlayerBase) || null == _type || null == _name) {
			_result = false;
			return;
		}
		...
		if ( !playerBase.is_multi())
			result = playerBase.set_comment( _typeKindMap.get( _type), _name, _comment);
		else
			result = playerBase.set_comment( _typeKindMap.get( _type), _name, _objectBase, _comment);
