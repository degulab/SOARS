...
public class MonitorTabbedPane extends JTabbedPane {
	 ...
	 * 
	 */
	private JMenuItem _remove_all_menuItem = null;

	/**
	 * 
	 */
	private JMenuItem _terminate_model_builder_menuItem = null;

	/**
	 * 
	 */
	private JMenuItem _copy_to_clipboard_menuItem = null;
	...
	private void setup_popup_menu() {
		...
		_remove_all_menuItem = _userInterface.append_popup_menuitem(
			_popupMenu,
		...
		_terminate_model_builder_menuItem = _userInterface.append_popup_menuitem(
			_popupMenu,
		...
		_copy_to_clipboard_menuItem = _userInterface.append_popup_menuitem(
			_popupMenu,
	...
	protected void on_mouse_right_up(MouseEvent mouseEvent) {
		...
		_remove_all_menuItem.setEnabled( true);
		_terminate_model_builder_menuItem.setEnabled( true);
		_copy_to_clipboard_menuItem.setEnabled( true);
		...
		if ( monitorPropertyPage.is_alive()) {
			...
			_copy_to_clipboard_menuItem.setEnabled( false);
		} else {
			if ( 2 > getTabCount())
				_remove_menuItem.setEnabled( false);

			_terminate_model_builder_menuItem.setEnabled( false);
		}

		if ( 2 > getTabCount())
			_remove_all_menuItem.setEnabled( false);

		_terminate_model_builder_menuItem.setEnabled( null != monitorPropertyPage._process);
