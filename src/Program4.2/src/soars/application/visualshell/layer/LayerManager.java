...
//			MainFrame.get_instance().setTitle( title);
	...
	private boolean update_expression_manager(TreeMap<String, Expression> expressionMap, Layer newLayer) {
		...
		HashMap<String, String> functionNameMap = get_function_name_map( usedExpressionMap);
	 ...
	 * @return
	 */
	private HashMap<String, String> get_function_name_map(VisualShellExpressionManager usedExpressionMap) {
		HashMap<String, String> functionNameMap = new HashMap<String, String>();
	 ...
	 */
	private void update_function_name_map(Expression expression, HashMap<String, String> functionNameMap) {
	 ...
	 * @param functionNameMap
	 */
	private void update_argument(VisualShellExpressionManager usedExpressionMap, HashMap<String, String> functionNameMap) {
	 ...
	 * @param functionNameMap
	 */
	private void update_function(VisualShellExpressionManager usedExpressionMap, HashMap<String, String> functionNameMap) {
		Iterator iterator = functionNameMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean update_function(Layer newLayer, HashMap<String, String> functionNameMap) {
	 ...
	 * @return
	 */
	private boolean append_image(TreeMap<String, ImageProperty> imagePropertyMap, List overwrites, File rootDirectory) {
		Iterator iterator = imagePropertyMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 */
	public boolean append(HashMap<String, PlayerData> agentDataMap, HashMap<String, PlayerData> spotDataMap,
		HashMap<String, RoleData> agentRoleDataMap, HashMap<String, RoleData> spotRoleDataMap,
		...
		HashMap<String, String[]> simulationDataMap,
		TreeMap<String, Expression> visualShellExpressionMap,
		String otherScripts,
		HashMap<String, String> commentDataMap,
	 ...
	 * @return
	 */
	private boolean append_agent(HashMap<String, PlayerData> agentDataMap, JComponent component) {
		...
		while ( iterator.hasNext()) {
			...
			if ( null == agentObject) {
				agentObject = ( AgentObject)get( _currentLayer).append_object( "agent", get_unique_id(), playerData, component);
				if ( null == agentObject)
					return false;

				agentObject._multi = playerData.is_multi();
			}

			if ( !agentObject.update( playerData))
				return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	private boolean append_spot(HashMap<String, PlayerData> spotDataMap, JComponent component) {
		...
		while ( iterator.hasNext()) {
			...
			if ( null == spotObject) {
				spotObject = ( SpotObject)get( _currentLayer).append_object( "spot", get_unique_id(), playerData, component);
				if ( null == spotObject)
					return false;

				spotObject._multi = playerData.is_multi();
			}

			if ( !spotObject.update( playerData))
				return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	private boolean append_agent_role(HashMap<String, RoleData> agentRoleDataMap, JComponent component) {
	 ...
	 * @return
	 */
	private boolean append_spot_role(HashMap<String, RoleData> spotRoleDataMap, JComponent component) {
	...
	public PlayerBase get_playerBase_has_this_name(String fullName) {
		for ( Layer layer:this) {
			PlayerBase playerBase = layer.get_playerBase_has_this_name( fullName);
			if ( null != playerBase)
				return playerBase;
		}
		return null;
	...
	public boolean is_number_object_type_correct(String player, String numberObjectName, String newType) {
		for ( Layer layer:this) {
			if ( !layer.is_number_object_type_correct( player, numberObjectName, newType))
				return false;
		}
		return true;
	}
	...
	public boolean is_number_object_correct(String type, String numberObjectName, String numberObjectType, PlayerBase playerBase) {
		for ( Layer layer:this) {
			if ( !layer.is_number_object_correct( type, numberObjectName, numberObjectType, playerBase))
				return false;
		}
		return true;
	}
	...
	public boolean update_number_object_type(String type, String numberObjectName, String numberObjectType, PlayerBase playerBase) {
		boolean result = false;
		for ( Layer layer:this) {
			if ( layer.update_number_object_type( type, numberObjectName, numberObjectType, playerBase))
				result = true;
		}
		return result;
	}
	...
	public boolean contains(PlayerBase playerBase, String name, String number) {
		String type;
		if ( playerBase instanceof AgentObject)
			type = "agent";
		else if ( playerBase instanceof SpotObject)
			type = "spot";
		else
			return false;

		for ( Layer layer:this) {
			if ( layer.contains( playerBase, type, name, number)
				|| layer.contains( playerBase, name, number))
				return true;
		}
		return false;
	}
...
//	}
	...
	public void get_playerBases(Vector<DrawObject> playerBases) {
		for ( Layer layer:this)
			layer.get_playerBases( playerBases);
	}
	...
	public void get_gis_spots(List<PlayerBase> playerBases, String gis) {
		for ( Layer layer:this)
			layer.get_gis_spots( playerBases, gis);
	}
	 ...
	 */
	public String[] get_object_names(String kind, Vector<String> objectNames, PlayerBase playerBase, boolean containsEmpty) {
		for ( Layer layer:this)
			layer.get_object_names( kind, objectNames, playerBase);
	 ...
	 */
	public String[] get_agent_object_names(String kind, Vector<String> agentObjectNames, PlayerBase playerBase, boolean containsEmpty) {
		for ( Layer layer:this)
			layer.get_agent_object_names( kind, agentObjectNames, playerBase);
	 ...
	 */
	public String[] get_spot_object_names(String kind, Vector<String> spotObjectNames, PlayerBase playerBase, boolean containsEmpty) {
		for ( Layer layer:this)
			layer.get_spot_object_names( kind, spotObjectNames, playerBase);
	...
	public boolean is_object_name(String kind, String name, PlayerBase playerBase) {
		for ( Layer layer:this) {
			if ( layer.is_object_name( kind, name, playerBase))
				return true;
		}
		return false;
	}
	...
	public boolean other_uses_this_class_variable_as_different_class(String classVariable, String jarFilename, String classname, PlayerBase playerBase) {
		for ( Layer layer:this) {
			if ( layer.other_uses_this_class_variable_as_different_class( classVariable, jarFilename, classname, playerBase))
				return true;
		}
		return false;
	}
	...
	public boolean can_remove(String kind, String name, boolean otherSpotsHaveThisObjectName, String headName, Vector ranges, boolean onRemove) {
		...
		boolean result = true;
		for ( Layer layer:this) {
			if ( !layer.can_remove( kind, name, otherSpotsHaveThisObjectName, headName, ranges, onRemove))
				result = false;
		}
		return result;
	}
	...
	public boolean can_remove(String player, String kind, String objectName, boolean otherPlayersHaveThisObjectName, String headName, Vector ranges, boolean onRemove) {
		...
		boolean result = true;
		for ( Layer layer:this) {
			if ( !layer.can_remove( player, kind, objectName, otherPlayersHaveThisObjectName, headName, ranges, onRemove))
				result = false;
		}
		return result;
	}
	...
	public boolean can_remove(String kind, String name, boolean otherSpotsHaveThisObjectName, String headName, Vector ranges, PlayerBase playerBase, boolean onRemove) {
		...
		boolean result = true;
		for ( Layer layer:this) {
			if ( !layer.can_remove( kind, name, otherSpotsHaveThisObjectName, headName, ranges, playerBase, onRemove))
				result = false;
		}
		return result;
	}
	...
	public boolean can_remove(String player, String kind, String objectName, boolean otherPlayersHaveThisObjectName, String headName, Vector ranges, PlayerBase playerBase, boolean onRemove) {
		...
		boolean result = true;
		for ( Layer layer:this) {
			if ( !layer.can_remove( player, kind, objectName, otherPlayersHaveThisObjectName, headName, ranges, playerBase, onRemove))
				result = false;
		}
		return result;
	}
	...
	public boolean update_object_name(String kind, String name, String newName, PlayerBase playerBase) {
		boolean result = false;
		for ( Layer layer:this) {
			if ( layer.update_object_name( kind, name, newName, playerBase))
				result = true;
		}
		return result;
	}
	...
	public boolean update_number_object_name(String player, String name, String newName, PlayerBase playerBase) {
		boolean result = false;
		for ( Layer layer:this) {
			if ( layer.update_number_object_name( player, name, newName, playerBase))
				result = true;
		}
		return result;
	}
	...
	public boolean can_remove_expression(Expression expression) {
		return can_remove_expression( expression._value[ 0]);
	}

	/**
	 * Returns true if the specified expression can be removed.
	 * @param function the function of specified the expression
	 * @return true if the specified expression can be removed
	 */
	public boolean can_remove_expression(String function) {
		boolean result = true;
		for ( Layer layer:this) {
			if ( !layer.can_remove_expression( function))
				result = false;
		}
		return result;
	}
...
//		}
	 ...
	 * @return
	 */
	public boolean export_TEntityTypes(Map<String, String> spotNameMap, File programRootFolder, String packagePrefix, Class cls, String className, String prefix) {
		// TODO Auto-generated method stub
		String text = "package " + packagePrefix + ";\n\n"
			+ "public class " + className + " {\n";
		for ( Layer layer:this) {
			text = layer.get_TEntityTypes( text, spotNameMap, programRootFolder, packagePrefix, cls, className, prefix);
			if ( null == text)
				return false;
		}
		...
		File file = new File( programRootFolder, className + ".java");
	 ...
	 * @param roleDataSet 
	 */
	public void get_agent_common_variables(String commonRoleClassName, RoleDataSet roleDataSet) {
		// TODO Auto-generated method stub
		if ( !agent_common_role_exists())
			return;

		Vector<DrawObject> agents = new Vector<>();
		get_agents( agents);
		if ( agents.isEmpty())
			return;
		else if ( 1 == agents.size()) {
			roleDataSet._agentCommonVariableMap = ( ( AgentObject)agents.get( 0))._objectMapMap;
			( ( AgentObject)agents.get( 0)).get_variables( commonRoleClassName, roleDataSet._variableMap);
		} else
			( ( PlayerBase)agents.get( 0)).get_common_variables( commonRoleClassName, roleDataSet._agentCommonVariableMap, roleDataSet._variableMap, agents);
	}

	/**
	 * @return
	 */
	private boolean agent_common_role_exists() {
		// TODO Auto-generated method stub
		for ( Layer layer:this) {
			if ( layer.agent_common_role_exists())
				return true;
		}
		return false;
	}
	...
	public boolean get_role_class_java_program(RoleDataSet roleDataSet, File programRootFolder, String packagePrefix, String stageType) {
		// TODO Auto-generated method stub
		for ( Layer layer:this) {
			if ( !layer.get_role_class_java_program( roleDataSet, programRootFolder, packagePrefix, stageType))
				return false;
		}
		return true;
	}
	...
	public boolean export_RoleClasses(RoleDataSet roleDataSet, File programRootFolder, String packagePrefix, Map<String, String> stageNameMap) {
		// TODO Auto-generated method stub
		for ( Layer layer:this) {
			if ( !layer.export_RoleClasses( roleDataSet, programRootFolder, packagePrefix, stageNameMap))
				return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public List<PlayerBase> get_owner(Role role) {
		List<PlayerBase> entities = new ArrayList<>();
		for ( Layer layer:this)
			layer.get_owner( entities, role);
		return entities;
	}
	...
	public boolean get_ruleDataMap(RoleDataSet roleDataSet, Map<String, String> roleClassNameMap, String stageType) {
		// TODO Auto-generated method stub
		for ( Layer layer:this) {
			if ( !layer.get_ruleDataMap( roleDataSet, roleClassNameMap, stageType))
				return false;
		}

		if ( !roleDataSet.create_ruleOwnerRolesMap())
			return false;
	...
	public boolean export_RuleClasses(RoleDataSet roleDataSet, Map<String, String> agentNameMap, Map<String, String> spotNameMap, InitialValueMap initialValueMap, File programRootFolder, String packagePrefix) {
		// TODO Auto-generated method stub
		for ( Layer layer:this) {
			if ( !layer.export_RuleClasses( roleDataSet, agentNameMap, spotNameMap, initialValueMap, programRootFolder, packagePrefix))
				return false;
		}
		return true;
	}
	...
	public void get_entities_have_this_variable(List<PlayerBase> entities, String type, String kind, String variableName) {
		// TODO Auto-generated method stub
		for ( Layer layer:this)
			layer.get_entities_have_this_variable( entities, type, kind, variableName);
	}
...
//				continue;
			...
			try {
				sftpClient.put( ( new File( currentDirectoryName + "/" + jarFilenames[ i])).getAbsolutePath(), scriptDirectoryName + jarFilename);
			} catch (IOException e) {
				return false;
			}
		}
...
//				continue;
			...
			try {
				sftpClient.put( ( new File( currentDirectoryName + "/" + lines[ i])).getAbsolutePath(), scriptDirectoryName + jarFilename);
			} catch (IOException e) {
				return false;
			}
		}
	...
	public boolean copy_objects() {
		...
		if ( !SaxWriter.execute( new File( rootDirectory, Constant._dataFilename), ( File[])imagefiles.toArray( new File[ 0]))) {
			FileUtility.delete( parentDirectory, true);
			return false;
		}
	...
	public boolean paste_objects(Graphics2D graphics2D, JComponent component) {
		...
		File rootDirectory = new File( parentDirectory, Constant._rootDirectoryName);
		rootDirectory = new File( parentDirectory, Constant._rootDirectoryName);
		if ( !rootDirectory.exists() || !rootDirectory.isDirectory()) {
			FileUtility.delete( parentDirectory, true);
			return false;
		}

		if ( !SaxLoader.execute( new File( rootDirectory, Constant._dataFilename), rootDirectory, graphics2D, component)) {
			FileUtility.delete( parentDirectory, true);
			return false;
		}
