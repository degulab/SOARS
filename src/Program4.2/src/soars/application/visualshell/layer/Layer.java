...
public class Layer extends Vector<DrawObject> implements ILayerManipulator {
	...
	public boolean is_unique_gis_id(String gis) {
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof PlayerBase))
				continue;

			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase._gis.equals( gis))
				return false;
		}
		return true;
	}
	...
	public int how_many(String type) {
		int counter = 0;
		for ( DrawObject drawObject:this) {
			//if ( ( type.equals( "objectbase") && ( drawObject instanceof PlayerBase))
			if ( ( type.equals( "playerbase") && ( drawObject instanceof PlayerBase))
	 ...
	 */
	public PlayerBase get_playerBase_has_this_name(String fullName) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase && drawObject.has_same_name( fullName))
				return ( PlayerBase)drawObject;
		}
		return null;
	}
	...
	public boolean is_number_object_type_correct(String player, String numberObjectName, String newType) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof Role) {
				...
				if ( !role.is_number_object_type_correct( player, numberObjectName, newType))
					return false;
			}
		}
		return true;
	 ...
	 * @param playerBase the object(agent object or spot object)
	 * @return true if the number variable is correct
	 */
	public boolean is_number_object_correct(String type, String numberObjectName, String numberObjectType, PlayerBase playerBase) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase pb = ( PlayerBase)drawObject;
			if ( pb.equals( playerBase))
				continue;
	 ...
	 */
	public boolean update_number_object_type(String type, String numberObjectName, String numberObjectType, PlayerBase playerBase) {
	...
	public boolean contains(String type, String fullName) {
		for ( DrawObject drawObject:this) {
			if ( type.equals( "agent") || type.equals( "spot")) {
				if ( drawObject instanceof PlayerBase) {
					PlayerBase playerBase = ( PlayerBase)drawObject;
					if ( playerBase.has_same_name( fullName)
						/*|| playerBase._name.equals( full_name)*/)
						return true;
	...
	public boolean contains(String type, String name, String number, String exception) {
		for ( DrawObject drawObject:this) {
			...
			if ( type.equals( "agent") || type.equals( "spot")) {
				if ( drawObject instanceof PlayerBase) {
					PlayerBase playerBase = ( PlayerBase)drawObject;
					if ( playerBase.has_same_name( name, number))
						return true;
//				} else if ( drawObject instanceof Role) {
//					Role role = ( Role)drawObject;
...
//		return false;
	 ...
	 */
	public boolean contains(PlayerBase playerBase, String type, String name, String number) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)drawObject;
				if ( !pb.equals( playerBase)
...
//					return true;
	 ...
	 * @return
	 */
	public boolean contains(PlayerBase playerBase, String name, String number) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)drawObject;
				if ( !pb.equals( playerBase) && ( pb.contains( name, number)))
					return true;
			}
		}
		return false;
	...
	public boolean contains_as_object_name(String name, String number) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( playerBase.contains( name, number))
					return true;
			}
		}
		return false;
		...
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof Role) {
				...
				if ( !r.equals( role) && r.is_same_name( type, name))
					return true;
//			} else if ( drawObject instanceof PlayerBase) {
//				PlayerBase playerBase = ( PlayerBase)drawObject;
//				if ( playerBase.is_same_name( type, name) || playerBase.has_same_name( name))
//					return true;
			}
		}
		return false;
	...
	public boolean contains(ChartObject chartObject, String name) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( playerBase._name.equals( name) || playerBase.has_same_name( name))
					return true;
			} else if ( drawObject instanceof Role) {
				Role role = ( Role)drawObject;
...
//	}
	...
	public boolean is_multi(String type, String name) {
		...
		if ( type.equals( "agent"))
			playerBase = ( PlayerBase)_agentMap.get( name);
		else if ( type.equals( "spot"))
			playerBase = ( PlayerBase)_spotMap.get( name);

		if ( null == playerBase)
			return false;

		return playerBase.is_multi();
//		for ( DrawObject drawObject:this) {
//			if ( ( type.equals( "agent") || type.equals( "spot"))
//				&& ( drawObject instanceof PlayerBase)) {
//				PlayerBase playerBase = ( PlayerBase)drawObject;
//				if ( playerBase._name.equals( name))
//					return playerBase.is_multi();
//			}
//		}
//		return false;
	...
	public boolean object_contains_this_alias(String kind, String alias) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.object_contains_this_alias( kind, alias))
				return true;
	 ...
	 * @param playerBases the array for the selected agent and spot objects
	 */
	public void get_selected_playerBases(Vector<DrawObject> playerBases) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase && drawObject.selected())
				playerBases.add( drawObject);
		}
	}
	 ...
	 * @param playerBases the array for the all agent and spot objects
	 */
	public void get_playerBases(Vector<DrawObject> playerBases) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase)
				playerBases.add( drawObject);
		}
	 ...
	 * @param gis
	 */
	public void get_gis_spots(List<PlayerBase> playerBases, String gis) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof SpotObject) {
				SpotObject spotObject = ( SpotObject)drawObject;
	...
	public boolean initial_data_file_exists() {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.has_initial_data_file())
				return true;
		}
		return false;
	}
	...
	public void get_initial_data_files(List<String> initialDataFiles) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			playerBase.get_initial_data_files( initialDataFiles);
		}
	}
	...
	public void get_exchange_algebra_initial_data_files(List<String> exchangeAlgebraInitialDataFiles) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			playerBase.get_exchange_algebra_initial_data_files( exchangeAlgebraInitialDataFiles);
		}
	}
	...
	public void get_object_names(String kind, Vector<String> objectNames) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			playerBase.get_object_names( kind, objectNames);
		}
	}
	 ...
	 * @param playerBase the specified object
	 */
	public void get_object_names(String kind, Vector<String> objectNames, PlayerBase playerBase) {
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof PlayerBase))
				continue;
	 ...
	 * @param playerBase the specified object
	 */
	public void get_agent_object_names(String kind, Vector<String> agentObjectNames, PlayerBase playerBase) {
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof AgentObject))
				continue;

			AgentObject agentObject = ( AgentObject)drawObject;
			if ( agentObject.equals( playerBase))
				continue;
	 ...
	 * @param playerBase the specified object
	 */
	public void get_spot_object_names(String kind, Vector<String> spotObjectNames, PlayerBase playerBase) {
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof SpotObject))
				continue;

			SpotObject spotObject = ( SpotObject)drawObject;
			if ( spotObject.equals( playerBase))
				continue;
	...
	public ClassVariableObject get_class_variable(FunctionalObject functionalObject) {
		...
		for ( DrawObject drawObject:this) {
			...
			ClassVariableObject classVariableObject = playerBase.get_class_variable( functionalObject._classVariable);
	...
	public boolean is_object_name(String kind, String name) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( playerBase.has_same_object_name( kind, name))
					return true;
			}
		}
		return false;
	 ...
	 * @param kind the type of the object
	 * @param name the specified name
	 * @param playerBase ths specified object
	 * @return true if the object which has the specified name exists except ths specified one
	 */
	public boolean is_object_name(String kind, String name, PlayerBase playerBase) {
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)drawObject;
				if ( pb.equals( playerBase))
	...
	public void get_object_initial_values(String kind, Vector<String> initialValues) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			playerBase.get_object_initial_values( kind, initialValues);
		}
	}
	 ...
	 */
	public boolean other_uses_this_class_variable_as_different_class(String classVariable, String jarFilename, String classname, PlayerBase playerBase) {
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof AgentObject) && !( drawObject instanceof SpotObject))
				continue;

			if ( drawObject instanceof AgentObject && !( playerBase instanceof AgentObject))
				continue;

			if ( drawObject instanceof SpotObject && !( playerBase instanceof SpotObject))
				continue;

			PlayerBase pb = ( PlayerBase)drawObject;
			if ( !pb.equals( playerBase) && pb.uses_this_class_variable_as_different_class( classVariable, jarFilename, classname))
				return true;
		}
		return false;
	}
	...
	public boolean other_uses_this_class_variable_as_different_class(String classVariable, String jarFilename, String classname, String type) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.uses_this_class_variable_as_different_class( classVariable, jarFilename, classname))
				return true;
		}
		return false;
	}
	...
	public boolean uses_this_class(String jarFilename, String classname) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.uses_this_class( jarFilename, classname))
				return true;
		}
		return false;
	}
	...
	public boolean uses_this_file(File file) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.uses_this_file( file))
				return true;
		}
		return false;
	}
	...
	public boolean move_file(File srcPath, File destPath) {
		...
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.move_file( srcPath, destPath))
				result = true;
		}
		return result;
	}
	 ...
	 */
	public boolean can_adjust_agent_name(String headName, Vector ranges, boolean onRemove) {
		...
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( ( !onRemove || !playerBase.selected()) && !playerBase.can_adjust_agent_name( headName, ranges))
					result1 = false;
			} else if ( drawObject instanceof Role) {
				Role role = ( Role)drawObject;
	...
	public boolean can_adjust_agent_name(String headName, Vector ranges, String newHeadName, Vector newRanges) {
		...
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( !playerBase.can_adjust_agent_name( headName, ranges, newHeadName, newRanges))
					result1 = false;
			} else if ( drawObject instanceof Role) {
				Role role = ( Role)drawObject;
	 ...
	 */
	public boolean can_adjust_spot_name(String headName, Vector ranges, boolean onRemove) {
		boolean result1 = true;
		boolean result2 = true;
		boolean result3 = true;
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( ( !onRemove || !playerBase.selected()) && !playerBase.can_adjust_spot_name( headName, ranges))
					result1 = false;
			} else if ( drawObject instanceof Role) {
				Role role = ( Role)drawObject;
	...
	public boolean can_adjust_spot_name(String headName, Vector ranges, String newHeadName, Vector newRanges) {
		...
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( !playerBase.can_adjust_spot_name( headName, ranges, newHeadName, newRanges))
					result1 = false;
			} else if ( drawObject instanceof Role) {
				Role role = ( Role)drawObject;
	 ...
	 */
	public boolean can_remove(String kind, String name, boolean otherSpotsHaveThisObjectName, String headName, Vector ranges, boolean onRemove) {
		// TODO 従来のもの
		boolean result = true;
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof Role))
	 ...
	 * @return
	 */
	public boolean can_remove(String player, String kind, String objectName, boolean otherPlayersHaveThisObjectName, String headName, Vector ranges, boolean onRemove) {
		...
		for ( DrawObject drawObject:this) {
			...
			Role role = ( Role)drawObject;
			if ( ( !onRemove || !role.selected())
				&& !role.can_remove( player, kind, objectName, otherPlayersHaveThisObjectName, headName, ranges))
				result = false;
		}
		return result;
	}
	 ...
	 * @return true if the specified object can be removed
	 */
	public boolean can_remove(String kind, String type, String name, String headName, Vector ranges, boolean onRemove) {
		boolean result = true;
		for ( DrawObject drawObject:this) {
			if ( !( drawObject instanceof ChartObject))
				continue;
	 ...
	 * @param headName the prefix of the agent or spot name
	 * @param ranges the ranges for the agent or spot number
	 * @param playerBase the specified agent or spot object
	 * @param onRemove true for removing the object
	 * @return true if the specified object can be removed, except the specified agent or spot object
	 */
	public boolean can_remove(String kind, String name, boolean otherSpotsHaveThisObjectName, String headName, Vector ranges, PlayerBase playerBase, boolean onRemove) {
		// TODO 従来のもの
		boolean result1 = true;
		boolean result2 = true;
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)drawObject;
				if ( ( !onRemove || !pb.selected()) && !pb.equals( playerBase)
	 ...
	 * @return
	 */
	public boolean can_remove(String player, String kind, String objectName, boolean otherPlayersHaveThisObjectName, String headName, Vector ranges, PlayerBase playerBase, boolean onRemove) {
		// TODO これからはこちらに移行してゆく
		boolean result1 = true;
		boolean result2 = true;
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)drawObject;
				if ( ( !onRemove || !pb.selected()) && !pb.equals( playerBase)
					&& !pb.can_remove( kind, objectName, otherPlayersHaveThisObjectName, headName, ranges))	// このpb.can_remove( ... )は不要か？
					result1 = false;
			} else if ( drawObject instanceof Role) {
				Role role = ( Role)drawObject;
				if ( ( !onRemove || !role.selected())
	 ...
	 */
	public boolean update_object_name(String kind, String name, String newName, PlayerBase playerBase) {
		boolean result = false;
		for ( DrawObject drawObject:this) {
			if ( drawObject instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)drawObject;
				...
				if ( pb.update_object_name( kind, name, newName, ( ( playerBase instanceof AgentObject) ? "agent" : "spot"))) {
					String[] message = new String[] {
						( ( pb instanceof AgentObject) ? "Agent : " : "Spot : ") + pb._name
					};
				...
				Role role = ( Role)drawObject;
				if ( role.update_object_name( kind, name, newName, ( ( playerBase instanceof AgentObject) ? "agent" : "spot"))) {
					String[] message = new String[] {
	 ...
	 */
	public boolean update_number_object_name(String player, String name, String newName, PlayerBase playerBase) {
		boolean result = false;
		for ( int i = 0; i < size(); ++i) {
			if ( get( i) instanceof PlayerBase) {
				PlayerBase pb = ( PlayerBase)get( i);
				if ( pb.equals( playerBase))
					continue;

				if ( pb.update_object_name( "number object", name, newName, player)) {
					String[] message = new String[] {
						( ( pb instanceof AgentObject) ? "Agent : " : "Spot : ") + pb._name
					};
				...
				Role role = ( Role)get( i);
				if ( role.update_object_name( "number object", name, newName, player)) {
					String[] message = new String[] {
						( ( role instanceof AgentRole) ? "Agent role : " : "Spot role : ") + role._name
					};
				...
				if ( chartObject.update_object_name( "number object", player, name, newName)) {
					String[] message = new String[] {
	...
	public boolean can_remove_role(String name, boolean onRemove, Role role) {
		...
		for ( DrawObject drawObject:this) {
			if ( ( role instanceof AgentRole && drawObject instanceof AgentObject)
				...
				PlayerBase playerBase = ( PlayerBase)drawObject;
				if ( ( !onRemove || !playerBase.selected()) && !playerBase.can_remove_role_name( name))
					result1 = false;
			} else if ( drawObject instanceof Role) {
				Role r = ( Role)drawObject;
	...
	public boolean can_remove_expression(String function) {
		...
		for ( DrawObject drawObject:this) {
			...
			Role role = ( Role)drawObject;
			if ( !role.can_remove_expression( function))
				result = false;
		}
		return result;
	}
	...
	public boolean uses_this_image(String filename) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase.uses_this_image( filename))
				return true;
		}
		return false;
	}
	...
	public void update_image(String originalFilename, String newFilename) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			playerBase.update_image( originalFilename, newFilename);
		}
	}
	...
	public boolean transform_map_objects() {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( !playerBase.transform_map_objects())
				return false;
		}
		return true;
	}
	...
	public boolean is_initial_data_file_correct() {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( !playerBase.is_initial_data_file_correct())
				return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public String get_TEntityTypes(String text, Map<String, String> entityNameMap, File programRootFolder, String packagePrefix, Class cls, String className, String prefix) {
		// TODO Auto-generated method stub
		for ( DrawObject drawObject:this) {
			if ( !cls.isInstance( drawObject))
				continue;

			String name = JavaProgramExporter.format_variable_name( drawObject._name, prefix);
			entityNameMap.put( drawObject._name, className + "." + name);

			text += "\n" + JavaProgramExporter._indents[ 1] + "/**  */\n";
			text += JavaProgramExporter._indents[ 1] + "public static final String " + name + " = \"" + drawObject._name + "\";\n";
		}
		return text;
	}
	 ...
	 * @return
	 */
	public boolean agent_common_role_exists() {
		// TODO Auto-generated method stub
		for ( DrawObject drawObject:this) {
			if ( AgentRole.class.isInstance( drawObject) && drawObject._name.equals( ""))
				return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public boolean get_role_class_java_program(RoleDataSet roleDataSet, File programRootFolder, String packagePrefix, String stageType) {
		// TODO Auto-generated method stub
		for ( DrawObject drawObject:this) {
			if ( !AgentRole.class.isInstance( drawObject) && !SpotRole.class.isInstance( drawObject))
				continue;

			String roleClassName = get_same_roleClass_semantically( ( Role)drawObject, roleDataSet._roleClassNameMap, stageType);
			if ( null != roleClassName) {
				roleDataSet._roleClassNameMap.put( drawObject._name, roleClassName);
				//System.out.println( drawObject._name + " -> " + roleClassName);
			} else {
				List<PlayerBase> owners = LayerManager.get_instance().get_owner( ( Role)drawObject);
				if ( ( ( Role)drawObject)._name.equals( "") || has_same_objects( owners)) {
					// 無名ロールの場合
					// 初期ロールとして保持するEntityが１つだけの場合
					// 初期ロールとして保持するEntityは複数だが、その全てのEntityの持つ変数が同じ場合
					if ( !( ( ( Role)drawObject).get_role_class_java_program( roleDataSet, programRootFolder, packagePrefix, owners)))
						return false;
				} else {
					// TODO 新たなロールクラスを作成する(未実装)
					System.out.println( "新たなロールが必要：" + drawObject._name);
				}
			}
		}
		return true;
	 ...
	 * @return
	 */
	private String get_same_roleClass_semantically(Role role, Map<String, String> roleClassNameMap, String stageType) {
		// TODO Auto-generated method stub
		Set<String> names = roleClassNameMap.keySet();
		for ( String name:names) {
			if ( name.equals( ""))
				continue;

			Role r = LayerManager.get_instance().get_role( name);
			List<PlayerBase> owners1 = LayerManager.get_instance().get_owner( r);
			List<PlayerBase> owners2 = LayerManager.get_instance().get_owner( role);
			//if ( 1 == owners1.size() && 1 == owners2.size()) {
				if ( r.has_same_this_stage_rules( role, stageType) && owners1.get( 0).has_same_objects( owners2.get( 0)))
					// if ( rのロールとroleのロールのメインステージでの処理内容が完全に一致
					// && rを初期ロールとして持つAgentまたはSpotが保持する変数とroleを初期ロールとして持つAgentまたはSpotが保持する変数が完全に一致
					return roleClassNameMap.get( r._name);
			//} else {
				// TODO どうするか？
			//}
		}
	 ...
	 * @return
	 */
	private boolean has_same_objects(List<PlayerBase> owners) {
		// TODO Auto-generated method stub
		if ( 1 == owners.size())
			return true;

		for ( int i = 1; i < owners.size(); ++i) {
			if ( !owners.get( i).has_same_objects( owners.get( 0))) {
				return false;
			}
		}
	 ...
	 * @return
	 */
	public void get_owner(List<PlayerBase> entities, Role role) {
		// TODO Auto-generated method stub
		for ( DrawObject drawObject:this) {
			if ( PlayerBase.class.isInstance( drawObject) && role._name.equals( ( ( PlayerBase)drawObject)._initialRole))
				entities.add( ( PlayerBase)drawObject);
		}
	}
	 ...
	 * @return
	 */
	public boolean export_RoleClasses(RoleDataSet roleDataSet, File programRootFolder, String packagePrefix, Map<String, String> stageNameMap) {
		// TODO Auto-generated method stub
		for ( DrawObject drawObject:this) {
			if ( !AgentRole.class.isInstance( drawObject) && !SpotRole.class.isInstance( drawObject))
				continue;

			if ( !( ( ( Role)drawObject).export_RoleClass( roleDataSet, programRootFolder, packagePrefix, stageNameMap)))
					return false;
		}
		return true;
	}
	...
	public boolean get_ruleDataMap(RoleDataSet roleDataSet, Map<String, String> roleClassNameMap, String stageType) {
		...
		for ( DrawObject drawObject:this) {
			...
			if ( !( ( ( Role)drawObject).get_ruleDataMap( roleDataSet, roleClassNameMap, stageType)))
				return false;
		}
		return true;
	}
	...
	public boolean export_RuleClasses(RoleDataSet roleDataSet, Map<String, String> agentNameMap, Map<String, String> spotNameMap, InitialValueMap initialValueMap, File programRootFolder, String packagePrefix) {
		...
		for ( DrawObject drawObject:this) {
			...
			if ( !( ( Role)drawObject).export_RuleClasses( roleDataSet, agentNameMap, spotNameMap, initialValueMap, programRootFolder, packagePrefix))
				return false;
		}
		return true;
	}
	 ...
	 * @param variableName
	 */
	public void get_entities_have_this_variable(List<PlayerBase> entities, String type, String kind, String variableName) {
		// TODO Auto-generated method stub
		for ( DrawObject drawObject:this) {
			if ( ( type.equals( "agent") && AgentObject.class.isInstance( drawObject))
				|| ( type.equals( "spot") && SpotObject.class.isInstance( drawObject))) {
				if ( ( ( PlayerBase)drawObject).has_this_variable( kind, variableName))
					entities.add( ( PlayerBase)drawObject);
			}
		}
	}
	...
	public boolean get_imagefiles_selected_objects_use(List<File> imagefiles, List<File> thumbnailImagefiles) {
		Vector<DrawObject> playerBases = new Vector<DrawObject>();
		get_selected_playerBases( playerBases);
		for ( DrawObject drawObject:playerBases) {
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( playerBase._imageFilename.equals( ""))
				continue;

			File imagefile = new File( LayerManager.get_instance().get_image_directory(), playerBase._imageFilename);
			if ( !imagefile.exists() || !imagefile.isFile() || !imagefile.canRead())
				return false;

			File thumbnailImagefile = new File( LayerManager.get_instance().get_thumbnail_image_directory(), playerBase._imageFilename);
	...
	public boolean get_user_data_files_selected_objects_use(List<File> files) {
		...
		for ( DrawObject drawObject:playerBases) {
			PlayerBase playerBase = ( PlayerBase)drawObject;
			if ( !playerBase.get_user_data_files( files))
				return false;
		}
		return true;
	}
	...
	public void get_jar_filenames(List<String> list) {
		for ( DrawObject drawObject:this) {
			...
			PlayerBase playerBase = ( PlayerBase)drawObject;
			playerBase.get_jar_filenames( list);
		}
	}
	...
	public String get_graphic_properties(String type, Point originCoordinate) {
		...
		for ( DrawObject drawObject:this) {
			if ( ( type.equals( "role") && ( drawObject instanceof Role))
				...
				|| ( type.equals( "spot") && ( drawObject instanceof SpotObject))) {
				PlayerBase playerBase = ( PlayerBase)drawObject;
				text += ( type + "\t" + playerBase._name + "\t"
					+ playerBase._number + "\t"
					+ String.valueOf( playerBase._position.x - originCoordinate.x) + "\t"
					+ String.valueOf( playerBase._position.y - originCoordinate.y) + "\t"
					+ playerBase._imageFilename + "\n");
			}
		}
		return text;
	}
