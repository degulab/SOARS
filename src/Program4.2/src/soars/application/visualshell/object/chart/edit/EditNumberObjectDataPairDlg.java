...
public class EditNumberObjectDataPairDlg extends Dialog implements IObjectSelectorHandler {
	...
	private void setup_step_section(final int index, ButtonGroup buttonGroup) {
		...
		partial_panel.setLayout( new FlowLayout( FlowLayout.LEFT, 0, 0));
		...
		partial_panel.add( _radioButtons1[ index][ 2]);
	...
	private void update(ObjectSelector objectSelector) {
		...
		if ( null == playerBase || null == full_name || 0 == full_name.length()) {
			comboBox.removeAllItems();
			return;
		}

		CommonTool.update( comboBox, playerBase.get_object_names( "number object", full_name.substring( playerBase._name.length()), false));
	}
	...
	public void changed(String name, String number, String fullName, ObjectSelector objectSelector) {
		...
		PlayerBase playerBase = null;
		if ( objectSelector.equals( _agentSelector[ 0]) || objectSelector.equals( _agentSelector[ 1]))
			playerBase = LayerManager.get_instance().get_agent_has_this_name( fullName);
		else if ( objectSelector.equals( _spotSelector[ 0]) || objectSelector.equals( _spotSelector[ 1]))
			playerBase = LayerManager.get_instance().get_spot_has_this_name( fullName);
		else
			return;
		...
		CommonTool.update( comboBox, playerBase.get_object_names( "number object", number, false));
	}
