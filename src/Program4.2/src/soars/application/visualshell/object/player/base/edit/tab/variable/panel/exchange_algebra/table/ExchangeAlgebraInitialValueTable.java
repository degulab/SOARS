 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.table;
...
public class ExchangeAlgebraInitialValueTable extends InitialValueTable {
	 ...
	 * @param parent
	 */
	public ExchangeAlgebraInitialValueTable(Color color, PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, VariablePanelBase variablePanelBase, Frame owner, Component parent) {
		super("exchange algebra", color, playerBase, propertyPageMap, variablePanelBase, owner, parent);
	}
	...
	public boolean update_object_name(String type, String name, String new_name) {
		...
		for ( int i = 0; i < getRowCount(); ++i) {
			ExchangeAlgebraInitialValue exchangeAlgebraInitialValue = ( ExchangeAlgebraInitialValue)getValueAt( i, 0);
			if ( exchangeAlgebraInitialValue.update_object_name( type, name, new_name))
				result = true;
		}
		return result;
	}
	...
	public void paste(int[] rows) {
		...
		if ( 0 == getRowCount()) {
			for ( int i = 0; i < __initialValueBasesMap.get( _kind).size(); ++i) {
				...
				if ( !exchangeAlgebraInitialValue.can_paste( _propertyPageMap)) {
