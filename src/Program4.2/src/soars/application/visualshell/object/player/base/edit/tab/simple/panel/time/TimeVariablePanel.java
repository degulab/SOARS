 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.simple.panel.time;
...
public class TimeVariablePanel extends SimpleVariablePanel {
	 ...
	 * @param parent
	 */
	public TimeVariablePanel(PlayerBase playerBase, Map propertyPageMap, VariableTableBase variableTableBase, Color color, Frame owner, Component parent) {
		super("time variable", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	}
	...
	public void clear() {
		super.clear();
		_timeTextField.setText( "");
		for ( ComboBox omboBox:_timeComboBoxes)
			omboBox.setSelectedIndex( 0);
	}
	...
	protected ObjectBase create_and_get() {
		...
		if ( _timeTextField.getText().startsWith( "$"))
			initial_value = _timeTextField.getText();
		else {
			...
			initial_value += ( ( String)_timeComboBoxes.get( 0).getSelectedItem() + ":" + ( String)_timeComboBoxes.get( 1).getSelectedItem());
		}
		timeVariableObject._initialValue = initial_value;
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !timeVariableObject._name.equals( "") && !_nameTextField.getText().equals( timeVariableObject._name)) {
			_propertyPageMap.get( "variable").update_object_name( "time variable", timeVariableObject._name, _nameTextField.getText());

			WarningManager.get_instance().cleanup();

			boolean result = LayerManager.get_instance().update_object_name( "time variable", timeVariableObject._name, _nameTextField.getText(), _playerBase);
			if ( result) {
				if ( _playerBase.update_object_name( "time variable", timeVariableObject._name, _nameTextField.getText())) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( timeVariableObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], timeVariableObject._name, _playerBase))
				return false;
		}
