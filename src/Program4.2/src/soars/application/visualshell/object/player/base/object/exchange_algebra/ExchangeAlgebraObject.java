...
public class ExchangeAlgebraObject extends ObjectBase {
	...
	public boolean verify(PlayerDataMap agentDataMap, PlayerDataMap spotDataMap, PlayerData playerData) {
		for ( ExchangeAlgebraInitialValue exchangeAlgebraInitialValue:_exchangeAlgebraInitialValues) {
			if ( !exchangeAlgebraInitialValue.verify( _name, playerData))
				return false;
		}
		return true;
	}
	...
	public boolean verify(PlayerDataMap agentDataMap, PlayerDataMap spotDataMap, Vector indices, PlayerData playerData) {
		for ( ExchangeAlgebraInitialValue exchangeAlgebraInitialValue:_exchangeAlgebraInitialValues) {
			if ( !exchangeAlgebraInitialValue.verify( _name, indices, playerData))
				return false;
		}
		return true;
	}
	...
	public boolean can_paste(Map<String, PropertyPageBase> propertyPageMap) {
		for ( ExchangeAlgebraInitialValue exchangeAlgebraInitialValue:_exchangeAlgebraInitialValues) {
			if ( !exchangeAlgebraInitialValue.can_paste( propertyPageMap))
				return false;
		}
		return true;
	}
