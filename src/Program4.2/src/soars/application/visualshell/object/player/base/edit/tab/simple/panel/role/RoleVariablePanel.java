 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.simple.panel.role;
...
public class RoleVariablePanel extends SimpleVariablePanel {
	 ...
	 * @param parent
	 */
	public RoleVariablePanel(PlayerBase playerBase, Map propertyPageMap, VariableTableBase variableTableBase, Color color, Frame owner, Component parent) {
		super("role variable", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	}
	...
	/* (non-Javadoc)
	 * @see soars.application.visualshell.object.player.base.edit.tab.simple.panel.base.SimpleVariablePanel#on_update(soars.application.visualshell.object.player.base.object.base.ObjectBase)
	 */
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !roleVariableObject._name.equals( "") && !_nameTextField.getText().equals( roleVariableObject._name)) {
			_propertyPageMap.get( "variable").update_object_name( "role variable", roleVariableObject._name, _nameTextField.getText());

			WarningManager.get_instance().cleanup();

			boolean result = LayerManager.get_instance().update_object_name( "role variable", roleVariableObject._name, _nameTextField.getText(), _playerBase);
			if ( result) {
				if ( _playerBase.update_object_name( "role variable", roleVariableObject._name, _nameTextField.getText())) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( roleVariableObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], roleVariableObject._name, _playerBase))
				return false;
		}
