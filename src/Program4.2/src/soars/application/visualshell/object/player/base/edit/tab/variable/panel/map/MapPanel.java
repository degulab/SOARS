 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.variable.panel.map;
...
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.map.initial.MapInitialValueButtonPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.map.initial.MapInitialValueKeyPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.map.initial.MapInitialValueValuePanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.map.table.MapInitialValueTable;
...
public class MapPanel extends VariablePanelBase {
	 ...
	 * @param parent
	 */
	public MapPanel(PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, VariableTableBase variableTableBase, Color color, Frame owner, Component parent) {
		super("map", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	}
	...
	protected boolean setup_center_panel(JPanel parent) {
		_mapInitialValueTable = new MapInitialValueTable( _color, _playerBase, _propertyPageMap, this, _owner, _parent);
	...
	private void setup_left_panel(JPanel parent) {
		...
		insert_vertical_strut( north_panel);
		setup_mapInitialValuePanels( north_panel);
		insert_vertical_strut( north_panel);
	...
	private void setup_mapInitialValuePanels(JPanel parent) {
		_mapInitialValueKeyPanel = new MapInitialValueKeyPanel( _color, _playerBase, _propertyPageMap);
		_mapInitialValueValuePanel = new MapInitialValueValuePanel( _color, _playerBase, _propertyPageMap);
...
//		scrollPane.setRowHeader( viewport);
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !mapObject._name.equals( "") && !_nameTextField.getText().equals( mapObject._name)) {
			_propertyPageMap.get( "variable").update_object_name( "map", mapObject._name, _nameTextField.getText());

			WarningManager.get_instance().cleanup();

			boolean result = LayerManager.get_instance().update_object_name( "map", mapObject._name, _nameTextField.getText(), _playerBase);
			if ( result) {
				if ( _playerBase.update_object_name( "map", mapObject._name, _nameTextField.getText())) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( mapObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], mapObject._name, _playerBase))
				return false;
		}
		...
		if ( !mapObject.can_paste( _propertyPageMap, objectBase, objectBases, _playerBase))
			return false;
