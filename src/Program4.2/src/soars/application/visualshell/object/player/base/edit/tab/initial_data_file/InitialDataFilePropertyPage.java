 ...
 * @author kurata
 */
public class InitialDataFilePropertyPage extends PropertyPageBase implements IFileManagerCallBack {
	 ...
	 * 
	 */
	private InitialDataFileTable _initialDataFileTable = null;

	/**
	 * 
	 */
	private InitialDataFilePanel _initialDataFilePanel = null;
	 ...
	 * @param parent
	 */
	public InitialDataFilePropertyPage(String title, PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, int index, Frame owner, Component parent) {
	...
	protected void set_property_to_environment_file() {
		Environment.get_instance().set(
			Environment._editObjectDialogInitialDataFilePropertyPageDividerLocationKey, String.valueOf( _splitPane.getDividerLocation()));
		Environment.get_instance().set(
			Environment._editObjectDialogInitialDataFileManagerDividerLocationKey, String.valueOf( _fileManager.getDividerLocation()));
	}
...
//	}
	...
	protected boolean on_create() {
		...
		_splitPane.setDividerLocation( Integer.parseInt( Environment.get_instance().get( Environment._editObjectDialogInitialDataFilePropertyPageDividerLocationKey, "250")));
	...
	private void create_fileTable() {
		_initialDataFileTable = new InitialDataFileTable( _playerBase, _propertyPageMap, this, _owner, _parent);
	}
	...
	private boolean setup_fileTable(JPanel parent) {
		...
		if ( !_initialDataFileTable.setup())
			return false;

		if ( _playerBase.is_multi())
			_initialDataFileTable.setEnabled( false);

		JScrollPane scrollPane = new JScrollPane();
		scrollPane.getViewport().setView( _initialDataFileTable);
	...
	private boolean setup_components(JPanel parent) {
		_initialDataFilePanel = new InitialDataFilePanel( _playerBase, _propertyPageMap, _initialDataFileTable, _fileManager, SystemColor.textText, _owner, _parent);
		if ( !_initialDataFilePanel.setup())
			return false;

		_panelBaseMap.put( ResourceManager.get_instance().get( "edit.object.dialog.tree.initial.data.file"), _initialDataFilePanel);
		parent.add( _initialDataFilePanel);
	...
	public void changeSelection(ObjectBase objectBase) {
		changeSelection( ( InitialDataFileObject)objectBase, true);
	}

	/**
	 * @param initialDataFileObject
	 * @param update_all
	 */
	public void changeSelection(InitialDataFileObject initialDataFileObject, boolean update_all) {
		_initialDataFilePanel.update( initialDataFileObject);

		if ( !update_all)
			return;

		if ( null == initialDataFileObject)
			return;

		// FileManagerの表示も更新する必要がある
		_fileManager.select( initialDataFileObject._name.startsWith( "$") ? "" : initialDataFileObject._name);
	}
	...
	private boolean setup_fileManager(JPanel parent) {
		...
		_fileManager.setDividerLocation( Integer.parseInt( Environment.get_instance().get( Environment._editObjectDialogInitialDataFileManagerDividerLocationKey, "100")));
	...
	public void modified(IFileManager fileManager) {
		...
		_propertyPageMap.get( "file").refresh();	// 他のFileManagerを更新
	...
	public boolean can_remove(File file) {
		...
		return ( !_initialDataFileTable.uses_this_file( file)
			&& !_propertyPageMap.get( "file").uses_this_file( file)
	...
	public void on_move(File srcPath, File destPath) {
		...
		_initialDataFilePanel.update_initial_value( srcPath, destPath);
		_propertyPageMap.get( "file").update_file( srcPath, destPath);
		_initialDataFileTable.move_file( srcPath, destPath);
		_propertyPageMap.get( "file").move_file( srcPath, destPath);
		if ( Environment.get_instance().is_extransfer_enable()) {
...
//			fileEditorFrame.setIconImage( image);
	...
	public void on_setup_completed() {
		if ( _playerBase.is_multi()) {
			_initialDataFileTable.setEnabled( false);
			_initialDataFilePanel.setEnabled( false);
		} else {
			_fileManager.on_setup_completed();
			_initialDataFileTable.on_setup_completed();
		}
	}
...
//			Observer.get_instance().on_update_spot_object( "file");
	...
	public boolean confirm(boolean fromTree) {
		...
		if ( !_initialDataFilePanel.confirm( fromTree))
			return false;
	...
	public ObjectBase confirm(int row, ObjectBase targetObjectBase, ObjectBase selectedObjectBase) {
		return _initialDataFilePanel.confirm( row, targetObjectBase, selectedObjectBase);
	}
}
