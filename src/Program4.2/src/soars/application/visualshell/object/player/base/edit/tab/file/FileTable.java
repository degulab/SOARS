 ...
 * @author kurata
 */
public class FileTable extends VariableTableBase implements DropTargetListener {
	 ...
	 * @param parent
	 */
	public FileTable(PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, PropertyPageBase propertyPageBase, Frame owner, Component parent) {
		super("file", playerBase, propertyPageMap, propertyPageBase, 3, owner, parent);
	}
...
//		return LayerManager.get_instance().uses_this_file( file);
	...
	public void move_file(File srcPath, File destPath) {
		...
		for ( int i = 0; i < getRowCount(); ++i) {
			FileObject fileObject = ( FileObject)getValueAt( i, 0);
			fileObject.move_file( srcPath, destPath);
		}
...
//		return LayerManager.get_instance().move_file( srcPath, destPath);
	...
	public boolean setup() {
		...
		defaultTableColumnModel.getColumn( 0).setHeaderValue(
			ResourceManager.get_instance().get( "edit.object.dialog.file.table.header.name"));
		defaultTableColumnModel.getColumn( 1).setHeaderValue(
			ResourceManager.get_instance().get( "edit.object.dialog.file.table.header.initial.value"));
		defaultTableColumnModel.getColumn( 2).setHeaderValue(
			ResourceManager.get_instance().get( "edit.object.dialog.file.table.header.comment"));
		...
		if ( !_playerBase.is_multi()) {
			if ( !setup( _playerBase._objectMapMap.get( "file"), defaultTableModel))
				return false;
		}
	...
	private void update_initial_value(int row, File file) {
		...
		FileObject fileObject = ( FileObject)getValueAt( row, 0);
		fileObject._initialValue = ( file.getAbsolutePath().replaceAll( "\\\\", "/").substring( user_data_directory.getAbsolutePath().replaceAll( "\\\\", "/").length() + 1)
			+ ( file.isDirectory() ? "/" : ""));

		setRowSelectionInterval( row, row);
		( ( FilePropertyPage)_propertyPageBase).changeSelection( ( FileObject)getValueAt( row, 0), true);
	}
	 ...
	 */
	public void on_ok() {
		_playerBase._objectMapMap.get( "file").clear();

		for ( int i = 0; i < getRowCount(); ++i) {
			FileObject fileObject = ( FileObject)getValueAt( i, 0);
			_playerBase._objectMapMap.get( "file").put( fileObject._name, fileObject);
		}
	}
    ...
    DataFlavor[] dataFlavors = arg0.getCurrentDataFlavors();
		if ( dataFlavors[ 0].getHumanPresentableName().equals( DirectoryNodeTransferable._name)
			...
			try {
				...
				setRowSelectionInterval( row, row);
				( ( FilePropertyPage)_propertyPageBase).changeSelection( ( FileObject)getValueAt( row, 0), false);
