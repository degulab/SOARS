 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.simple.panel.keyword;
...
public class KeywordPanel extends SimpleVariablePanel {
	 ...
	 * @param parent
	 */
	public KeywordPanel(PlayerBase playerBase, Map propertyPageMap, VariableTableBase variableTableBase, Color color, Frame owner, Component parent) {
		super("keyword", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	}
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !keywordObject._name.equals( "") && !_nameTextField.getText().equals( keywordObject._name)) {
			...
			boolean result = LayerManager.get_instance().update_object_name( "keyword", keywordObject._name, _nameTextField.getText(), _playerBase);
			if ( result) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		if ( _playerBase instanceof AgentObject && ( keywordObject._name.equals( "$Name")
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( keywordObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], keywordObject._name, _playerBase))
				return false;
		}
