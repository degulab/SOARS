...
public class ClassVariableTable extends VariableTableBase implements DropTargetListener {
	 ...
	 * @param parent
	 */
	public ClassVariableTable(PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, PropertyPageBase propertyPageBase, Frame owner, Component parent) {
		super("class variable", playerBase, propertyPageMap, propertyPageBase, 4, owner, parent);
	}
	...
	public boolean setup() {
		...
		if ( !_playerBase.is_multi()) {
			if ( !setup( _playerBase._objectMapMap.get( "class variable"), defaultTableModel))
				return false;
		}
	 ...
	 */
	public void on_ok() {
		_playerBase._objectMapMap.get( "class variable").clear();

		DefaultTableModel defaultTableModel = ( DefaultTableModel)getModel();
		for ( int i = 0; i < defaultTableModel.getRowCount(); ++i) {
			ClassVariableObject classVariableObject = ( ClassVariableObject)defaultTableModel.getValueAt( i,  0);
			_playerBase._objectMapMap.get( "class variable").put( classVariableObject._name, classVariableObject);
		}
	}
    ...
    DataFlavor[] dataFlavors = arg0.getCurrentDataFlavors();
		if ( dataFlavors[ 0].getHumanPresentableName().equals( DomNodeTransferable._name)) {
			try {
				...
				( ( ClassVariablePropertyPage)_propertyPageBase).changeSelection( ( ClassVariableObject)getValueAt( row, 0), false);
	...
	private boolean update(int row, String jarfilename, String classname) {
		...
		setRowSelectionInterval( row, row);
		( ( ClassVariablePropertyPage)_propertyPageBase).changeSelection( ( ClassVariableObject)getValueAt( row, 0), true);
