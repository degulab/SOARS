 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra;
...
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.ExchangeAlgebraInitialValueButtonPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.HatPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.KeywordPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.NamePanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.SubjectPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.TimePanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.initial.UnitPanel;
import soars.application.visualshell.object.player.base.edit.tab.variable.panel.exchange_algebra.table.ExchangeAlgebraInitialValueTable;
...
public class ExchangeAlgebraPanel extends VariablePanelBase {
	 ...
	 * @param parent
	 */
	public ExchangeAlgebraPanel(PlayerBase playerBase, Map propertyPageMap, VariableTableBase variableTableBase, Color color, Frame owner, Component parent) {
		super("exchange algebra", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	}
	...
	protected boolean setup_center_panel(JPanel parent) {
		_exchangeAlgebraInitialValueTable = new ExchangeAlgebraInitialValueTable( _color, _playerBase, _propertyPageMap, this, _owner, _parent);
	...
	private void setup_left_panel(JPanel parent) {
		...
		insert_vertical_strut( north_panel);
		setup_value_textField( north_panel);
		insert_vertical_strut( north_panel);
		setup_base_type_comboBox( north_panel);
		insert_vertical_strut( north_panel);
		setup_exchangeAlgebraInitialValuePanels( north_panel);
		insert_vertical_strut( north_panel);
	...
	private void setup_exchangeAlgebraInitialValuePanels(JPanel parent) {
		_keywordPanel = new KeywordPanel( _color, _playerBase, _propertyPageMap);
		_namePanel = new NamePanel( _color, _playerBase, _propertyPageMap);
		_hatPanel = new HatPanel( _color, _playerBase, _propertyPageMap);
		_unitPanel = new UnitPanel( _color, _playerBase, _propertyPageMap);
		_timePanel = new TimePanel( _color, _playerBase, _propertyPageMap);
		_subjectPanel = new SubjectPanel( _color, _playerBase, _propertyPageMap);
...
//		scrollPane.setRowHeader( viewport);
	...
	/* (non-Javadoc)
	 * @see soars.application.visualshell.object.player.base.edit.tab.base.PanelBase#clear_textUndoRedoManagers()
	 */
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !exchangeAlgebraObject._name.equals( "") && !_nameTextField.getText().equals( exchangeAlgebraObject._name)) {
			_propertyPageMap.get( "variable").update_object_name( "exchange algebra", exchangeAlgebraObject._name, _nameTextField.getText());

			WarningManager.get_instance().cleanup();

			boolean result = LayerManager.get_instance().update_object_name( "exchange algebra", exchangeAlgebraObject._name, _nameTextField.getText(), _playerBase);
			if ( result) {
				if ( _playerBase.update_object_name( "exchange algebra", exchangeAlgebraObject._name, _nameTextField.getText())) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( exchangeAlgebraObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], exchangeAlgebraObject._name, _playerBase))
				return false;
		}
		...
		if ( !exchangeAlgebraObject.can_paste( _propertyPageMap))
			return false;
