 ...
 * 2005/05/13
 */
package soars.application.visualshell.object.player.base;
...
import soars.application.visualshell.object.player.base.edit.EditObjectDlg;
...
import soars.application.visualshell.object.player.base.object.initial_data_file.InitialDataFileChecker;
 ...
 */
public class PlayerBase extends DrawObject {
	 ...
	 */
	public PlayerBase(int id, String name, Point position, Graphics2D graphics2D) {
	 ...
	 * @param graphics2D the graphics object of JAVA
	 */
	public PlayerBase(int id, String name, Point position, String number, String initialRole, String imageFilename, String gis, String[] gisCoordinates, Graphics2D graphics2D) {
		super(id, name, position, imageFilename, graphics2D);
		_number = number;
	...
	/**
	 * Creates this object with the specified data.
	 * @param playerBase the specified data
	 */
	 ...
	 * @param playerBase
	 */
	private void copy(PlayerBase playerBase) {
		_multi = playerBase._multi;
		_number = playerBase._number;
		_initialRole = playerBase._initialRole;
		_others = playerBase._others;
		_gis = playerBase._gis;
		_gisCoordinates[ 0] = playerBase._gisCoordinates[ 0];
		_gisCoordinates[ 1] = playerBase._gisCoordinates[ 1];
		for ( String kind:Constant._kinds)
			copy( _objectMapMap.get( kind), playerBase._objectMapMap.get( kind));
		for ( String kind:Constant._exceptionalKinds)
			copy( _objectListMap.get( kind), playerBase._objectListMap.get( kind));
	}
	...
	private void copy(TreeMap<String, Object> destinationMap, TreeMap<String, Object> sourceMap) {
		...
		while ( iterator.hasNext()) {
			...
			if ( !is_multi())
				...
				HashMap<ObjectBase, Vector<int[]>> srcIndicesMap = ( HashMap<ObjectBase, Vector<int[]>>)entry.getValue();

				HashMap<ObjectBase, Vector<int[]>> dstIndicesMap = new HashMap<ObjectBase, Vector<int[]>>();
	 ...
	 * @param srcIndicesMap
	 */
	private void copy(HashMap<ObjectBase, Vector<int[]>> dstIndicesMap, HashMap<ObjectBase, Vector<int[]>> srcIndicesMap) {
	...
	public boolean is_number_object_correct(String numberObjectName, String numberObjectType) {
		if ( !is_multi()) {
			...
			HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)_objectMapMap.get( "number object").get( numberObjectName);
			if ( null == indicesMap)
				return false;
	...
	public static boolean is_number_object_correct(String type, String name, String numberObjectName, String numberObjectType, TreeMap<String, Object> treeMap, boolean multi) {
		if ( !multi) {
			...
			HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)treeMap.get( numberObjectName);
			if ( null == indicesMap)
				return true;
	...
	public void get_class_variable_map(Map<String, ClassVariableObject> classVariableMap) {
		...
		while ( iterator.hasNext()) {
			...
			if ( !is_multi()) {
				...
				HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)entry.getValue();
				Vector<ObjectBase> keys = new Vector<ObjectBase>( indicesMap.keySet());
				if ( keys.isEmpty())
					continue;
	...
	public ClassVariableObject get_class_variable(String name) {
		if ( !is_multi())
			...
			HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)_objectMapMap.get( "class variable").get( name);
			if ( null == indicesMap)
				return null;
	 ...
	 * @param playerData the specified data
	 * @return true for updating this object with the specified data
	 */
	public boolean update(PlayerData playerData) {
		update_initial_role( playerData);
		update_number( playerData);
		for ( String kind:Constant._kinds)
			update( _objectMapMap.get( kind), playerData._objectMapMap.get( kind));
		return true;
	}
	 ...
	 * @param playerData
	 */
	private void update_initial_role(PlayerData playerData) {
		if ( !playerData._initialRole.equals( ""))
			_initialRole = playerData._initialRole;
	}

	/**
	 * @param playerData
	 */
	private void update_number(PlayerData playerData) {
		if ( playerData._number.equals( ""))
			return;

		if ( !playerData.is_multi())
			_number = playerData._number;
		else {
			if ( _number.equals( ""))
				_number = playerData._number;
			else {
				int number = Integer.parseInt( _number);
				if ( Integer.parseInt( _number) < Integer.parseInt( playerData._number))
					_number = playerData._number;
			}
		}
	}
	...
	private void update(TreeMap<String, Object> destinationMap, TreeMap<String, Object> sourceMap) {
		...
		while ( iterator.hasNext()) {
			...
			if ( !is_multi()) {
				...
				HashMap<ObjectBase, Vector<int[]>> dstIndicesMap = ( HashMap<ObjectBase, Vector<int[]>>)destinationMap.get( name);
	 ...
	 * @param srcIndicesMap
	 */
	private void update(HashMap<ObjectBase, Vector<int[]>> dstIndicesMap, HashMap<ObjectBase, Vector<int[]>> srcIndicesMap) {
		Iterator iterator = srcIndicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
			...
			PlayerData.arrange( dstIndicesMap, srcIndices, dstIndices);
		}
	}
	 ...
	 * @return
	 */
	private boolean make_file(HashMap<ObjectBase, Vector<int[]>> srcIndicesMap) {
		boolean result = true;
		Iterator iterator = srcIndicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
...
//	}
	 ...
	 */
	public Vector get_object_ranges(String kind, String name) {
		if ( !is_multi())
			return new Vector();
		...
		return PlayerData.get_object_ranges( treeMap, name);
	}
	 ...
	 * @return
	 */
	private boolean contains(HashMap<ObjectBase, Vector<int[]>> indicesMap, int number) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 */
	private void get_exchange_algebra_names(HashMap<ObjectBase, Vector<int[]>> indicesMap, String name, int number, Vector<String> exchangeAlgebraNames) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean contains(HashMap<ObjectBase, Vector<int[]>> indicesMap, String type, int number) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @param exchangeAlgebraNames
	 */
	private void get_exchange_algebra_names(HashMap<ObjectBase, Vector<int[]>> indicesMap, String name, Vector<String> exchangeAlgebraNames) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean is_exchange_algebra(HashMap<ObjectBase, Vector<int[]>> indicesMap, String name) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean contains(HashMap<ObjectBase, Vector<int[]>> indicesMap, String type) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean object_contains_this_alias(HashMap<ObjectBase, Vector<int[]>> indicesMap, String alias) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @param initialValues
	 */
	private void get_object_initial_values1(HashMap<ObjectBase, Vector<int[]>> indicesMap, Vector<String> initialValues) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	...
	private void get_object_initial_values2(TreeMap<String, Object> treeMap, String name, Vector<String> initialValues) {
		if ( !is_multi()) {
			...
			HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)treeMap.get( name);
	 ...
	 * @param initialValues
	 */
	private void get_object_initial_values2(HashMap<ObjectBase, Vector<int[]>> indicesMap, Vector<String> initialValues) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	...
	private void get_object_initial_values2(String type, String name, Vector<String> initialValues) {
		if ( !is_multi()) {
			...
			HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)_objectMapMap.get( "number object").get( name);
	 ...
	 */
	private void get_number_object_initial_values(String type, HashMap<ObjectBase, Vector<int[]>> indicesMap, Vector<String> initialValues) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean uses_this_class_variable_as_different_class(HashMap<ObjectBase, Vector<int[]>> indicesMap, String classVariable, String jarFilename, String classname) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean uses_this_class(HashMap<ObjectBase, Vector<int[]>> indicesMap, String jarFilename, String classname) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean uses_this_file(HashMap<ObjectBase, Vector<int[]>> indicesMap, File file) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean move_file(HashMap<ObjectBase, Vector<int[]>> indicesMap, File srcPath, File destPath) {
		boolean result = false;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean get_user_data_files(HashMap<ObjectBase, Vector<int[]>> indicesMap, List<File> files) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean can_adjust_name(String type, String headName, Vector ranges) {
		boolean result = true;
		for ( String kind:Constant._kinds) {
	 ...
	 * @return
	 */
	private boolean can_adjust_name(TreeMap<String, Object> treeMap, String type, String headName, Vector ranges) {
		boolean result = true;
		Iterator iterator = treeMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean can_adjust_name(HashMap<ObjectBase, Vector<int[]>> indicesMap, String type, String headName, Vector ranges) {
		boolean result = true;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean can_adjust_name(String type, String headName, Vector ranges, String newHeadName, Vector newRanges) {
		boolean result = true;
		for ( String kind:Constant._kinds) {
	 ...
	 * @return
	 */
	private boolean can_adjust_name(TreeMap<String, Object> treeMap, String type, String headName, Vector ranges, String newHeadName, Vector newRanges) {
		boolean result = true;
		Iterator iterator = treeMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean can_adjust_name(HashMap<ObjectBase, Vector<int[]>> indicesMap, String type, String headName, Vector ranges, String newHeadName, Vector newRanges) {
		boolean result = true;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean update_name_and_number(String type, String newName, String originalName, String headName, Vector ranges, String newHeadName, Vector newRanges) {
		boolean result = false;
		for ( String kind:Constant._kinds) {
...
/**
	 ...
	 * @return
	 */
	private boolean update_name_and_number(TreeMap<String, Object> treeMap, String type, String newName, String originalName, String headName, Vector ranges, String newHeadName, Vector newRanges) {
		boolean result = false;
		Iterator iterator = treeMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean update_name_and_number(HashMap<ObjectBase, Vector<int[]>> indicesMap, String type, String newName, String originalName, String headName, Vector ranges, String newHeadName, Vector newRanges) {
		boolean result = false;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	...
	private boolean can_remove_object_names_new(String kind, TreeMap<String, Object> treeMap) {
		...
		String headNumber = _name.substring( headName.length());
		Vector ranges = SoarsCommonTool.get_ranges( headNumber, _number);
	...
	protected boolean can_remove_role_variable_names_old() {
		...
		Vector ranges = null;
//		if ( this instanceof SpotObject) {
//			headName = PlayerBase.separate( _name);
//			String head_number = _name.substring( head_name.length());
//			ranges = PlayerBase.get_ranges( head_number, _number);
//		}
	...
	public boolean can_remove_role_name(String roleName) {
		...
		if ( !is_multi()) {
			...
			while ( iterator.hasNext()) {
				...
				Map.Entry entry = ( Map.Entry)object;
				HashMap<ObjectBase, Vector<int[]>> indicesMap = ( HashMap<ObjectBase, Vector<int[]>>)entry.getValue();
				if ( contains_role_name( indicesMap, roleName)) {
//				if ( indicesMap.containsKey( roleName)) {
	 ...
	 * @return
	 */
	private boolean contains_role_name(HashMap<ObjectBase, Vector<int[]>> indicesMap, String roleName) {
		// TODO Auto-generated method stub
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
			RoleVariableObject roleVariableObject = ( RoleVariableObject)entry.getKey();
			if ( roleVariableObject._initialValue.equals( roleName))
				return true;
		}
		return false;
	 ...
	 */
	public boolean update_object_name(String kind, String name, String newName, String player) {
		if ( name.equals( newName))
			return false;

		if ( ( player.equals( "agent") && this instanceof AgentObject)
			|| ( player.equals( "spot") && this instanceof SpotObject))
	 ...
	 * @param indicesMap
	 */
	private void update_object_name(String newName, HashMap<ObjectBase, Vector<int[]>> indicesMap) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @return
	 */
	private boolean update_variable_initial_value(String type, String name, String newName, HashMap<ObjectBase, Vector<int[]>> indicesMap) {
		boolean result = false;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean can_paste(HashMap<ObjectBase, Vector<int[]>> indicesMap, Layer drawObjects) {
		boolean result = true;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private boolean update_role_name(String originalName, String name, HashMap<ObjectBase, Vector<int[]>> indicesMap) {
		boolean result = false;
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @param indicesMap
	 */
	private void on_remove_role_name(Vector<String> roleNames, HashMap<ObjectBase, Vector<int[]>> indicesMap) {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	...
	public String get_initial_data() {
		...
		for ( String kind:Constant._kinds)
			script += get_initial_data( _objectMapMap.get( kind), entity + "\t" + _name + "\t");
	 ...
	 * @return
	 */
	private String get_initial_data(String prefix, HashMap<ObjectBase, Vector<int[]>> indicesMap) {
		String script = "";
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private String get_script(int index, HashMap<ObjectBase, Vector<int[]>> indicesMap, IntBuffer counter, InitialValueMap initialValueMap) {
		String script = "";
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @return
	 */
	private String get_variable_initial_values_script(int index, HashMap<ObjectBase, Vector<int[]>> indicesMap, InitialValueMap initialValueMap) {
		String script = "";
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
	 ...
	 * @param values
	 */
	public void append_object(String kind, String[] values) {
		ObjectBase objectBase = ObjectBase.create( kind);
		if ( null == objectBase)
			return;

		if ( kind.equals( "keyword")) {
			KeywordObject keywordObject = ( KeywordObject)objectBase;
			keywordObject._name = values[ 0];
			keywordObject._initialValue = values[ 1];
			keywordObject._comment = values[ 2];
		} else if ( kind.equals( "number object")) {
			NumberObject numberObject = ( NumberObject)objectBase;
			numberObject._name = values[ 0];
			numberObject._type = values[ 1];
			numberObject._initialValue = values[ 2];
			numberObject._comment = values[ 3];
		} else
			return;

		append_object( objectBase);
	}
	...
	public void append_object(ObjectBase objectBase, String number) {
		...
		PlayerData.append_object( _objectMapMap.get( objectBase._kind), objectBase, indices);
	}
	 ...
	 * @throws SAXException
	 */
	private void write(HashMap<ObjectBase, Vector<int[]>> indicesMap, Writer writer) throws SAXException {
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
	 ...
	 * @param playerBase
	 */
	public void update_on_gis(PlayerBase playerBase) {
		if ( this == playerBase)
			return;

		_comment = playerBase._comment;
		_others = playerBase._others;
	}
	 ...
	 * @return
	 */
	public boolean has_same_objects(PlayerBase playerBase) {
		// TODO Auto-generated method stub
		if ( is_multi())
			return false;
		...
		for ( String kind:Constant._kinds) {
			TreeMap<String, Object> treeMap = _objectMapMap.get( kind);
			//System.out.println( _name + " : " + kind + " : " + treeMap.size() + " - " + playerBase._objectMapMap.get( kind).size());
			if ( treeMap.size() != playerBase._objectMapMap.get( kind).size())
				return false;
			...
			while ( iterator.hasNext()) {
				...
				Map.Entry entry = ( Map.Entry)object;
				ObjectBase objectBase = ( ObjectBase)entry.getValue();
				if ( !playerBase.has_same_object( kind, objectBase))
					return false;
			}
		}
		return true;
	 ...
	 * @return
	 */
	private boolean has_same_object(String kind, ObjectBase objectBase) {
		// TODO Auto-generated method stub
		TreeMap<String, Object> treeMap = _objectMapMap.get( kind);
		ObjectBase ob = ( ObjectBase)treeMap.get( objectBase._name);
		if ( null == ob)
			return false;

		return ob.same_as( objectBase);
	}

	/**
	 * @param commonRoleClassName 
	 * @param agentCommonVariableMap
	 * @param variableMap 
	 * @param agents 
	 */
	public void get_common_variables(String commonRoleClassName, Map<String, TreeMap<String, Object>> agentCommonVariableMap, Map<String, Map<String, Map<String, List<String>>>> variableMap, Vector<DrawObject> agents) {
		// TODO Auto-generated method stub
		for ( String kind:Constant._kinds) {
			TreeMap<String, Object> treeMap = _objectMapMap.get( kind);
			Iterator iterator = treeMap.entrySet().iterator();
			while ( iterator.hasNext()) {
				...
				Map.Entry entry = ( Map.Entry)object;
				ObjectBase objectBase = ( ObjectBase)entry.getValue();
				boolean hasSameObject = true;
				for ( int i = 1; i < agents.size(); ++i) {	// 配列の先頭は自分自身
					if ( !( ( ( PlayerBase)agents.get( i)).has_same_object( kind, objectBase))) {
						hasSameObject = false;
						break;
					}
				}
				if ( hasSameObject) {
					if ( null == agentCommonVariableMap.get( kind))
						agentCommonVariableMap.put( kind, new TreeMap<>());
					agentCommonVariableMap.get( kind).put( objectBase._name, objectBase/*new ObjectBase( objectBase)*/);
					get_variable( commonRoleClassName, kind, objectBase, variableMap, AgentObject.class.isInstance( this) ? "agent" : "spot");
				}
			}
		}
	}
	 ...
	 * @param variableMap
	 */
	public void get_variables(String commonRoleClassName, Map<String, Map<String, Map<String, List<String>>>> variableMap) {
		// TODO Auto-generated method stub
		for ( String kind:Constant._kinds) {
			TreeMap<String, Object> treeMap = _objectMapMap.get( kind);
			Iterator iterator = treeMap.entrySet().iterator();
			while ( iterator.hasNext()) {
				Object object = iterator.next();
				Map.Entry entry = ( Map.Entry)object;
				get_variable( commonRoleClassName, kind, ( ObjectBase)entry.getValue(), variableMap, AgentObject.class.isInstance( this) ? "agent" : "spot");
			}
		}
	}
	 ...
	 * @param variableMap
	 */
	private static void get_variable(String roleClassName, String kind, ObjectBase objectBase, Map<String, Map<String, Map<String, List<String>>>> variableMap, String type) {
		// TODO Auto-generated method stub
		Map<String, Map<String, List<String>>> map = variableMap.get( type);
		if ( null == map) {
			map = new HashMap<>();
			variableMap.put( type, map);
		}
		Map<String, List<String>> m = map.get( roleClassName);
		if ( null == m) {
			m = new HashMap<String, List<String>>();
			map.put( roleClassName, m);
		}
		List<String> list = m.get( kind);
		if ( null == list) {
			list = new ArrayList<String>();
			m.put( kind, list);
		}
		if ( !list.contains( "f" + objectBase._name))
			list.add( "f" + objectBase._name);
	}
	 ...
	 * @return
	 */
	public String get_variable_definitions(String roleClassName, Map<String, TreeMap<String, Object>> agentCommonVariableMap, Map<String, Map<String, Map<String, List<String>>>> variableMap, List<Variable> variables, String type) {
		// TODO Auto-generated method stub
		return get_variable_definitions( roleClassName, _objectMapMap, agentCommonVariableMap, variableMap, variables, type);
	}
	 ...
	 * @return
	 */
	public static String get_variable_definitions(String roleClassName, Map<String, TreeMap<String, Object>> objectMapMap, Map<String, TreeMap<String, Object>> agentCommonVariableMap, Map<String, Map<String, Map<String, List<String>>>> variableMap, List<Variable> variables, String type) {
		// TODO Auto-generated method stub
		String text = "";
		for ( String kind:Constant._kinds) {
			TreeMap<String, Object> treeMap = objectMapMap.get( kind);
			...
			while ( iterator.hasNext()) {
				...
				Map.Entry entry = ( Map.Entry)object;
				ObjectBase objectBase = ( ObjectBase)entry.getValue();
				if ( null != agentCommonVariableMap && type.equals( "agent")) {
					TreeMap<String, Object> tm = agentCommonVariableMap.get( kind);
					if ( null == tm)
						continue;
					if ( null != tm.get( objectBase._name))
						continue;
				}
				text += JavaProgramExporter._indents[ 1] + "/**  */\n";
				text += JavaProgramExporter._indents[ 1] + "private " + get_variable_type( kind) + " f" + objectBase._name + ";\n";
				variables.add( new Variable( get_variable_type( kind), "f" + objectBase._name));
				get_variable( roleClassName, kind, objectBase, variableMap, type);
			}
		}
		//private double fProbabilityISO2;
		return ( text.equals( "") ? "" : "\n") + text;
	}
	 ...
	 * @return
	 */
	public static String get_variable_type(String kind) {
		// TODO Auto-generated method stub
		switch ( kind) {
			case "probability":
			case "number object":
	 ...
	 * @return
	 */
	public static String get_default_return_value(String kind) {
		// TODO Auto-generated method stub
		switch ( kind) {
			case "probability":
			case "number object":
	 ...
	 * @return
	 */
	public List<String> get_imports(Map<String, TreeMap<String, Object>> agentCommonVariableMap) {
		// TODO Auto-generated method stub
		return get_imports( _objectMapMap, agentCommonVariableMap);
	}
	 ...
	 * @return
	 */
	public static List<String> get_imports(Map<String, TreeMap<String, Object>> objectMapMap, Map<String, TreeMap<String, Object>> agentCommonVariableMap) {
		// TODO Auto-generated method stub
		...
		for ( String kind:Constant._kinds) {
			...
			if ( null == treeMap || treeMap.isEmpty())
				continue;
			if ( null == agentCommonVariableMap)
				get_import( kind, imports);
			else {
				TreeMap<String, Object> tm = agentCommonVariableMap.get( kind);
	 ...
	 * @param imports
	 */
	public static void get_import(String kind, List<String> imports) {
		// TODO Auto-generated method stub
		String text = get_import1( kind);
		if ( text.equals( "") || imports.contains( text))
			return;
	 ...
	 * @return
	 */
	public static String get_import1(String kind) {
		// TODO Auto-generated method stub
		switch ( kind) {
			case "probability":
			case "number object":
	 ...
	 * @return
	 */
	public static String get_import2(String type) {
		// TODO Auto-generated method stub
	 ...
	 * @return
	 */
	public boolean has_this_variable(String kind, String variableName) {
		// TODO Auto-generated method stub
		if ( null == _objectMapMap.get( kind))
			return false;
		return ( null != _objectMapMap.get( kind).get( variableName));
	}
	...
	public void print() {
		...
		for ( String kind:Constant._kinds)
			PlayerData.print_object( _objectMapMap.get( kind), is_multi());
	}
}
