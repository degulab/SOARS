...
public class ExTransferPanel extends PanelBase {
	 ...
	 * @param parent
	 */
	public ExTransferPanel(PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, VariableTableBase variableTableBase, FileManager fileManager, Color color, Frame owner, Component parent) {
		super("extransfer", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	...
	private void setup_initialValueTextField(JPanel parent) {
		...
		_initialValueTextField = !_playerBase.is_multi() ? new FileDDTextField() : new JTextField();
	...
	protected boolean can_get_data(ObjectBase objectBase) {
		...
		String[] property_pages = Constant.get_property_pages( "extransfer");
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !exTransferObject._name.equals( "") && !_nameTextField.getText().equals( exTransferObject._name)) {
			...
			_propertyPageMap.get( "variable").update_object_name( "extransfer", exTransferObject._name, _nameTextField.getText());

			boolean result = LayerManager.get_instance().update_object_name( "extransfer", exTransferObject._name, _nameTextField.getText(), _playerBase);
			if ( result) {
				if ( _playerBase.update_object_name( "extransfer", exTransferObject._name, _nameTextField.getText())) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		if ( _playerBase instanceof AgentObject && ( exTransferObject._name.equals( "$Name")
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( exTransferObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], exTransferObject._name, _playerBase))
				return false;
		}
