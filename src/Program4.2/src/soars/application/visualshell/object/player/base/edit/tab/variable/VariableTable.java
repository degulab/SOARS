...
public class VariableTable extends VariableTableBase {
	 ...
	 * @param parent
	 */
	public VariableTable(PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, PropertyPageBase propertyPageBase, Frame owner, Component parent) {
		super("variable", playerBase, propertyPageMap, propertyPageBase, 4, owner, parent);
	}
	...
	public boolean setup() {
		...
		if ( !_playerBase.is_multi()) {
			if ( !setup( _playerBase._objectMapMap.get( "collection"), defaultTableModel))
				return false;
			if ( !setup( _playerBase._objectMapMap.get( "list"), defaultTableModel))
				return false;
			if ( !setup( _playerBase._objectMapMap.get( "map"), defaultTableModel))
				return false;
			if ( Environment.get_instance().is_exchange_algebra_enable()
				&& !setup( _playerBase._objectMapMap.get( "exchange algebra"), defaultTableModel))
				return false;
		}
	 ...
	 */
	public void on_ok() {
		_playerBase._objectMapMap.get( "collection").clear();
		_playerBase._objectMapMap.get( "list").clear();
		_playerBase._objectMapMap.get( "map").clear();
		_playerBase._objectMapMap.get( "exchange algebra").clear();
		...
		for ( int i = 0; i < defaultTableModel.getRowCount(); ++i) {
			...
			if ( objectBase instanceof VariableObject && objectBase._kind.equals( "collection"))
				_playerBase._objectMapMap.get( "collection").put( objectBase._name, objectBase);
			else if ( objectBase instanceof VariableObject && objectBase._kind.equals( "list"))
				_playerBase._objectMapMap.get( "list").put( objectBase._name, objectBase);
			else if ( objectBase instanceof MapObject)
				_playerBase._objectMapMap.get( "map").put( objectBase._name, objectBase);
			else if ( objectBase instanceof ExchangeAlgebraObject)
				_playerBase._objectMapMap.get( "exchange algebra").put( objectBase._name, objectBase);
		}
	}
}
