 ...
 * 
 */
package soars.application.visualshell.object.player.base.object.exchange_algebra;
...
//		return base;
	 ...
	 * @return
	 */
	public boolean verify(String name, PlayerData playerData) {
		if ( _keyword.equals( ""))
			return true;

		if ( playerData.has_same_object_name( "keyword", _keyword))
			return true;
		...
		if ( null == playerBase || playerBase.is_multi() || !playerBase.has_same_object_name( "keyword", _keyword)) {
	 ...
	 * @return
	 */
	public boolean verify(String name, Vector indices, PlayerData playerData) {
		if ( _keyword.equals( ""))
			return true;

		if ( !verify( "keyword", _keyword, indices, playerData)) {
			String[] message = new String[] {
				( playerData._type.equals( "agent") ? "Agent" : "Spot"),
				"name = " + playerData._name,
				"exchange algebra name = " + name,
	 ...
	 * @return
	 */
	private boolean verify(String kind, String name, Vector indices, PlayerData playerData) {
		Vector objectRanges = playerData.get_object_ranges( kind, name);
		...
		if ( null != playerBase && playerBase.is_multi()) {
			Vector ranges = playerBase.get_object_ranges( kind, name);
	 ...
	 * @return
	 */
	public boolean can_paste(Map<String, PropertyPageBase> propertyPageMap) {
		if ( _keyword.equals( ""))
			return true;

		PropertyPageBase propertyPageBase = propertyPageMap.get( "simple variable");
		return ( ( null != propertyPageBase) && propertyPageBase.contains( _keyword));
	}
}
