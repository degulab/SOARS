 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.simple.panel.number;
...
public class NumberVariablePanel extends SimpleVariablePanel {
	 ...
	 * @param parent
	 */
	public NumberVariablePanel(PlayerBase playerBase, Map propertyPageMap, VariableTableBase variableTableBase, Color color, Frame owner, Component parent) {
		super("number object", playerBase, propertyPageMap, variableTableBase, color, owner, parent);
	}
	...
	protected boolean can_get_data(ObjectBase objectBase) {
		...
		if ( !numberObject._type.equals( ( String)_typeComboBox.getSelectedItem())) {
			if ( !LayerManager.get_instance().is_number_object_type_correct( ( _playerBase instanceof AgentObject) ? "agent" : "spot", numberObject._name, ( String)_typeComboBox.getSelectedItem())) {
				JOptionPane.showMessageDialog( _parent,
	...
	protected void get_data(ObjectBase objectBase) {
		...
		if ( !numberObject._name.equals( "") && !_nameTextField.getText().equals( numberObject._name))
			result1 = LayerManager.get_instance().update_number_object_name(
				( _playerBase instanceof AgentObject) ? "agent" : "spot",
					numberObject._name, _nameTextField.getText(), _playerBase);
		...
		if ( !numberObject._type.equals( "") && !numberObject._type.equals( NumberObject.get_type( type)))
			result2 = LayerManager.get_instance().update_number_object_type(
				( _playerBase instanceof AgentObject) ? "agent" : "spot",
				_nameTextField.getText(), NumberObject.get_type( type), _playerBase);
		if ( result1 || result2) {
	...
	public boolean can_paste(ObjectBase objectBase, List<ObjectBase> objectBases) {
		...
		for ( int i = 0; i < property_pages.length; ++i) {
			PropertyPageBase propertyPageBase = _propertyPageMap.get( property_pages[ i]);
			if ( null != propertyPageBase && propertyPageBase.contains( numberObject._name))
				return false;
		}
		...
		for ( int i = 0; i < kinds.length; ++i) {
			if ( LayerManager.get_instance().is_object_name( kinds[ i], numberObject._name, _playerBase))
				return false;
		}
