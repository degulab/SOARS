 ...
 * 
 */
package soars.application.visualshell.object.player.base.edit.tab.variable.panel.set_and_list.base.panel.table;
...
public class VariableInitialValueTable extends InitialValueTable {
	 ...
	 * @param parent
	 */
	public VariableInitialValueTable(Color color, PlayerBase playerBase, Map<String, PropertyPageBase> propertyPageMap, VariablePanelBase variablePanelBase, Frame owner, Component parent) {
		super("variable", color, playerBase, propertyPageMap, variablePanelBase, owner, parent);
	}
	...
	public boolean update_object_name(String type, String name, String new_name) {
		...
		for ( int i = 0; i < getRowCount(); ++i) {
			VariableInitialValue variableInitialValue = ( VariableInitialValue)getValueAt( i, 0);
			if ( variableInitialValue.update_object_name( type, name, new_name))
				result = true;
		}
		return result;
	}
	...
	public void paste(int[] rows) {
		...
		if ( 0 == getRowCount()) {
			for ( int i = 0; i < __initialValueBasesMap.get( _kind).size(); ++i) {
				...
				if ( !variableInitialValue.can_paste( _propertyPageMap, null, null, _playerBase)) {
					String[] message = new String[] {
						PropertyPageBase._nameMap.get( variableInitialValue._type),
