...
//	protected ComboBox _stageComboBox = null;
	...
	/**
	 * @param src_column_widths
	 * @return
	 */
	private Vector copy(Vector src_column_widths) {
		Vector dst_column_widths = new Vector();
		for ( int i = 0; i < src_column_widths.size(); ++i) {
			Integer integer = ( Integer)src_column_widths.get( i);
			dst_column_widths.add( Integer.valueOf( integer.intValue()));
		}
		return dst_column_widths;
	}
...
//	}
	 ...
	 * @param north_panel
	 */
	private void setup_roleCommentTextField(JPanel north_panel) {
		JPanel panel = new JPanel();
		panel.setLayout( new BoxLayout( panel, BoxLayout.X_AXIS));
...
//		});
	...
	private boolean setup_condition() {
		if ( !_conditionRuleTree.setup( _conditionTabbedPane,
			ResourceManager.get_instance().get( "edit.rule.dialog.condition.rule.tree.root.text")))
			return false;
		...
		if ( !_conditionTabbedPane.setup( _conditionRuleTree, _role, this, this))
			return false;
	...
	private boolean setup_command() {
		if ( !_commandRuleTree.setup( _commandTabbedPane,
			ResourceManager.get_instance().get( "edit.rule.dialog.command.rule.tree.root.text")))
			return false;
		...
		if ( !_commandTabbedPane.setup( _commandRuleTree, _role, this, this))
			return false;
	...
	private void setup_south_panel() {
		...
		setup_editStageButton( basePanel);
...
//			_conditionTable1.getColumnCount() > RuleManager.get_column_minimum_size());
	...
	public void on_update_stage() {
		...
		RulePropertyPageBase rulePropertyPageBase = _commandTabbedPane.get( ResourceManager.get_instance().get( "rule.type.command.others"));
		if ( null != rulePropertyPageBase) {
			...
			othersCommandPropertyPage.on_update_stage();
		}
	}
	...
	public boolean update_stage_name(String newName, String originalName) {
		...
		if ( null != rulePropertyPageBase) {
			OthersCommandPropertyPage othersCommandPropertyPage = ( OthersCommandPropertyPage)rulePropertyPageBase;
			othersCommandPropertyPage.update_stage_name( newName, originalName);
		}
	 ...
	 * @param actionEvent
	 */
	public void on_apply(RulePropertyPageBase rulePropertyPageBase, ActionEvent actionEvent) {
		boolean updated = false;
		int index = _conditionTabbedPane.get_index( rulePropertyPageBase);
		if ( 0 <= index)
			updated = _ruleTable.update( rulePropertyPageBase);
		else {
			index = _commandTabbedPane.get_index( rulePropertyPageBase);
			if ( 0 <= index)
				updated = _ruleTable.update( rulePropertyPageBase);
		}
		...
		_conditionTabbedPane.update( rulePropertyPageBase);
		_commandTabbedPane.update( rulePropertyPageBase);
