 ...
 */
package soars.application.visualshell.object.role.base.edit.tab.command.exchange_algebra;
...
import soars.application.visualshell.object.role.base.edit.tab.command.exchange_algebra.panel.BasePanel;
 ...
 * @author kurata
 */
public class ExchangeAlgebraCommandPropertyPage1 extends RulePropertyPageBase {
	 ...
	 * 
	 */
	private ComboBox _projectionExchangeAlgebraComboBox = null;
	 ...
	 * @param parent
	 */
	public ExchangeAlgebraCommandPropertyPage1(String title, String kind,
	...
	protected boolean on_create() {
		...
		setup_projectionExchangeAlgebraComboBox( northPanel);

		insert_vertical_strut( northPanel);

		if ( !setup_basePanel( northPanel))
			return false;
	 ...
	 * @param parent
	 */
	private void setup_projectionExchangeAlgebraComboBox(JPanel parent) {
		int pad = 5;
		...
		JLabel label = create_label(
			ResourceManager.get_instance().get( "edit.rule.dialog.command.exchange.algebra.label.projection.from"),
			true);
		panel.add( label);
		_labels.add( label);

		_projectionExchangeAlgebraComboBox = create_comboBox( null, _standardControlWidth, false);
		panel.add( _projectionExchangeAlgebraComboBox);
	...
	private boolean setup_basePanel(JPanel parent) {
		_basePanel = new BasePanel( false, this);
	...
	private void adjust() {
		...
		width = _basePanel.get_width( width);
	...
	protected void reset(ObjectSelector objectSelector, CheckBox spotVariableCheckBox, ComboBox spotVariableComboBox) {
		...
		CommonTool.update( _projectionExchangeAlgebraComboBox, ( !_spotCheckBox.isSelected() && !spotVariableCheckBox.isSelected()) ? get_agent_exchange_algebra_names2( false) : get_spot_exchange_algebra_names2( false));
	...
	protected void update(ObjectSelector objectSelector, CheckBox spotVariableCheckBox, ComboBox spotVariableComboBox) {
		...
		CommonTool.update( _projectionExchangeAlgebraComboBox, get_spot_exchange_algebra_names2( false));
		_basePanel.update();
	}
	...
	protected void update(SpotObject spotObject, String number, ObjectSelector objectSelector, CheckBox spotVariableCheckBox, ComboBox spotVariableComboBox) {
		...
		CommonTool.update( _projectionExchangeAlgebraComboBox, !spotVariableCheckBox.isSelected() ? spotObject.get_object_names( "exchange algebra", number, false) : get_spot_exchange_algebra_names2( false));
	...
	/* (Non Javadoc)
	 * @see soars.application.visualshell.object.role.base.rule.edit.tab.command.exchange_algebra.ExchangeAlgebraCommandPropertyPageBase#set_handler()
	 */
	...
	public boolean set(Rule rule) {
		...
		_projectionExchangeAlgebraComboBox.setSelectedItem( elements[ 1]);

		if ( !_basePanel.set( kind, CommonRuleManipulator.get_full_prefix( rule._value), elements)) {
			set_handler();
			return false;
		}
	...
	public Rule get() {
		...
		String projectionExchangeAlgebra = ( String)_projectionExchangeAlgebraComboBox.getSelectedItem();
		if ( null == projectionExchangeAlgebra || projectionExchangeAlgebra.equals( ""))
			return null;
		...
		String value = ( ExchangeAlgebraCommand._delimiter + projectionExchangeAlgebra + ExchangeAlgebraCommand._delimiter + base);

		String spot = get( _spotCheckBox, _spotSelector, _spotVariableCheckBox, _spotVariableComboBox);

		return Rule.create( _kind, _type, spot + ExchangeAlgebraCommand._reservedWords[ 0] + exchangeAlgebra + value);
	}
}
