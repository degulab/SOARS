...
public class CreateObjectCommand extends Rule {
	 ...
	 * 
	 */
	public static final String[] _reserved_word = {
		"Probability ",
		"Collection ",
		"List ",
		"Map ",
	...
	public static int get_kind(String value) {
		for ( int i = 0; i < _reserved_word.length; ++i) {
			if ( value.startsWith( _reserved_word[ i]))
				return i;
		}
		return -1;
	}
	...
	/**
	 * @param spot_name
	 * @param number_object_name
	 * @return
	 */
	public static String get_number_object_type(String spot_name, String number_object_name) {
		if ( spot_name.equals( ""))
			return LayerManager.get_instance().get_agent_number_object_type( number_object_name);
		else if ( spot_name.equals( "<>"))
			return LayerManager.get_instance().get_spot_number_object_type( number_object_name);
		else {
			SpotObject spotObject = LayerManager.get_instance().get_spot_has_this_name( spot_name);
			if ( null == spotObject)
				return null;

			return spotObject.get_number_object_type( number_object_name);
		}
	}
	...
	public boolean update_spot_name_and_number(String new_name, String original_name, String head_name, Vector ranges, String new_head_name, Vector new_ranges) {
		...
		String element = CommonRuleManipulator.update_spot_name2( elements[ 0], new_name, original_name, head_name, ranges, new_head_name, new_ranges);
		if ( null != element) {
			elements[ 0] = element;
			result1 = true;
		}
		...
		if ( 8 == get_kind( _value) && 1 < elements.length) {
			element = CommonRuleManipulator.get_new_agent_or_spot_name( elements[ 1], new_name, original_name, head_name, ranges, new_head_name, new_ranges);
			if ( null != element) {
				elements[ 1] = element;
				result2 = true;
			}
		...
		_value = ( _reserved_word[ kind] + elements[ 0]);
	...
	public boolean update_role_name(String original_name, String name) {
		...
		_value = ( _reserved_word[ 6] + elements[ 0] + "=" + name);
	...
	public static String update_role_name(String value) {
		...
		return ( _reserved_word[ 6] + elements[ 0] + "=" + roles[ 0]);
	}
}
