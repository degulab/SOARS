 ...
 */
package soars.application.visualshell.object.role.base.edit.tab.command.base;
 ...
 * @author kurata
 */
public class CollectionAndListCommandPropertyPageBase extends RulePropertyPageBase {
	 ...
	 * @param parent
	 */
	public CollectionAndListCommandPropertyPageBase(String title, String kind,
	...
	protected void setup_spot_checkBox_and_spot_selector(JPanel parent) {
		...
		_spot_checkBox = create_checkBox(
			( _type.equals( ResourceManager.get_instance().get( "rule.type.condition.collection")))
				? ResourceManager.get_instance().get( "edit.rule.dialog.command.collection.spot.check.box.name")
				: ResourceManager.get_instance().get( "edit.rule.dialog.command.list.spot.check.box.name"),
			false, true);
	...
	protected void setup_header(JPanel parent) {
		...
		_label[ 0] = create_label(
			( _type.equals( ResourceManager.get_instance().get( "rule.type.condition.collection")))
				? ResourceManager.get_instance().get( "edit.rule.dialog.command.collection.header")
				: ResourceManager.get_instance().get( "edit.rule.dialog.command.list.header"),
			true);
		panel.add( _label[ 0]);
	...
	protected void adjust() {
		...
		_dummy2.setPreferredSize( new Dimension( width, _dummy2.getPreferredSize().height));
