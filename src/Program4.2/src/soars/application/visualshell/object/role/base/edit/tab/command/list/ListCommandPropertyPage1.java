 ...
 * @author kurata
 */
public class ListCommandPropertyPage1 extends CollectionAndListCommandPropertyPageBase {
	 ...
	 * 
	 */
	private ObjectSelector _add_first_agent_agent_selector = null;

	/**
	 * 
	 */
	private ObjectSelector _add_first_spot_spot_selector = null;

	/**
	 * 
	 */
	private ComboBox _add_first_equip_probability_comboBox = null;

	/**
	 * 
	 */
	private ComboBox _add_first_equip_keyword_comboBox = null;

	/**
	 * 
	 */
	private ComboBox _add_first_equip_number_object_comboBox = null;

	/**
	 * 
	 */
	private TextField _add_first_equip_object_textField = null;

	/**
	 * 
	 */
	private TextField _add_first_string_textField = null;
	 ...
	 * @param parent
	 */
	public ListCommandPropertyPage1(String title, String kind,
	...
	protected boolean on_create() {
		...
		setup_add_first_agent( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_add_first_spot( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_add_first_equip( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_add_first_string( buttonGroup1, north_panel);
	...
	private void setup_add_first_agent(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 0].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				...
				_add_first_agent_agent_selector.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_add_first_agent_agent_selector = create_agent_selector( true, true, panel);
	...
	private void setup_add_first_spot(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 1].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 2].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_add_first_spot_spot_selector.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_add_first_spot_spot_selector = create_spot_selector( true, true, panel);
	...
	private void setup_add_first_equip(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_add_first_equip_probability_comboBox = create_comboBox( null, _standardControlWidth, false);
		panel.add( _add_first_equip_probability_comboBox);
		...
		_add_first_equip_keyword_comboBox = create_comboBox( null, _standardControlWidth, false);
		panel.add( _add_first_equip_keyword_comboBox);
		...
		_add_first_equip_number_object_comboBox = create_comboBox( null, _standardControlWidth, false);
		panel.add( _add_first_equip_number_object_comboBox);
		...
		_add_first_equip_object_textField = create_textField( new TextExcluder( Constant._prohibitedCharacters1), _standardControlWidth, false);
		panel.add( _add_first_equip_object_textField);
	...
	private void setup_add_first_string(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 3].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 3].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_add_first_string_textField.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_add_first_string_textField = create_textField( new TextExcluder( Constant._prohibitedCharacters3), _standardControlWidth, false);
		panel.add( _add_first_string_textField);
	...
	protected void reset(ObjectSelector objectSelector, CheckBox spot_variable_checkBox, ComboBox spot_variable_comboBox) {
		...
		CommonTool.update( _add_first_equip_probability_comboBox, ( !_spot_checkBox.isSelected() && !spot_variable_checkBox.isSelected()) ? get_agent_probability_names( false) : get_spot_probability_names( false));
		CommonTool.update( _add_first_equip_keyword_comboBox, ( !_spot_checkBox.isSelected() && !spot_variable_checkBox.isSelected()) ? get_agent_keyword_names( false) : get_spot_keyword_names( false));
		CommonTool.update( _add_first_equip_number_object_comboBox, ( !_spot_checkBox.isSelected() && !spot_variable_checkBox.isSelected()) ? get_agent_number_object_names( false) : get_spot_number_object_names( false));
	}
	...
	protected void update(ObjectSelector objectSelector, CheckBox spot_variable_checkBox, ComboBox spot_variable_comboBox) {
		...
		CommonTool.update( _add_first_equip_probability_comboBox, get_spot_probability_names( false));
		CommonTool.update( _add_first_equip_keyword_comboBox, get_spot_keyword_names( false));
		CommonTool.update( _add_first_equip_number_object_comboBox, get_spot_number_object_names( false));
	}
	...
	protected void update(SpotObject spotObject, String number, ObjectSelector objectSelector, CheckBox spot_variable_checkBox, ComboBox spot_variable_comboBox) {
		...
		CommonTool.update( _add_first_equip_probability_comboBox, !spot_variable_checkBox.isSelected() ? spotObject.get_object_names( "probability", number, false) : get_spot_probability_names( false));
		CommonTool.update( _add_first_equip_keyword_comboBox, !spot_variable_checkBox.isSelected() ? spotObject.get_object_names( "keyword", number, false) : get_spot_keyword_names( false));
		CommonTool.update( _add_first_equip_number_object_comboBox, !spot_variable_checkBox.isSelected() ? spotObject.get_object_names( "number object", number, false) : get_spot_number_object_names( false));
	}
	...
	private void update_components(boolean[] enables) {
		...
		_add_first_agent_agent_selector.setEnabled( enables[ 0]);

		_label[ 2].setEnabled( enables[ 1]);
		_add_first_spot_spot_selector.setEnabled( enables[ 1]);

		for ( int i = 0; i < _radioButtons2[ 0].length; ++i)
			_radioButtons2[ 0][ i].setEnabled( enables[ 2]);

		_add_first_equip_probability_comboBox.setEnabled( enables[ 2]);
		_add_first_equip_keyword_comboBox.setEnabled( enables[ 2]);
		_add_first_equip_number_object_comboBox.setEnabled( enables[ 2]);
		_add_first_equip_object_textField.setEnabled( enables[ 2]);

		_label[ 3].setEnabled( enables[ 3]);
		_add_first_string_textField.setEnabled( enables[ 3]);
	}
	...
	public boolean set(Rule rule) {
		...
		switch ( kind) {
			case 0:
				set2( _comboBox, _add_first_agent_agent_selector, values[ 2], ( 2 > elements.length) ? "" : elements[ 1]);
				break;
			case 1:
				set2( _comboBox, _add_first_spot_spot_selector, values[ 2], ( 2 > elements.length) ? "" : elements[ 1]);
				break;
			case 2:
				set1( _comboBox, _add_first_equip_probability_comboBox, values[ 2], elements[ 1]);
				break;
			case 3:
				set1( _comboBox, _add_first_equip_keyword_comboBox, values[ 2], elements[ 1]);
				break;
			case 4:
				set1( _comboBox, _add_first_equip_number_object_comboBox, values[ 2], elements[ 1]);
				break;
			case 5:
				set3( _comboBox, _add_first_equip_object_textField, values[ 2], elements[ 1]);
				break;
			case 6:
				set3( _comboBox, _add_first_string_textField, values[ 2], elements[ 1]);
				break;
		}
	...
	public Rule get() {
		...
		switch ( kind) {
			case 0:
				value = get3( _comboBox, _add_first_agent_agent_selector);
				break;
			case 1:
				value = get3( _comboBox, _add_first_spot_spot_selector);
				break;
			case 2:
				value = get2( _comboBox, _add_first_equip_probability_comboBox, false);
				break;
			case 3:
				value = get2( _comboBox, _add_first_equip_keyword_comboBox, false);
				break;
			case 4:
				value = get2( _comboBox, _add_first_equip_number_object_comboBox, false);
				break;
			case 5:
				if ( null == _add_first_equip_object_textField.getText()
					|| _add_first_equip_object_textField.getText().equals( "")
					|| _add_first_equip_object_textField.getText().equals( "$")
					|| 0 < _add_first_equip_object_textField.getText().indexOf( '$')
					|| _add_first_equip_object_textField.getText().equals( "$Name")
					|| _add_first_equip_object_textField.getText().equals( "$Role")
					|| _add_first_equip_object_textField.getText().equals( "$Spot")
					|| 0 <= _add_first_equip_object_textField.getText().indexOf( Constant._experimentName))
					return null;

				if ( _add_first_equip_object_textField.getText().startsWith( "$")
					&& ( 0 < _add_first_equip_object_textField.getText().indexOf( "$", 1)
					|| 0 < _add_first_equip_object_textField.getText().indexOf( ")", 1)))
					return null;

				// TODO 要動作確認！
				if ( !is_object( spot, _add_first_equip_object_textField.getText(),
					new String[] {
						"collection",
						"list",
						"spot variable",
				...
				value = get4( _comboBox, _add_first_equip_object_textField, false);
				break;
			case 6:
				if ( null == _add_first_string_textField.getText()
					|| _add_first_string_textField.getText().equals( "")
					|| _add_first_string_textField.getText().equals( "$")
					|| 0 < _add_first_string_textField.getText().indexOf( '$')
					|| _add_first_string_textField.getText().startsWith( " ")
					|| _add_first_string_textField.getText().endsWith( " ")
					|| _add_first_string_textField.getText().equals( "$Name")
					|| _add_first_string_textField.getText().equals( "$Role")
					|| _add_first_string_textField.getText().equals( "$Spot")
					|| 0 <= _add_first_string_textField.getText().indexOf( Constant._experimentName))
					return null;

				if ( _add_first_string_textField.getText().startsWith( "$")
					&& ( 0 <= _add_first_string_textField.getText().indexOf( " ")
					|| 0 < _add_first_string_textField.getText().indexOf( "$", 1)
					|| 0 < _add_first_string_textField.getText().indexOf( ")", 1)))
					return null;

				value = get4( _comboBox, _add_first_string_textField, false);
				break;
			default:
				return null;
		}
