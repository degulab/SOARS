 ...
 *
 */
public class SubstitutionCommand extends NumberRule {

	/**
	 * 
	 */
	public static String[] _sub_types = {
		ResourceManager.get_instance().get( "rule.sub.type.command.substitution.value.name"),
		ResourceManager.get_instance().get( "rule.sub.type.command.substitution.number.object.name"),
		ResourceManager.get_instance().get( "rule.sub.type.command.substitution.expression.name"),
		ResourceManager.get_instance().get( "rule.sub.type.command.substitution.others.name")
	};
	...
	public static ExpressionElements get1(String[] elements) {
		...
		expressionElements._function = elements[ 1];
	 ...
	 * @param value
	 */
	public SubstitutionCommand(String kind, String type, String value) {
	...
	private void get_number_object_names1(String value, int kind, String prefix, Vector number_object_names) {
		switch ( kind) {
			case 0:
				get_number_object_names( value, 2, number_object_names);
				break;
			case 1:
				get_number_object_names( value, 2, number_object_names);
				get_number_object_names( value, 2, new int[] { 1}, prefix, number_object_names);
				break;
			case 2:
				String[] elements = value.split( " ");
				if ( null == elements || 2 > elements.length)
	...
	protected boolean update_number_object_name(String name, String new_name, String type, Role role) {
		...
		if ( subType._operator.equals( "")) {
			...
			String value0 = update_number_object_name1(
				values[ 0], subType._kind[ 0], name, new_name, type);
			if ( null != value0)
				values[ 0] = value0;
	...
	private String update_number_object_name11(String value, String name, String new_name, String type) {
		...
		if ( elements[ 1].startsWith( "<")) {
			...
			boolean result1 = CommonRuleManipulator.correspond( prefix, elements[ 1], name, type);
			if ( result1)
				elements[ 1] = new_name;

			if ( null == element0 && !result1)
				return null;
	...
	private boolean can_paste_number_object_names1(String value, int kind, String prefix, Layer drawObjects) {
		switch ( kind) {
			case 0:
				return can_paste_number_object_names( value, 2, drawObjects);
			case 1:
				return ( can_paste_number_object_names( value, 2, drawObjects)
					&& can_paste_number_object_names( value, 2, new int[] { 1}, prefix, drawObjects));
			case 2:
				String[] elements = value.split( " ");
				if ( null == elements || 2 > elements.length)
					return false;
	...
	private String get_script1(String value, int kind, boolean bracket1, boolean bracket2) {
		switch ( kind) {
			...
			case 2:
				...
				return get_expression_script1( expressionElements, elements[ 0], "", bracket1, bracket2);
			case 3:
	...
	public List<String> get_java_program(String value, RuleData ruleData, RoleDataSet roleDataSet, Map<String, String> agentNameMap, Map<String, String> spotNameMap, List<String> imports, List<String> commands, String packagePrefix, Role role, boolean initialStage) {
		...
		//System.out.println( "command : " + mainValue);
		...
		if ( subType._operator.equals( "")) {
			...
			scripts[ 1] = get_script2_for_java_program( values[ 1], subType._kind[ 1], false, entityPrefix, expressionData, ruleData, roleDataSet, agentNameMap, spotNameMap, imports, commands, packagePrefix, role, initialStage);
			if ( null == scripts[ 0] || null == scripts[ 1])
				return get_unknown_codes();
	 ...
	 * @return
	 */
	private String format(String script, String operator, ExpressionData expressionData, boolean initialStage) {
		// TODO Auto-generated method stub
		...
		String[] scripts = script.split( operator);
		if ( 2 != scripts.length)
			return "Unknown";

		String variableName = get_variableName( scripts[ 0]);
		...
		String right = "";
		while ( true) {
			int start = scripts[ 1].indexOf( "__|");
			if ( 0 > start)
				break;

			right += scripts[ 1].substring( 0, start);
			scripts[ 1] = scripts[ 1].substring( start + 3);

			int end = scripts[ 1].indexOf( "|__");
			if ( 0 > end)
				return "Unknown";

			//right += expressionData.get( 0) + ".get" + scripts[ 1].substring( 0, end) + "()";
			for ( int i = 0; i < expressionData._variables.size(); ++i) {
				if ( expressionData._variables.get( i)._name.equals( scripts[ 1].substring( 0, end))) {
					right += expressionData._prefixes.get( i) + ( expressionData._prefixes.get( i).equals( "") ? "" : ".") +  "get" + scripts[ 1].substring( 0, end)
						+ "(" + ( expressionData._variables.get( i)._spotSet ? spotDBname : "") + ")";
					//script = script.substring( end + 3);
					break;
				}
			}
			scripts[ 1] = scripts[ 1].substring( end + 3);
		}

		right += scripts[ 1];

		for ( int i = 0; i < expressionData._variables.size(); ++i) {
			if ( expressionData._variables.get( i)._name.equals( variableName))
				return expressionData._prefixes.get( i) + ( expressionData._prefixes.get( i).equals( "") ? "" : ".") + "set" + variableName
					+ "(" + ( expressionData._variables.get( i)._spotSet ? spotDBname + "," : "") + right + ")";
		}

		return "Unknown";
//		return expressionData.get( 0) + ".set" + variableName + "(" + right + ")";
	}
...
//				}
				...
				String expression = get_expression_script1_java_program( expressionElements, elements[ 0], "=", bracket1, bracket2).split( " ")[ 1];
	 ...
	 * @return
	 */
	private String get_script2_for_java_program(String value, int kind, boolean bracket, String entityPrefix, ExpressionData expressionData, RuleData ruleData, RoleDataSet roleDataSet, Map<String, String> agentNameMap, Map<String, String> spotNameMap, List<String> imports, List<String> commands, String packagePrefix, Role role, boolean initialStage) {
		// TODO Auto-generated method stub
		switch ( kind) {
			case 0:
				return ( value.startsWith( "-") ? "(" + value + ")" : value);
			case 1:
				return get_number_object_script2_for_java_program( value, entityPrefix, expressionData, ruleData, roleDataSet, agentNameMap, spotNameMap, imports, commands, packagePrefix, role, initialStage);
			case 2:
				ExpressionElements expressionElements = get2( value);
				if ( null == expressionElements)
					return "";
...
//			scripts[ 1] = get_script2_for_java_program( values[ 1], subType._kind[ 1], false);
//			if ( null == scripts[ 0] || null == scripts[ 1])
//				return "";
//
//			return ( scripts[ 0] + subType._operator + scripts[ 1]);
...
//	 * @return
//	 */
//	private String get_script1_for_java_program(String value, int kind, boolean bracket1, boolean bracket2) {
//		// TODO Auto-generated method stub
//		switch ( kind) {
//			case 0:
...
//				}
//
//				String expression = get_expression_script1( expressionElements, elements[ 0], "", bracket1, bracket2).split( " ")[ 1];
//				return replace( expression);
//			case 3:
//				return get_others_script1( value);
//		}
...
//	 * @return
//	 */
//	private String get_script2_for_java_program(String value, int kind, boolean bracket) {
//		// TODO Auto-generated method stub
//		switch ( kind) {
//			case 0:
//				return ( value.startsWith( "-") ? "(" + value + ")" : value);
