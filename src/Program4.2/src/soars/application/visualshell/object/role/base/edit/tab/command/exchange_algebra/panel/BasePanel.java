...
import soars.application.visualshell.object.role.base.edit.tab.command.exchange_algebra.ExchangeAlgebraCommandPropertyPageBase;
...
public class BasePanel extends JPanel {
	 ...
	 */
	public BasePanel(boolean margin, RulePropertyPageBase rulePropertyPageBase) {
		super();
		_margin = margin;
	...
	private boolean setup_baseTypeComboBox(RadioButton radioButton, JPanel parent) {
		...
		JLabel label = _rulePropertyPageBase.create_label(
			ResourceManager.get_instance().get( "edit.rule.dialog.command.exchange.algebra.base.type"),
			true);
		panel.add( label);
		_labels.add( label);

		_baseTypeComboBox = _rulePropertyPageBase.create_comboBox(
			new String[] {
	...
	private boolean setup1(JPanel parent) {
		...
		_baseTextField = _rulePropertyPageBase.create_textField( new TextExcluder( Constant._prohibitedCharacters12), false);
	...
	private boolean setup3(JPanel parent) {
		...
		for ( int i = 0; i < labels.length; ++i) {
			if ( 1 == i) {
				_hatPanel = new HatPanel( _margin, labels[ i], _rulePropertyPageBase);
				if ( !_hatPanel.create())
					return false;
		...
		switch ( kind) {
			case 0:
				if ( elements[ 2].startsWith( "$"))
					...
					if ( !ExchangeAlgebraCommandPropertyPageBase.set_base( _baseTextField, elements[ 2]))
						return false;
				}
				break;
			case 1:
		...
		switch ( _baseTypeComboBox.getSelectedIndex()) {
			case 0:
				...
				if ( _baseTextField.getText().startsWith( "$"))
					...
					String base = ExchangeAlgebraCommandPropertyPageBase.get_base( _baseTextField);
					if ( null == base)
						return null;

					ExchangeAlgebraCommandPropertyPageBase.set_base( _baseTextField, base);
					return base;
				}
			case 1:
