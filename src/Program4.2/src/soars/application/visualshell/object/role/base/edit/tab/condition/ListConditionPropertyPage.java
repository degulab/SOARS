 ...
 * @author kurata
 */
public class ListConditionPropertyPage extends CollectionAndListConditionPropertyPageBase {

	/**
	 * 
	 */
	private ObjectSelector _is_first_agent_agent_selector = null;

	/**
	 * 
	 */
	private ObjectSelector _is_first_spot_spot_selector = null;

	/**
	 * 
	 */
	private TextField _is_first_equip_textField = null;

	/**
	 * 
	 */
	private TextField _is_first_string_textField = null;

	/**
	 * 
	 */
	private ObjectSelector _is_last_agent_agent_selector = null;

	/**
	 * 
	 */
	private ObjectSelector _is_last_spot_spot_selector = null;

	/**
	 * 
	 */
	private TextField _is_last_equip_textField = null;

	/**
	 * 
	 */
	private TextField _is_last_string_textField = null;
	 ...
	 * @param parent
	 */
	public ListConditionPropertyPage(String title, String kind,
	...
	protected boolean on_create() {
		...
		setup_is_first_agent( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_first_spot( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_first_equip( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_first_string( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_last_agent( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_last_spot( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_last_equip( buttonGroup1, north_panel);

		insert_vertical_strut( north_panel);

		setup_is_last_string( buttonGroup1, north_panel);
	...
	private void setup_is_first_agent(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 0].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				...
				_is_first_agent_agent_selector.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_first_agent_agent_selector = create_agent_selector( true, true, panel);
	...
	private void setup_is_first_spot(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 1].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 2].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_first_spot_spot_selector.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_first_spot_spot_selector = create_spot_selector( true, true, panel);
	...
	private void setup_is_first_equip(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 2].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 3].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_first_equip_textField.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_first_equip_textField = create_textField( new TextExcluder( Constant._prohibitedCharacters1), _standardControlWidth, false);
		panel.add( _is_first_equip_textField);
	...
	private void setup_is_first_string(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 3].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 4].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_first_string_textField.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_first_string_textField = create_textField( new TextExcluder( Constant._prohibitedCharacters3), _standardControlWidth, false);
		panel.add( _is_first_string_textField);
	...
	private void setup_is_last_agent(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 4].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 5].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_last_agent_agent_selector.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_last_agent_agent_selector = create_agent_selector( true, true, panel);
	...
	private void setup_is_last_spot(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 5].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 6].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_last_spot_spot_selector.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_last_spot_spot_selector = create_spot_selector( true, true, panel);
	...
	private void setup_is_last_equip(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 6].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 7].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_last_equip_textField.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_last_equip_textField = create_textField( new TextExcluder( Constant._prohibitedCharacters1), _standardControlWidth, false);
		panel.add( _is_last_equip_textField);
	...
	private void setup_is_last_string(ButtonGroup buttonGroup1, JPanel parent) {
		...
		_radioButtons1[ 7].addItemListener( new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				_label[ 8].setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
				_is_last_string_textField.setEnabled(
					ItemEvent.SELECTED == arg0.getStateChange());
			}
		});
		...
		_is_last_string_textField = create_textField( new TextExcluder( Constant._prohibitedCharacters3), _standardControlWidth, false);
		panel.add( _is_last_string_textField);
	...
	private void update_components(boolean[] enables) {
		...
		_is_first_agent_agent_selector.setEnabled( enables[ 0]);

		_label[ 2].setEnabled( enables[ 1]);
		_is_first_spot_spot_selector.setEnabled( enables[ 1]);

		_label[ 3].setEnabled( enables[ 2]);
		_is_first_equip_textField.setEnabled( enables[ 2]);

		_label[ 4].setEnabled( enables[ 3]);
		_is_first_string_textField.setEnabled( enables[ 3]);

		_label[ 5].setEnabled( enables[ 4]);
		_is_last_agent_agent_selector.setEnabled( enables[ 4]);

		_label[ 6].setEnabled( enables[ 5]);
		_is_last_spot_spot_selector.setEnabled( enables[ 5]);

		_label[ 7].setEnabled( enables[ 6]);
		_is_last_equip_textField.setEnabled( enables[ 6]);

		_label[ 8].setEnabled( enables[ 7]);
		_is_last_string_textField.setEnabled( enables[ 7]);
	}
	...
	public boolean set(Rule rule) {
		...
		switch ( kind) {
			case 0:
				set2( _comboBox, _is_first_agent_agent_selector, values[ 2], ( 2 > elements.length) ? "" : elements[ 1]);
				break;
			case 1:
				set2( _comboBox, _is_first_spot_spot_selector, values[ 2], ( 2 > elements.length) ? "" : elements[ 1]);
				break;
			case 2:
				set3( _comboBox, _is_first_equip_textField, values[ 2], elements[ 1]);
				break;
			case 3:
				set3( _comboBox, _is_first_string_textField, values[ 2], elements[ 1]);
				break;
			case 4:
				set2( _comboBox, _is_last_agent_agent_selector, values[ 2], ( 2 > elements.length) ? "" : elements[ 1]);
				break;
			case 5:
				set2( _comboBox, _is_last_spot_spot_selector, values[ 2], ( 2 > elements.length) ? "" : elements[ 1]);
				break;
			case 6:
				set3( _comboBox, _is_last_equip_textField, values[ 2], elements[ 1]);
				break;
			case 7:
				set3( _comboBox, _is_last_string_textField, values[ 2], elements[ 1]);
				break;
			default:
				set_handler();
				return false;
	...
	public Rule get() {
		...
		switch ( kind) {
			case 0:
				value = get3( _comboBox, _is_first_agent_agent_selector);
				break;
			case 1:
				value = get3( _comboBox, _is_first_spot_spot_selector);
				break;
			case 2:
				if ( null == _is_first_equip_textField.getText()
					|| _is_first_equip_textField.getText().equals( "")
					|| _is_first_equip_textField.getText().equals( "$")
					|| 0 < _is_first_equip_textField.getText().indexOf( '$')
					|| _is_first_equip_textField.getText().equals( "$Name")
					|| _is_first_equip_textField.getText().equals( "$Role")
					|| _is_first_equip_textField.getText().equals( "$Spot")
					|| 0 <= _is_first_equip_textField.getText().indexOf( Constant._experimentName))
					return null;

				if ( _is_first_equip_textField.getText().startsWith( "$")
					&& ( 0 < _is_first_equip_textField.getText().indexOf( "$", 1)
					|| 0 < _is_first_equip_textField.getText().indexOf( ")", 1)))
					return null;

				// TODO 要動作確認！
				if ( !is_object( spot, _is_first_equip_textField.getText()))
					return null;

				value = get4( _comboBox, _is_first_equip_textField, false);
				break;
			case 3:
				if ( null == _is_first_string_textField.getText()
					|| _is_first_string_textField.getText().equals( "")
					|| _is_first_string_textField.getText().equals( "$")
					|| 0 < _is_first_string_textField.getText().indexOf( '$')
					|| _is_first_string_textField.getText().startsWith( " ")
					|| _is_first_string_textField.getText().endsWith( " ")
					|| _is_first_string_textField.getText().equals( "$Name")
					|| _is_first_string_textField.getText().equals( "$Role")
					|| _is_first_string_textField.getText().equals( "$Spot")
					|| 0 <= _is_first_string_textField.getText().indexOf( Constant._experimentName))
					return null;

				if ( _is_first_string_textField.getText().startsWith( "$")
					&& ( 0 <= _is_first_string_textField.getText().indexOf( " ")
					|| 0 < _is_first_string_textField.getText().indexOf( "$", 1)
					|| 0 < _is_first_string_textField.getText().indexOf( ")", 1)))
					return null;

				value = get4( _comboBox, _is_first_string_textField, false);
				break;
			case 4:
				value = get3( _comboBox, _is_last_agent_agent_selector);
				break;
			case 5:
				value = get3( _comboBox, _is_last_spot_spot_selector);
				break;
			case 6:
				if ( null == _is_last_equip_textField.getText()
					|| _is_last_equip_textField.getText().equals( "")
					|| _is_last_equip_textField.getText().equals( "$")
					|| 0 < _is_last_equip_textField.getText().indexOf( '$')
					|| _is_last_equip_textField.getText().equals( "$Name")
					|| _is_last_equip_textField.getText().equals( "$Role")
					|| _is_last_equip_textField.getText().equals( "$Spot")
					|| 0 <= _is_last_equip_textField.getText().indexOf( Constant._experimentName))
					return null;

				if ( _is_last_equip_textField.getText().startsWith( "$")
					&& ( 0 < _is_last_equip_textField.getText().indexOf( "$", 1)
					|| 0 < _is_last_equip_textField.getText().indexOf( ")", 1)))
					return null;

				// TODO 要動作確認！
				if ( !is_object( spot, _is_last_equip_textField.getText()))
					return null;

				value = get4( _comboBox, _is_last_equip_textField, false);
				break;
			case 7:
				if ( null == _is_last_string_textField.getText()
					|| _is_last_string_textField.getText().equals( "")
					|| _is_last_string_textField.getText().equals( "$")
					|| 0 < _is_last_string_textField.getText().indexOf( '$')
					|| _is_last_string_textField.getText().startsWith( " ")
					|| _is_last_string_textField.getText().endsWith( " ")
					|| _is_last_string_textField.getText().equals( "$Name")
					|| _is_last_string_textField.getText().equals( "$Role")
					|| _is_last_string_textField.getText().equals( "$Spot")
					|| 0 <= _is_last_string_textField.getText().indexOf( Constant._experimentName))
					return null;

				if ( _is_last_string_textField.getText().startsWith( "$")
					&& ( 0 <= _is_last_string_textField.getText().indexOf( " ")
					|| 0 < _is_last_string_textField.getText().indexOf( "$", 1)
					|| 0 < _is_last_string_textField.getText().indexOf( ")", 1)))
					return null;

				value = get4( _comboBox, _is_last_string_textField, false);
				break;
			default:
				return null;
		}
		...
		return Rule.create( _kind, _type,
			( _checkBox.isSelected() ? "!" : "") + ( ListCondition._reservedWords[ kind] + spot + value));
//		String value = null;
//		int kind = SwingTool.get_enabled_radioButton( _radioButtons1);
//		switch ( kind) {
//			case 0:
