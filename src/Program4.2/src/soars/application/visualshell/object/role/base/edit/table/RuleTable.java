...
public class RuleTable extends SpreadSheetTable implements IBasicMenuHandler1, IBasicMenuHandler2, IBasicMenuHandler5, ITextUndoRedoManagerCallBack {
	 ...
	 * 
	 */
	static public List<BlockData<Rule>> _selectedAgentRules = new ArrayList<BlockData<Rule>>();

	/**
	 * 
	 */
	static public List<BlockData<Rule>> _selectedSpotRules = new ArrayList<BlockData<Rule>>();

	/**
	 * 
	 */
	static public List<RowData<Rule>> _selectedAgentRowRules = new ArrayList<RowData<Rule>>();

	/**
	 * 
	 */
	static public List<RowData<Rule>> _selectedSpotRowRules = new ArrayList<RowData<Rule>>();

	/**
	 * 
	 */
	static public List<ColumnData<Rule>> _selectedAgentColumnRules = new ArrayList<ColumnData<Rule>>();

	/**
	 * 
	 */
	static public List<ColumnData<Rule>> _selectedSpotColumnRules = new ArrayList<ColumnData<Rule>>();
	...
	public boolean setup_column(Vector<Integer> columnWidths) {
		...
		for ( int i = 0; i < defaultTableColumnModel.getColumnCount(); ++i) {
			...
			tableColumn.setCellRenderer( new RuleTableCellRenderer());
	...
	public int on_change_selection(int row, int column, int optionType) {
		...
		RulePropertyPageBase rulePropertyPageBase = ruleTree.get_selected_page();
		if ( null == rulePropertyPageBase)
			return result;

		Rule rule1 = rulePropertyPageBase.get();
		if ( null == rule1)
			return result;
		...
		if ( !type_changed( rule1, rule2) && value_changed( rule1, rule2)) {
			...
			if ( JOptionPane.YES_OPTION == result) {
				...
				if ( rulePropertyPageBase instanceof TimeCommandPropertyPage)
					rulePropertyPageBase.confirm();

				RuleTabbedPane ruleTabbedPane = ( RuleTabbedPane)_ruleTabTabbedPane.getSelectedComponent();
				ruleTabbedPane.update( rulePropertyPageBase);
				refresh();
				repaint();
			} else
				rulePropertyPageBase.set( rule2);
		}
	...
	private void internal_select(Rule rule) {
		...
		boolean exist = false;
		for ( RulePropertyPageBase rulePropertyPageBase:ruleTabbedPane._rulePropertyPageBases) {
			rulePropertyPageBase.on_setup_completed();
			if ( rulePropertyPageBase.set( rule)) {
				ruleTabbedPane.select( rulePropertyPageBase);
				ruleTree.select( rulePropertyPageBase);
				exist = true;
			}
		}
		...
		if ( rule._kind.equals( "condition")) {
			RulePropertyPageBase rulePropertyPageBase = ruleTabbedPane.get( ResourceManager.get_instance().get( "rule.type.condition.others"));
			if ( null != rulePropertyPageBase) {
				ruleTabbedPane.select( rulePropertyPageBase);
				EditRuleValuePropertyPage editRuleValuePropertyPage = ( EditRuleValuePropertyPage)rulePropertyPageBase;
				editRuleValuePropertyPage.set( rule._value);
				ruleTree.select( rulePropertyPageBase);
			}
		} else {
			RulePropertyPageBase rulePropertyPageBase = ruleTabbedPane.get( ResourceManager.get_instance().get( "rule.type.command.others"));
			if ( null != rulePropertyPageBase) {
				ruleTabbedPane.select( rulePropertyPageBase);
				OthersCommandPropertyPage othersCommandPropertyPage = ( OthersCommandPropertyPage)rulePropertyPageBase;
				othersCommandPropertyPage.set( Rule.create( rule));
				ruleTree.select( rulePropertyPageBase);
			}
		}
	}
	...
	private void update_activeCellTextField(Rule rule) {
		...
		_activeCellTextField.setText( ( rule._or ? " || " : "")
			+ CommonRuleData.get_name( rule) + " : " + value);

		Color color = CommonRuleData.get_color( rule);
		if ( null != color)
	 ...
	 * @return
	 */
	public boolean update(RulePropertyPageBase rulePropertyPageBase) {
		int[] rows = getSelectedRows();
		if ( 1 != rows.length)
			return false;
		...
		Rule newRule = rulePropertyPageBase.get();
		if ( null == newRule)
			return false;
