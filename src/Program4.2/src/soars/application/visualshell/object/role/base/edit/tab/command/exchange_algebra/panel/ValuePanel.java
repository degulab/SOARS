...
public class ValuePanel extends JPanel {
	 ...
	 */
	public ValuePanel(boolean margin, RulePropertyPageBase rulePropertyPageBase) {
		super();
		_margin = margin;
	...
	public boolean create(RadioButton radioButton) {
		...
		_label = _rulePropertyPageBase.create_label(
			ResourceManager.get_instance().get( "edit.rule.dialog.command.exchange.algebra.value"),
			true);
		add( _label);

		_valueTypeComboBox = _rulePropertyPageBase.create_comboBox(
			new String[] {
		...
		_textField = _rulePropertyPageBase.create_textField( new TextExcluder( Constant._prohibitedCharacters4), true);
		...
		_numberObjectComboBox = _rulePropertyPageBase.create_comboBox( null, false);
	...
	public void reset(CheckBox spotCheckBox, CheckBox spotVariableCheckBox) {
		CommonTool.update( _numberObjectComboBox, ( !spotCheckBox.isSelected() && !spotVariableCheckBox.isSelected()) ? RulePropertyPageBase.get_agent_number_object_names( false) : RulePropertyPageBase.get_spot_number_object_names( false));
	}
	 ...
	 */
	public void update() {
		CommonTool.update( _numberObjectComboBox, RulePropertyPageBase.get_spot_number_object_names( false));
	}
	...
	public void update(SpotObject spotObject, String number, CheckBox spotVariableCheckBox) {
		CommonTool.update( _numberObjectComboBox, !spotVariableCheckBox.isSelected() ? spotObject.get_object_names( "number object", number, false) : RulePropertyPageBase.get_spot_number_object_names( false));
	}
