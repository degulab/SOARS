...
public class ItemPanel extends JPanel {
	 ...
	 */
	public ItemPanel(boolean margin, String name, String[] items, RulePropertyPageBase rulePropertyPageBase) {
		super();
		_margin = margin;
		_name = name;
		_items = items;
	...
	public boolean create() {
		...
		_label = _rulePropertyPageBase.create_label( _name, true);
		add( _label);

		_typeComboBox = _rulePropertyPageBase.create_comboBox( _items, false);
		...
		for ( String item:_items) {
			if ( item.equals( ResourceManager.get_instance().get( "edit.rule.dialog.command.exchange.algebra.keyword"))) {
				_keywordComboBox = _rulePropertyPageBase.create_comboBox( null, false);
				...
				_timeVariableComboBox = _rulePropertyPageBase.create_comboBox( null, false);
				...
				_timePanel = new TimePanel( _rulePropertyPageBase);
				...
				_textField = _rulePropertyPageBase.create_textField( new TextExcluder( Constant._prohibitedCharacters12), false);
	...
	public void reset(CheckBox spotCheckBox, CheckBox spotVariableCheckBox) {
		if ( null != _keywordComboBox)
			CommonTool.update( _keywordComboBox, ( !spotCheckBox.isSelected() && !spotVariableCheckBox.isSelected()) ? RulePropertyPageBase.get_agent_keyword_names( false) : RulePropertyPageBase.get_spot_keyword_names( false));
		if ( null != _timeVariableComboBox)
			CommonTool.update( _timeVariableComboBox, ( !spotCheckBox.isSelected() && !spotVariableCheckBox.isSelected()) ? RulePropertyPageBase.get_agent_time_variable_names( false) : RulePropertyPageBase.get_spot_time_variable_names( false));
	}
	...
	public void update() {
		if ( null != _keywordComboBox)
			CommonTool.update( _keywordComboBox, RulePropertyPageBase.get_spot_keyword_names( false));
		if ( null != _timeVariableComboBox)
			CommonTool.update( _timeVariableComboBox, RulePropertyPageBase.get_spot_time_variable_names( false));
	}
	...
	public void update(SpotObject spotObject, String number, CheckBox spotVariableCheckBox) {
		if ( null != _keywordComboBox)
			CommonTool.update( _keywordComboBox, !spotVariableCheckBox.isSelected() ? spotObject.get_object_names( "keyword", number, false) : RulePropertyPageBase.get_spot_keyword_names( false));
		if ( null != _timeVariableComboBox)
			CommonTool.update( _timeVariableComboBox, !spotVariableCheckBox.isSelected() ? spotObject.get_object_names( "time variable", number, false) : RulePropertyPageBase.get_spot_time_variable_names( false));
	}
