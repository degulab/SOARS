...
public class ExTransferCommand extends Rule {
	...
	protected boolean update_spot_variable_name(String originalName, String newName, String player, Role role) {
		...
		elements[ 0] = CommonRuleManipulator.update_spot_variable_name2( elements[ 0], originalName, newName, player);
		if ( null == elements[ 0])
			return false;
	...
	protected boolean update_extransfer_name(String originalName, String newName, String player, Role role) {
		...
		String element = CommonRuleManipulator.update_object_name( elements[ 0], originalName, newName, player);
		if ( null == element)
			return false;
	...
	private boolean update_object_name(String originalName, String newName, String player, Role role) {
		...
		if ( CommonRuleManipulator.correspond( prefix, elements[ 1], originalName, player)) {
			elements[ 1] = newName;
			result = true;
		}

		if ( CommonRuleManipulator.correspond( prefix, elements[ 2], originalName, player)) {
			elements[ 2] = newName;
			result = true;
		}
	...
	/* (non-Javadoc)
	 * @see soars.application.visualshell.object.role.base.object.base.Rule#get_cell_text()
	 */
