 ...
 *
 */
public class GetEquipCommand extends EquipCommand {

	/**
	 * 
	 */
	public static String _reserved_word = "getEquip ";
	 ...
	 * @param value
	 */
	public GetEquipCommand(String kind, String type, String value) {
	...
	private String get_used_spot_name() {
		...
		return CommonRuleManipulator.extract_spot_name2( elements[ 1]);
	}
	...
	private String get_used_spot_variable_name() {
		...
		return CommonRuleManipulator.get_spot_variable_name2( elements[ 1]);
	}
	...
	public boolean update_spot_name_and_number(String new_name, String original_name, String head_name, Vector ranges, String new_head_name, Vector new_ranges) {
		...
		elements[ 1] = CommonRuleManipulator.update_spot_name2( elements[ 1], new_name, original_name, head_name, ranges, new_head_name, new_ranges);
		if ( null == elements[ 1])
			return false;
	...
	protected boolean update_spot_variable_name(String name, String new_name, String type, Role role) {
		...
		elements[ 1] = CommonRuleManipulator.update_spot_variable_name2( elements[ 1], name, new_name, type);
		if ( null == elements[ 1])
			return result1;
	...
	private boolean can_paste_spot_name(int kind, Layer drawObjects) {
		...
		return CommonRuleManipulator.can_paste_spot_and_spot_variable_name2( elements[ 1], drawObjects);
	}
	...
	protected String get_script(String value, Role role) {
		...
		String[] prefix_and_object1 = CommonRuleManipulator.get_prefix_and_object( elements2[ 0]);
		if ( !prefix_and_object1[ 0].equals( "") && !prefix_and_object1[ 0].equals( "<>"))
			return "";

		String[] prefix_and_object2 = CommonRuleManipulator.get_prefix_and_object( elements2[ 1]);
		...
		return get_script( prefix_and_object2[ 0], elements1[ 0], prefix_and_object1[ 1],
			prefix_and_object2[ 1], ( prefix_and_object1[ 0].equals( "") ? "" : "<>"), keyword, time_variable, spot_variable);
	}
	...
	public List<String> get_java_program(String value, RuleData ruleData, RoleDataSet roleDataSet, Map<String, String> agentNameMap, Map<String, String> spotNameMap, List<String> imports, List<String> commands, String packagePrefix, Role role, boolean initialStage) {
		...
		String[] prefixAndObject1 = CommonRuleManipulator.get_prefix_and_object( elements2[ 0]);
		if ( !prefixAndObject1[ 0].equals( "") && !prefixAndObject1[ 0].equals( "<>"))
			return get_unknown_codes();

		String[] prefixAndObject2 = CommonRuleManipulator.get_prefix_and_object( elements2[ 1]);
		if ( prefixAndObject2[ 0].equals( ""))
			return get_unknown_codes();

		String kind1 = CommonRuleManipulator.what_kind_object_is( elements2[ 0]);
		if ( null == kind1)
			return get_unknown_codes();

		String kind2 = CommonRuleManipulator.what_kind_object_is( elements2[ 1]);
		if ( null == kind2)
			return get_unknown_codes();
