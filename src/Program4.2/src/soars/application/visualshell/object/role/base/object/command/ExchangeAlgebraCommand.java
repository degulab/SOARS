...
//			return false;
//
//		if ( null == bases[ 0] || bases[ 0].equals( "") || !is_valid( bases[ 0], Constant._prohibited_characters11))
//			return false;
//
//		if ( null == bases[ 1] || ( !bases[ 1].equals( "NO_HAT") && !bases[ 1].equals( "HAT")))
//			return false;
//
//		if ( 3 <= bases.length && ( null == bases[ 2] || !is_valid( bases[ 2], Constant._prohibited_characters11)))
//			return false;
//
//		if ( 4 <= bases.length && ( null == bases[ 3] || !is_valid( bases[ 3], Constant._prohibited_characters11)))
//			return false;
//
//		if ( 5 <= bases.length && ( null == bases[ 4] || !is_valid( bases[ 4], Constant._prohibited_characters11)))
//			return false;
//
//		return true;
//	}
...
//	 * @return
//	 */
//	private static boolean is_valid(String value, String prohibited_characters) {
//		for ( int i = 0; i < prohibited_characters.length(); ++i) {
//			if ( 0 > value.indexOf( prohibited_characters.charAt( i)))
//				continue;
//
//			return false;
//		}
...
//			return -1;
//
//		if ( reserved_word.equals( _reservedWords[ 0]))
//			return 0;
////		if ( reserved_word.equals( _reservedWords[ 1]))
////			return 1;
////		else if ( reserved_word.equals( _reservedWords[ 2]))
////			return 2;
//		else if ( reserved_word.equals( _reservedWords[ 3]))
//			return 3;
////		else if ( reserved_word.equals( _reservedWords[ 4]))
////			return 4;
////		else if ( reserved_word.equals( _reservedWords[ 5]))
////			return 5;
////		else if ( reserved_word.equals( _reservedWords[ 6]))
////			return 6;
////		else if ( reserved_word.equals( _reservedWords[ 7]))
////			return 7;
////		else if ( reserved_word.equals( _reservedWords[ 8]))
////			return 8;
//		else if ( reserved_word.equals( _reservedWords[ 9]))
//			return 9;
//		else if ( reserved_word.equals( _reservedWords[ 10]))
//			return 10;
//		else if ( reserved_word.equals( _reservedWords[ 11]))
//			return 11;
//		else if ( reserved_word.equals( _reservedWords[ 12]))
//			return 12;
////		else if ( reserved_word.equals( _reservedWords[ 13]))
////			return 13;
//		else if ( reserved_word.equals( _reservedWords[ 14]))
//			return 14;
//		else if ( reserved_word.equals( _reservedWords[ 15]))
//			return 15;
//		else if ( reserved_word.equals( _reservedWords[ 16]))
//			return 16;
////		else if ( reserved_word.equals( _reservedWords[ 17]))
////			return 17;
////		else if ( reserved_word.equals( _reservedWords[ 18]))
////			return 18;
//		else if ( reserved_word.equals( _reservedWords[ 19]))
//			return 19;
//		else if ( reserved_word.equals( _reservedWords[ 20]))
//			return 20;
//		else if ( reserved_word.equals( _reservedWords[ 21]))
//			return 21;
//
//		return -1;
//	}
	...
	protected boolean update_spot_variable_name(String originalName, String newName, String player, Role role) {
		...
		String value = CommonRuleManipulator.update_spot_variable_name2( _value, originalName, newName, player);
		if ( null == value)
			return false;
	...
	protected boolean update_keyword_name(String originalName, String newName, String player, Role role) {
		...
		switch ( kind) {
			...
			case 17:
				if ( 3 <= elements.length
					&& !elements[ 2].equals( "")
					&& CommonRuleManipulator.correspond( prefix, elements[ 2], originalName, player)) {
			...
			case 8:
			case 18:
				return update_object_name( kind, prefix, elements, originalName, newName, player);
		}
	 ...
	 * @return
	 */
	private boolean update_object_name(int kind, String prefix, String[] elements, String originalName, String newName, String player) {
		if ( 7 > elements.length)
			return false;
		...
		for ( int i = 2; i < 7; ++i) {
			...
			if ( !elements[ i].equals( "") && CommonRuleManipulator.correspond( prefix, elements[ i], originalName, player)) {
				elements[ i] = newName;
				result = true;
			}
		}
		...
		switch ( kind) {
			...
			case 13:
				if ( 2 <= elements.length
					&& !elements[ 1].equals( "")
					&& CommonRuleManipulator.correspond( prefix, elements[ 1], originalName, player)) {
			...
			case 19:
				if ( 1 <= elements.length
					&& !elements[ 0].equals( "")
					&& CommonRuleManipulator.correspond( prefix, elements[ 0], originalName, player)) {
	...
	protected boolean update_class_variable_name(String originalName, String newName, String player, Role role) {
		return update_object_name( originalName, newName, player, role);
	}
	 ...
	 * @return
	 */
	private boolean update_object_name(String originalName, String newName, String player, Role role) {
		if ( !Environment.get_instance().is_exchange_algebra_enable())
			return false;
		...
		switch ( kind) {
			...
			case 15:
				if ( 2 <= elements.length && !elements[ 0].equals( "") && !elements[ 1].equals( "")) {
					...
					boolean result1 = CommonRuleManipulator.correspond( prefix, elements[ 0], originalName, player);
					boolean result2 = CommonRuleManipulator.correspond( prefix, elements[ 1], originalName, player);
			...
			case 20:
				if ( 2 <= elements.length && !elements[ 0].equals( "") && !elements[ 1].equals( "")) {
					...
					if ( role instanceof AgentRole) {
						result1 = CommonRuleManipulator.correspond( "", elements[ 0], originalName, player);
						result2 = CommonRuleManipulator.correspond( prefix, elements[ 1], originalName, player);
					} else if ( role instanceof SpotRole) {
						result1 = CommonRuleManipulator.correspond( "<>", elements[ 0], originalName, player);
						result2 = CommonRuleManipulator.correspond( prefix, elements[ 1], originalName, player);
					}
			...
			case 21:
				if ( 2 <= elements.length && !elements[ 0].equals( "") && !elements[ 1].equals( "")) {
					...
					if ( role instanceof AgentRole) {
						result1 = CommonRuleManipulator.correspond( prefix, elements[ 0], originalName, player);
						result2 = CommonRuleManipulator.correspond( "", elements[ 1], originalName, player);
					} else if ( role instanceof SpotRole) {
						result1 = CommonRuleManipulator.correspond( prefix, elements[ 0], originalName, player);
						result2 = CommonRuleManipulator.correspond( "<>", elements[ 1], originalName, player);
					}
