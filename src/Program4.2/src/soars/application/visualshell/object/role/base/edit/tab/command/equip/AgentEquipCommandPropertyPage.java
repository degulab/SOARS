 ...
 * @author kurata
 */
public class AgentEquipCommandPropertyPage extends EquipCommandPropertyPageBase {
	 ...
	 * @param parent
	 */
	public AgentEquipCommandPropertyPage(String title, String kind,
	 ...
	 */
	protected boolean set(String value, int kind, int agent, int spot) {
		String[] elements = CommonRuleManipulator.get_elements( value, 2);
		if ( null == elements)
			return false;

		String[] values = CommonRuleManipulator.get_spot_and_object( elements[ spot]);
		if ( null == values)
			return false;
		...
		set1( _comboBoxes.get( kind).get( 0), _comboBoxes.get( kind).get( 1), elements[ agent], values[ 2]);

		_directionComboBox.setSelectedIndex( agent);
