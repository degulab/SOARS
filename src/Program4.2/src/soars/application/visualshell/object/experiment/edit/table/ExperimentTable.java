 ...
 */
public class ExperimentTable extends SpreadSheetTable implements IBasicMenuHandler1, IBasicMenuHandler2, IMessageCallback {
	...
	public void get_column_widths() {
		...
		for ( int i = 0; i < defaultTableColumnModel.getColumnCount(); ++i) {
			_experimentManager._columnWidths.add(
				Integer.valueOf( defaultTableColumnModel.getColumn( i).getPreferredWidth()));
		}
	}
...
//		_experimentTableBase.clear();
	 ...
	 * @param row
	 */
	public void export_java_program(int row) {
		// TODO Auto-generated method stub
		ExportJavaProgramDlg exportJavaProgramDlg = new ExportJavaProgramDlg( MainFrame.get_instance(), ResourceManager.get_instance().get( "file.export.java.program.menu"), true);
		...
		if ( !MessageDlg.execute( MainFrame.get_instance(), ResourceManager.get_instance().get( "application.title"), true,
			"on_export_java_program", ResourceManager.get_instance().get( "file.export.java.program.show.message"),
			new Object[] { folder,
				Environment.get_instance().get( Environment._exportJavaProgramPackagePathKey, "soars2/model"),
				initialValueMap}, this, MainFrame.get_instance()))
			JOptionPane.showMessageDialog(
				_parent,
	...
	public boolean message_callback(String id, Object[] objects, MessageDlg messageDlg) {
		// TODO Auto-generated method stub
		if ( id.equals( "on_export_java_program"))
			return JavaProgramExporter.execute( ( File)objects[ 0], ( String)objects[ 1], ( InitialValueMap)objects[ 2]);
		return true;
	}
