...
import soars.application.visualshell.object.experiment.edit.table.menu.ExportJavaProgramAction;
...
public class ExperimentRowHeaderTable extends SpreadSheetRowHeaderTable implements IBasicMenuHandler3 {
	 ...
	 * 
	 */
	private JMenuItem _exportJavaProgramMenuItem = null;
	...
	protected void setup_popup_menu() {
		...
		_exportJavaProgramMenuItem = _userInterface.append_popup_menuitem(
			_popupMenu,
			ResourceManager.get_instance().get( "file.export.java.program.menu"),
			new ExportJavaProgramAction( ResourceManager.get_instance().get( "file.export.java.program.menu"), this),
	...
	private boolean on_mouse_right_up(int row, int column, Point point) {
		...
		_exportJavaProgramMenuItem.setEnabled( true);
		...
		if ( 0 == defaultTableModel.getRowCount()) {
			...
			_exportJavaProgramMenuItem.setEnabled( false);
		} else {
			if ( ( 0 <= row && getRowCount() > row)
				...
				_exportJavaProgramMenuItem.setEnabled( 1 == rows.length && contains && !contains0);
			} else {
				_copyRowMenuItem.setEnabled( false);
...
//		}
	 ...
	 * @param actionEvent
	 */
	public void on_export_java_program(ActionEvent actionEvent) {
		...
		( ( ExperimentTable)_spreadSheetTableBase).export_java_program( row);
	}
}
