...
public class GisDataManager extends Vector<GisData> {
	 ...
	 * @return
	 */
	public boolean can_append_spots(List<Field> spotFields, List<SimpleVariableObject> simpleVariableObjects, Component component) {
		int[] indices = get_indices( spotFields);
		if ( null == indices)
			return false;

		if ( !set_indices( simpleVariableObjects))
			return false;

		for ( GisData gisData:this) {
			if ( !gisData.can_append_spots( indices, spotFields, simpleVariableObjects, component))
				return false;
		}
	 ...
	 * @return
	 */
	public boolean append_spots(List<Field> spotFields, List<SimpleVariableObject> simpleVariableObjects, double[] range, double[] ratio, JComponent component) {
		int[] indices = get_indices( spotFields);
		if ( null == indices)
			return false;
		...
		boolean result = true;
		for ( GisData gisData:this) {
			if ( !gisData.append_spots( indices, spotFields, simpleVariableObjects, gis, range, ratio, component)) {
				result = false;
				break;
			}
		}
	 ...
	 * @return
	 */
	private boolean set_indices(List<SimpleVariableObject> simpleVariableObjects) {
		for ( SimpleVariableObject simpleVariableObject:simpleVariableObjects) {
			simpleVariableObject._indices = get_indices( simpleVariableObject._fields);
			if ( null == simpleVariableObject._indices)
				return false;
		}
		return true;
	}
