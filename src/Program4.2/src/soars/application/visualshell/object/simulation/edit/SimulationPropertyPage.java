 ...
 * @author kurata
 */
public class SimulationPropertyPage extends TabbedPage implements ITextUndoRedoManagerCallBack {
	...
	private JPanel create1() {
		...
		_panel[ 0] = new JPanel();
		_panel[ 0].setLayout( new BoxLayout( _panel[ 0], BoxLayout.Y_AXIS));

		insert_vertical_strut( _panel[ 0]);

		setup_startTimeComponents( _panel[ 0]);

		insert_vertical_strut( _panel[ 0]);

		setup_stepTimeComponents( _panel[ 0]);

		insert_vertical_strut( _panel[ 0]);

		setup_endTimeComponents( _panel[ 0]);

		insert_vertical_strut( _panel[ 0]);

		_panel[ 0].setBorder( BorderFactory.createLineBorder( getForeground(), 1));

		basicPanel.add( _panel[ 0]);
	...
	private JPanel create2() {
		...
		basic_panel.setLayout( new FlowLayout( FlowLayout.LEFT, 5, 0));

		_panel[ 1] = new JPanel();
		_panel[ 1].setLayout( new BoxLayout( _panel[ 1], BoxLayout.Y_AXIS));

		insert_vertical_strut( _panel[ 1]);

		setup_logStepTimeComponents( _panel[ 1]);
...
//		_set_log_step_time_checkBox.setSelected( SimulationManager.get_instance()._export_log_step_time);
		...
		_panel[ 1].setBorder( new ComponentTitledBorder( _setLogStepTimeCheckBox,
			_panel[ 1], BorderFactory.createLineBorder( getForeground())));

		basic_panel.add( _panel[ 1]);
	...
	private JPanel create3() {
		...
		_panel[ 2] = new JPanel();
		_panel[ 2].setLayout( new BoxLayout( _panel[ 2], BoxLayout.Y_AXIS));

		insert_vertical_strut( _panel[ 2]);

		setup_exportEndTimeCheckBox( _panel[ 2]);

		insert_vertical_strut( _panel[ 2]);

		_panel[ 2].setBorder( BorderFactory.createLineBorder( getForeground(), 1));

		basic_panel.add( _panel[ 2]);
	...
	private JPanel create4() {
		...
		_panel[ 3] = new JPanel();
		_panel[ 3].setLayout( new BoxLayout( _panel[ 3], BoxLayout.Y_AXIS));

		insert_vertical_strut( _panel[ 3]);

		setup_randomSeedTextField( _panel[ 3]);

		insert_vertical_strut( _panel[ 3]);

		_panel[ 3].setBorder( BorderFactory.createLineBorder( getForeground(), 1));

		basic_panel.add( _panel[ 3]);
	...
	private void adjust() {
		...
		for ( int i = 0; i < _panel.length; ++i)
			width = Math.max( width, _panel[ i].getPreferredSize().width);
		for ( int i = 0; i < _panel.length; ++i)
			_panel[ i].setPreferredSize( new Dimension( width, _panel[ i].getPreferredSize().height));
	}
