...
public class AnimatorLauncher {
	...
	private static String[] get_cmdarray(String language, String home_directory_name, String property_directory_name, String anm_filename, String parameter_filename, String osname) {
		...
		if ( 0 <= osname.indexOf( "Windows")) {
			...
			list.add( "-X" + Constant._macIconFilename + "=" + home_directory_name + Constant._library_directory + "/" + Constant._icon_filename);
		...
		list.add( "-jar");
		list.add( home_directory_name + Constant._library_directory + "/" + CommonConstant._animatorRunnerInternalLauncherJarFilename);
		list.add( "-language");
		list.add( language);
		list.add( "-anm");
