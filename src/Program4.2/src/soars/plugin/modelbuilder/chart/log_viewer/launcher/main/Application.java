 ...
 * 
 */
package soars.plugin.modelbuilder.chart.log_viewer.launcher.main;
...
public class Application {
	...
	public boolean execute() {
		File current_directory = new File( "");
		if ( null == current_directory) {
			JOptionPane.showMessageDialog( null, "Could not start Chart Log Viewer!", "SOARS", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		debug( "Chart Log Viewer", System.getProperty( "os.name"), System.getProperty( "os.version"), cmdarray);
	...
	private String[] get_cmdarray(File current_directory) {
		...
		if ( 0 <= osname.indexOf( "Windows")) {
			...
			list.add( "-X" + Constant._macScreenMenuName + "=SOARS Chart Log Viewer");
			//list.add( "-D" + Constant._mac_screen_menu_name + "=SOARS Chart Log Viewer");
		...
		list.add( "-jar");
		list.add( current_directory.getAbsolutePath() + "/" + Constant._log_viewer_jar_filename);
		list.add( "-language");
