...
public class GridLoginDlg extends Dialog implements ITimerTaskImplementCallback {
	...
	private void setup_grid_portal_ip_address_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.grid2.login.dialog.gird.portal.ip.address"), SwingConstants.RIGHT);
		panel.add( label);
		_labels.add( label);
	...
	private void setup_username_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.grid2.login.dialog.username"));
		panel.add( label);
		_labels.add( label);
	...
	protected void on_ok(ActionEvent actionEvent) {
		...
		if ( !SshTool.directory_exists( _receiver.get(), _parameters._username, _parameters._password,
			...
			if ( JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog(
				this,
				ResourceManager.get( "plugin.grid2.login.dialog.confirm.create.new.user.message"),
				...
				if ( !SshTool.mkdirs( _receiver.get(), _parameters._username, _parameters._password,
					...
					JOptionPane.showMessageDialog(
						this,
						ResourceManager.get( "plugin.grid2.login.dialog.could.not.create.new.user.message"),
						ResourceManager.get( "plugin.grid2.login.dialog.title"),
