...
public class GridLoginDlg extends Dialog {
	...
	private void setup_host_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.tsubame2.login.dialog.gird.portal.ip.address"));
		panel.add( label);
		_labels.add( label);
	...
	private void setup_username_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.tsubame2.login.dialog.gird.portal.username"));
		panel.add( label);
		_labels.add( label);
	...
	private void setup_password_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.tsubame2.login.dialog.gird.portal.password"));
		panel.add( label);
		_labels.add( label);
	...
	private void setup_local_username_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.tsubame2.login.dialog.username"));
		panel.add( label);
		_labels.add( label);
	...
	private void setup_store_information_checkBox() {
		...
		_store_information_checkBox = new JCheckBox( ResourceManager.get( "plugin.tsubame2.login.dialog.store.information"),
	...
	protected void on_ok(ActionEvent actionEvent) {
		...
		if ( null == sshClient) {
			JOptionPane.showMessageDialog(
				this,
				ResourceManager.get( "plugin.tsubame2.login.dialog.could.not.login.message"),
		...
		if ( !SshTool.directory_exists( host, username, password, _root_directory + "/" + local_username)) {
			if ( JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog(
				this,
				ResourceManager.get( "plugin.tsubame2.login.dialog.confirm.create.new.user.message"),
				ResourceManager.get( "plugin.tsubame2.login.dialog.title"),
				...
				if ( !SshTool.mkdirs( host, username, password, _root_directory + "/" + local_username)) {
					JOptionPane.showMessageDialog(
						this,
						ResourceManager.get( "plugin.tsubame2.login.dialog.could.not.create.new.user.message"),
						ResourceManager.get( "plugin.tsubame2.login.dialog.title"),
						JOptionPane.ERROR_MESSAGE);
					store( host, username, password, local_username);
					return;
