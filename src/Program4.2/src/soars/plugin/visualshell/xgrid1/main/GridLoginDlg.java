...
public class GridLoginDlg extends Dialog {
	...
	private void setup_gridPortalIpAddressTextField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.xgrid1.login.dialog.gird.portal.ip.address"), SwingConstants.RIGHT);
		panel.add( label);
	...
	private void setup_usernameTextField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.xgrid1.login.dialog.username"), SwingConstants.RIGHT);
		panel.add( label);
		_components.add( label);
	...
	private void setup_privateKeyFilenameTextField() {
		...
		JButton button = new JButton( ResourceManager.get( "plugin.xgrid1.login.dialog.private.key.filename"));
	...
	protected void on_privateKeyFileSelectorButton_actionPerformed(ActionEvent actionEvent) {
		File file = Tool.get_file( _privateKeyFilename,
			ResourceManager.get( "plugin.xgrid1.private.key.file.selector.dialog.title"), new String[] { "ppk"}, "Private key file", this);
		if ( null == file)
			return;
	...
	protected void on_ok(ActionEvent actionEvent) {
		...
		if ( null == sshClient) {
			JOptionPane.showMessageDialog(
				this,
				ResourceManager.get( "plugin.xgrid1.login.dialog.could.not.login.message"),
		...
		if ( !SshTool2.directory_exists( gridPortalIpAddress, username, new File( _privateKeyFilename), dataDirectoryName)) {
			if ( !SshTool2.mkdirs( gridPortalIpAddress, username, new File( _privateKeyFilename), dataDirectoryName)) {
				JOptionPane.showMessageDialog(
					this,
					ResourceManager.get( "plugin.xgrid1.login.dialog.could.not.create.user.folder.message"),
					ResourceManager.get( "plugin.xgrid1.login.dialog.title"),
					JOptionPane.ERROR_MESSAGE);
				store( gridPortalIpAddress, username);
				return;
