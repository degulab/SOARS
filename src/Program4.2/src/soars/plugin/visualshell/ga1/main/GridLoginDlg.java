...
public class GridLoginDlg extends Dialog {
	...
	private void setup_grid_portal_ip_address_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.login.dialog.gird.portal.ip.address"), SwingConstants.RIGHT);
		panel.add( label);
		_labels.add( label);
	...
	private void setup_username_textField() {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.login.dialog.username"));
		panel.add( label);
		_labels.add( label);
	...
	protected void on_ok(ActionEvent actionEvent) {
		...
		if ( null == sshClient) {
			JOptionPane.showMessageDialog(
				this,
				ResourceManager.get( "plugin.ga1.login.dialog.could.not.login.message"),
...
//			store( grid_portal_ip_address, local_username);
		...
		if ( !SshTool.directory_exists( grid_portal_ip_address, _parameters._username, _parameters._password,
			...
			if ( JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog(
				this,
				ResourceManager.get( "plugin.ga1.login.dialog.confirm.create.new.user.message"),
				ResourceManager.get( "plugin.ga1.login.dialog.title"),
...
//				store( grid_portal_ip_address, local_username);
				...
				if ( !SshTool.mkdirs( grid_portal_ip_address, _parameters._username, _parameters._password,
					...
					JOptionPane.showMessageDialog(
						this,
						ResourceManager.get( "plugin.ga1.login.dialog.could.not.create.new.user.message"),
						ResourceManager.get( "plugin.ga1.login.dialog.title"),
						JOptionPane.ERROR_MESSAGE);
//					store( grid_portal_ip_address, local_username);
					return;
