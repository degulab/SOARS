...
public class GADlg extends Dialog implements INumberSpinnerHandler {
	...
	public static boolean execute(String name, File file, JFrame frame, String username, String password, String root_directory, String java_home, String home_directory, String sgeout_directory, String program_directory) {
		...
		if ( null == resultList || resultList.isEmpty() || null == resultList.get( 0)) {
			JOptionPane.showMessageDialog(
				frame,
				ResourceManager.get( "plugin.ga1.error.message.no.script"),
				ResourceManager.get( "plugin.ga1.dialog.title"),
		...
		algorithm_name_map.put( "UxMgg", ResourceManager.get( "plugin.ga1.uniform.crossover"));
		algorithm_name_map.put( "UndxMgg", ResourceManager.get( "plugin.ga1.unimodal.normal.distribution.crossover"));

		Map<String, String> algorithm_type_map = new HashMap<String, String>();
		algorithm_type_map.put( ResourceManager.get( "plugin.ga1.uniform.crossover"), "UxMgg");
		algorithm_type_map.put( ResourceManager.get( "plugin.ga1.unimodal.normal.distribution.crossover"), "UndxMgg");
		...
		GAMonitorDlg gaMonitorDlg = Environment.get_instance().get( Environment._grid_key).equals( "true")
			? new GAGridMonitorDlg(
					frame,
					ResourceManager.get( "plugin.ga1.grid.frame.title")
			...
			: new GALocalMonitorDlg(
					frame,
					ResourceManager.get( "plugin.ga1.local.frame.title")
						+ ( ( null == file) ? "" : ( " - " + file.getName())),
					true,
					name,
					localParameters,
	...
	private static boolean edit_experimentManager(String name, JFrame frame, Object experimentManager, List<String> algorithm) {
		...
		if ( !Reflection.execute_class_method( frame, "edit_experimentManager_for_genetic_algorithm",
			...
			new Object[] { experimentManager, ResourceManager.get( "plugin.ga1.dialog.title"), frame, algorithm},
			resultList)) {
			JOptionPane.showMessageDialog( frame,
				"Plugin error : Reflection.execute_class_method( ... )" + Constant._lineSeparator
				+ " Plugin name : " + name + Constant._lineSeparator
	...
	private void setup_left_panel(JPanel parent) {
		...
		setup_left_north_panel( north_panel);
	...
	private void setup_basic_panel(JPanel parent) {
		...
		basic_panel.add( Box.createHorizontalStrut( 10));
	...
	private void setup_population_size_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.population.size"));
	...
	private void setup_number_of_crossovers_numberSpinner(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.number.of.crossovers"));
		label.setHorizontalAlignment( SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
	...
	private void setup_number_of_generation_alternations_numberSpinner(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.number.of.generation.alternations"));
		label.setHorizontalAlignment( SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
	...
	private void setup_length_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.length"));
		label.setHorizontalAlignment( SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
	...
	private void setup_algorithm_comboBox(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.algorithm"));
		label.setHorizontalAlignment( SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
		...
		_algorithm_comboBox = new JComboBox( new String[] {
			ResourceManager.get( "plugin.ga1.uniform.crossover"),
			ResourceManager.get( "plugin.ga1.unimodal.normal.distribution.crossover")});
	...
	private void setup_minimization_checkBox(JPanel parent) {
		...
		_minimization_checkBox = new JCheckBox( ResourceManager.get( "plugin.ga1.dialog.minimization"));
	...
	private void setup_roulette_selection_checkBox(JPanel parent) {
		...
		_roulette_selection_checkBox = new JCheckBox( ResourceManager.get( "plugin.ga1.dialog.roulette.selection"));
	...
	private void setup_number_variable_comboBox(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.evaluation.value"));
		label.setHorizontalAlignment( SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
		...
		_spot_checkBox = new JCheckBox( ResourceManager.get( "plugin.ga1.dialog.spot"));
	...
	private void setup_local_log_directory_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.local.log.directory"), SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
		...
		_local_log_directory_selector_button = new JButton( ResourceManager.get( "plugin.ga1.dialog.local.log.reference"));
	...
	protected void on_local_log_directory_selector_button_actionPerformed(ActionEvent actionEvent) {
		File directory = Tool.get_directory( _local_log_directory_textField.getText(),
			ResourceManager.get( "plugin.ga1.local.log.directory.selector.dialog.title"), this);
		if ( null == directory)
			return;
	...
	private void setup_grid_panel(JPanel parent) {
		...
		grid_panel.add( Box.createHorizontalStrut( 10));
		...
		_grid_checkBox = new JCheckBox( ResourceManager.get( "plugin.ga1.dialog.grid"));
		...
		grid_panel.add( panel);

		grid_panel.add( Box.createHorizontalStrut( 5));

		parent.add( grid_panel);
	}
	...
	protected void on_grid_checkBox_state_changed(boolean selected) {
		if ( selected) {
			GridLoginDlg gridLoginDlg = new GridLoginDlg(
				_frame,
				ResourceManager.get( "plugin.ga1.login.dialog.title"),
				true,
				_root_directory,
				_parameters);
		...
		if ( selected) {
			if ( _advanced_memory_setting_checkBox.isSelected()) {
				...
				_model_builder_memory_size = ( memory_size.equals( ResourceManager.get( "plugin.ga1.dialog.model.builder.memory.non.use")) ? "0" : memory_size);
			}
		}
	}
	...
	private void setup_grid_portal_ip_address_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.gird.portal.ip.address"), SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
	...
	private void setup_local_username_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.username"), SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
	...
	private void setup_script_directory_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.script.directory"), SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
		...
		_script_directory_selector_button = new JButton( ResourceManager.get( "plugin.ga1.dialog.script.reference"));
	...
	protected void on_script_directory_selector_button_actionPerformed(ActionEvent actionEvent) {
		String directory = Tool.get_remote_directory( _script_directory_textField.getText(),
			ResourceManager.get( "plugin.ga1.script.directory.selector.dialog.title"),
	...
	private void setup_log_directory_textField(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.log.directory"), SwingConstants.RIGHT);
		_components.add( label);
		panel.add( label);
		...
		_log_directory_selector_button = new JButton( ResourceManager.get( "plugin.ga1.dialog.log.reference"));
	...
	protected void on_log_directory_selector_button_actionPerformed(ActionEvent actionEvent) {
		String directory = Tool.get_remote_directory( _log_directory_textField.getText(),
			ResourceManager.get( "plugin.ga1.log.directory.selector.dialog.title"),
			( Frame)getOwner(), this, _parameters._grid_portal_ip_address, _parameters._username, _parameters._password,
			_root_directory, _parameters._local_username);
		if ( null == directory)
			return;
	...
	private void setup_model_builder_memory_size_comboBox(JPanel parent) {
		...
		_model_builder_memory_size_label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.model.builder.memory.size"));
		...
		memory_sizes[ 0] = ResourceManager.get( "plugin.ga1.dialog.model.builder.memory.non.use");
	...
	private void setup_model_builder_memory_size_numberSpinner(JPanel parent) {
		...
		_advanced_memory_setting_checkBox = new JCheckBox( ResourceManager.get( "plugin.ga1.dialog.model.builder.advanced.memory.size"));
	...
	private void setup_right_panel(JPanel parent) {
		...
		JPanel north_panel = new JPanel();
		north_panel.setLayout( new BoxLayout( north_panel, BoxLayout.Y_AXIS));
		setup_right_north_panel( north_panel);
		panel.add( north_panel, "North");

		JPanel center_panel = new JPanel();
		center_panel.setLayout( new BoxLayout( center_panel, BoxLayout.Y_AXIS));
		setup_right_center_panel( center_panel);
	...
	private void setup_error_log_label(JPanel parent) {
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.dialog.error.log"));
		panel.add( label);
	...
	private void setup_south_panel(JPanel parent) {
		...
		setup_ok_and_cancel_button( panel,
			ResourceManager.get( "dialog.start"),
	...
	protected void on_setup_completed() {
		...
		_model_builder_memory_size_numberSpinner.set_value( Integer.valueOf( _model_builder_memory_size).intValue());
		if ( !_advanced_memory_setting_checkBox.isSelected()) {
			if ( _model_builder_memory_size.equals( "0") || Constant.contained( _model_builder_memory_size))
				_model_builder_memory_size_comboBox.setSelectedItem( _model_builder_memory_size.equals( ResourceManager.get( "plugin.ga1.dialog.model.builder.memory.non.use")) ? "0" : _model_builder_memory_size);
			else {
