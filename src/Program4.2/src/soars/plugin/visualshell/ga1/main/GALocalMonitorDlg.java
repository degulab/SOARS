...
public class GALocalMonitorDlg extends GAMonitorDlg {
	...
	protected void setup_local_panel(JPanel parent) {
		...
		local_panel.setLayout( new BoxLayout( local_panel, BoxLayout.X_AXIS));

		local_panel.add( Box.createHorizontalStrut( 10));
		...
		JLabel label = new JLabel( ResourceManager.get( "plugin.ga1.local.frame"));
		panel.setBorder( new ComponentTitledBorder( label, panel, BorderFactory.createLineBorder( getForeground())));

		local_panel.add( panel);

		local_panel.add( Box.createHorizontalStrut( 5));

		parent.add( local_panel);
	}
