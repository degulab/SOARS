 ...
 */
public class RoleForSpots extends RoleForExalge {
	...
	/**
	 * Command method to create new spot.
	 * @param equality setting formula
	 */
	public void createSpot(String equality) {
		String[] trio = equality.split("=", 3);
		final String nameHead = trio[0].trim();
		final int number = trio.length >= 2 ? Integer.parseInt(trio[1].trim()) : 1;
		final String roleType = trio.length >= 3 ? trio[2].trim() : null;
		new Resolver() {
			private static final long serialVersionUID = 4826095878849132423L;
			public void resolve() throws InstantiationException, IllegalAccessException {
				int n = number;
				while (n-- > 0) {
					Spot spot = new Spot();
					spot.assignName(nameHead);
					if (roleType != null) {
						startRuleImpl(spot, roleType);
					}
				}
			}
		}.requestResolve();
