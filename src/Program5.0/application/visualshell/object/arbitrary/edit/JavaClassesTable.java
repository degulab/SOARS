...
public class JavaClassesTable extends TableBase implements IBasicMenuHandler1 {
	...
	public void on_append(ActionEvent actionEvent) {
		...
		List<String> exceptedJavaClasses = new ArrayList();
		for ( int i = 0; i < defaultTableModel.getRowCount(); ++i)
			exceptedJavaClasses.add( ( String)defaultTableModel.getValueAt( i, 0));
		...
		SelectJavaClassDlg selectJavaClassDlg = new SelectJavaClassDlg(
			...
			true,
			exceptedJavaClasses/*.toArray( new String[ 0])*/);

		if ( !selectJavaClassDlg.do_modal( _parent))
			return;

		if( null == selectJavaClassDlg._javaClassName
			|| selectJavaClassDlg._javaClassName.equals( ""))
			return;

		while ( 0 < defaultTableModel.getRowCount())
			defaultTableModel.removeRow( 0);

		String selectedJavaClass = selectJavaClassDlg._javaClassName;

		exceptedJavaClasses.add( selectJavaClassDlg._javaClassName);

		String[] javaClasses = exceptedJavaClasses.toArray( new String[ 0]);
