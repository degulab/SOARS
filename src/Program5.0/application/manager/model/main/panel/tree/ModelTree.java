...
import soars.application.manager.model.menu.edit.NewSimulationModel4Action;
import soars.application.manager.model.menu.edit.NewSimulationModel5Action;
...
public class ModelTree extends StandardTree implements DragGestureListener, DragSourceListener, DropTargetListener, IIntProgressCallback, IFileManager, IFileManagerCallBack, IEditMenuHandler, IRunMenuHandler {
	 ...
	 * 
	 */
	private JMenuItem _editNewSimulationModel5MenuItem = null;

	/**
	 * 
	 */
	private JMenuItem _editNewSimulationModel4MenuItem = null;
...
//		TreePath treePath = treeSelectionEvent.getPath();
	...
	public void editMenuSelected(Map<String, JMenuItem> menuItemMap) {
		...
		if ( null == treePath) {
			...
			menuItemMap.get( ResourceManager.get_instance().get( "edit.new.simulation.model5.menu")).setEnabled( false);
			menuItemMap.get( ResourceManager.get_instance().get( "edit.new.simulation.model4.menu")).setEnabled( false);
		...
		menuItemMap.get( ResourceManager.get_instance().get( "edit.new.simulation.model5.menu")).setEnabled( file.isDirectory());
		menuItemMap.get( ResourceManager.get_instance().get( "edit.new.simulation.model4.menu")).setEnabled( file.isDirectory());
	...
	protected void setup_popup_menu() {
		...
		_editNewSimulationModel5MenuItem = _userInterface.append_popup_menuitem(
			_popupMenu,
			ResourceManager.get_instance().get( "edit.new.simulation.model5.menu"),
			new NewSimulationModel5Action( ResourceManager.get_instance().get( "edit.new.simulation.model5.menu"), this),
			ResourceManager.get_instance().get( "edit.new.simulation.model5.mnemonic"),
			ResourceManager.get_instance().get( "edit.new.simulation.model5.stroke"));

		_editNewSimulationModel4MenuItem = _userInterface.append_popup_menuitem(
			_popupMenu,
			ResourceManager.get_instance().get( "edit.new.simulation.model4.menu"),
			new NewSimulationModel4Action( ResourceManager.get_instance().get( "edit.new.simulation.model4.menu"), this),
			ResourceManager.get_instance().get( "edit.new.simulation.model4.mnemonic"),
			ResourceManager.get_instance().get( "edit.new.simulation.model4.stroke"));
	...
	protected void on_mouse_right_up(MouseEvent mouseEvent) {
		...
		_editNewSimulationModel5MenuItem.setEnabled( file.isDirectory());
		_editNewSimulationModel4MenuItem.setEnabled( file.isDirectory());
...
//				return true;
	...
	@Override
	public void on_edit_new_simulation_model5(ActionEvent actionEvent) {
		on_edit_new_simulation_model( false);
	}

	@Override
	public void on_edit_new_simulation_model4(ActionEvent actionEvent) {
		on_edit_new_simulation_model( true);
	}

	/**
	 * @param visualshell4
	 */
	private void on_edit_new_simulation_model(boolean visualshell4) {
		TreePath treePath = getSelectionPath();
		DefaultMutableTreeNode defaultMutableTreeNode = ( DefaultMutableTreeNode)treePath.getLastPathComponent();
		DefaultMutableTreeNode parent = ( DefaultMutableTreeNode)defaultMutableTreeNode.getParent();
		...
		ModelInformationDlg modelInformationDlg = new ModelInformationDlg(
			_owner,
			ResourceManager.get_instance().get( visualshell4 ? "new.simulation.model4.dialog.title" : "new.simulation.model5.dialog.title"),
			true,
			folder,
			visualshell4,
			_mainPanel);
	...
	private boolean paste(File[] files, int action) {
		...
		int result = IntProgressDlg.execute(
			...
			"copy",
			ResourceManager.get_instance().get( "dialog.cancel"),
			new Object[] { files, targetDirectory, Integer.valueOf( action), Boolean.valueOf( false)},
			this, _parent);
		switch ( result) {
			case -1:
				Message.on_error_from_parent_to_child( _owner);
...
//				break;
		...
		if ( 0 < result) {
			result = IntProgressDlg.execute(
				...
				"remove",
				ResourceManager.get_instance().get( "dialog.cancel"),
				new Object[] { files, targetDirectory, Integer.valueOf( action)},
				this, _parent);
			switch ( result) {
				case -3:
