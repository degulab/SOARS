...
public class GridFrame extends Frame implements Runnable, ITimerTaskImplementCallback {
	 ...
	 */
	public GridFrame(String arg0, String name, Component parent, Parameters parameters, Object experimentManager, Environment environment, Receiver receiver) throws HeadlessException {
		super(arg0);
		_name = name;
		_parent = parent;
	...
	protected void on_window_closing(WindowEvent windowEvent) {
		if ( JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog( this,
			ResourceManager.get_instance().get( "plugin.grid2.frame.exit.confirm.message"), getTitle(), JOptionPane.YES_NO_OPTION))
			return;
	...
	private void setup_grid_portal_ip_address_textField() {
		...
		_labels[ 0] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.gird.portal.ip.address"), SwingConstants.RIGHT);
		panel.add( _labels[ 0]);
	...
	private void setup_username_textField() {
		...
		_labels[ 1] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.username"), SwingConstants.RIGHT);
		panel.add( _labels[ 1]);
	...
	private void setup_script_directory_textField() {
		...
		_labels[ 2] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.script.directory"), SwingConstants.RIGHT);
	...
	private void setup_log_directory_textField() {
		...
		_labels[ 3] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.log.directory"), SwingConstants.RIGHT);
		panel.add( _labels[ 3]);
	...
	private void setup_model_builder_memory_size_textField() {
		...
		_labels[ 4] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.model.builder.memory.size"), SwingConstants.RIGHT);
		panel.add( _labels[ 4]);
	...
	private void setup_number_of_times_textField() {
		...
		_labels[ 5] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.number.of.times"), SwingConstants.RIGHT);
	...
	private void setup_log_analysis_condition_filename_textField() {
		...
		_labels[ 6] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.log.analysis.condition.filename"), SwingConstants.RIGHT);
	...
	private void setup_local_log_directory_textField() {
		...
		_labels[ 7] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.local.log.directory"), SwingConstants.RIGHT);
	...
	private void setup_number_of_scripts_textField() {
		...
		_labels[ 8] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.number.of.script"), SwingConstants.RIGHT);
		panel.add( _labels[ 8]);
	...
	private void setup_number_of_submitted_scripts_textField() {
		...
		_labels[ 9] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.number.of.submitted.script"), SwingConstants.RIGHT);
		panel.add( _labels[ 9]);
	...
	private void setup_status_textField() {
		...
		_labels[ 10] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.status"), SwingConstants.RIGHT);
	...
	private void setup_progressBar() {
		...
		_labels[ 11] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.progress"), SwingConstants.RIGHT);
	...
	private void setup_error_log_textArea() {
		...
		JLabel label = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.frame.error.log"));
		panel.add( label);
	...
	protected void on_stop(ActionEvent actionEvent) {
		if ( !_termination
			&& JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog( this,
				ResourceManager.get_instance().get( "plugin.grid2.frame.cancel.confirm.message"), getTitle(), JOptionPane.YES_NO_OPTION))
			return;
	...
	private void on_stop() {
		...
		JOptionPane.showMessageDialog( this,
			ResourceManager.get_instance().get( "plugin.grid2.frame.canceled"),
			getTitle(),
			JOptionPane.INFORMATION_MESSAGE);
		_status_textField.setText( "Canceled ...");
		_status_textField.update( _status_textField.getGraphics());
		_error_log_textArea.append( ResourceManager.get_instance().get( "plugin.grid2.frame.canceled"));
		setEnabled( true);
...
//		return null;
	...
	private boolean export_soars_scripts() {
		...
		if ( !Reflection.execute_class_method( object, "export_data_for_grid",
			...
			new Object[] { _experimentManager,
				...
				Integer.valueOf( _environment.get( Environment._number_of_times_for_grid_key, "1")),
				_receiver.get(),
	...
	private boolean submit() {
		...
		Connection connection = new Connection( _receiver.get(), 22);
		try {
			connection.connect();
		} catch (IOException e) {
			SshTool.close( sftpClient);
	...
	private boolean analyze(SshClient sshClient, SftpClient sftpClient, Connection connection) {
		...
		while ( iterator.hasNext()) {
			...
			String result = execute( sshClient, sftpClient,
				new SftpFile( analysis_shell_script_directory), connection, Constant._analysis_shell_script_file_name);
			if ( null == result) {
...
//			else {
	...
	public void execute_timer_task(int id) {
		...
		try {
			sshClient = SshTool.getSshClient( _receiver.get(), _parameters._username, _parameters._password);
		} catch (Throwable ex) {
			_running = false;
			return;
		}
		...
		try {
			connection = new Connection( _receiver.get(), 22);
		} catch (Throwable ex) {
