 ...
 * @author kurata
 */
public class GridDlg extends Dialog implements INumberSpinnerHandler, ITimerTaskImplementCallback {
	 ...
	 * @return
	 */
	public static boolean execute(String name, File file, JFrame frame, String port, String username, String password, String root_directory, String java_home, String home_directory, String sgeout_directory, String program_directory) {
		List resultList = get_experimentManager( name, frame);
		if ( null == resultList || resultList.isEmpty()) {
			JOptionPane.showMessageDialog(
				frame,
				ResourceManager.get_instance().get( "plugin.grid2.error.message.no.script"),
				ResourceManager.get_instance().get( "plugin.grid2.dialog.title"),
		...
		try {
			parameters._port = Integer.parseInt( port);
		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(
				frame,
				ResourceManager.get_instance().get( "plugin.grid2.error.message.invalid.port.number"),
				ResourceManager.get_instance().get( "plugin.grid2.dialog.title"),
				JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		try {
			receiver = new Receiver( parameters._port);
		} catch (SocketException e) {
			JOptionPane.showMessageDialog(
				frame,
				ResourceManager.get_instance().get( "plugin.grid2.error.message.could.not.get.portal.ip.address"),
				ResourceManager.get_instance().get( "plugin.grid2.dialog.title"),
				JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		GridFrame gridFrame = new GridFrame(
			ResourceManager.get_instance().get( "plugin.grid2.frame.title")
	 ...
	 */
	public GridDlg(JFrame arg0, String arg1, boolean arg2, String name, String root_directory, Parameters parameters, Object experimentManager, Receiver receiver) throws HeadlessException {
	...
	private void setup_grid_portal_ip_address_textField() {
		...
		_labels[ 0] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.gird.portal.ip.address"), SwingConstants.RIGHT);
		panel.add( _labels[ 0]);

		_grid_portal_ip_address_textField = new JTextField();
		_grid_portal_ip_address_textField.setEditable( false);
		_grid_portal_ip_address_textField.setPreferredSize( new Dimension( 400,
	...
	private void setup_username_textField() {
		...
		_labels[ 1] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.username"), SwingConstants.RIGHT);
		panel.add( _labels[ 1]);
	...
	private void setup_script_directory_textField() {
		...
		_labels[ 2] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.script.directory"), SwingConstants.RIGHT);
		panel.add( _labels[ 2]);
		...
		_script_directory_selector_button = new JButton( ResourceManager.get_instance().get( "plugin.grid2.dialog.script.reference"));
	...
	protected void on_script_directory_selector_button_actionPerformed(ActionEvent actionEvent) {
		...
		String directory = Tool.get_remote_directory( _script_directory_textField.getText(),
			ResourceManager.get_instance().get( "plugin.grid2.script.directory.selector.dialog.title"),
			( Frame)getOwner(), this, _receiver.get(), _parameters._username, _parameters._password,
	...
	private void setup_log_directory_textField() {
		...
		_labels[ 3] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.log.directory"), SwingConstants.RIGHT);
		panel.add( _labels[ 3]);
		...
		_log_directory_selector_button = new JButton( ResourceManager.get_instance().get( "plugin.grid2.dialog.log.reference"));
	...
	protected void on_log_directory_selector_button_actionPerformed(ActionEvent actionEvent) {
		...
		String directory = Tool.get_remote_directory( _log_directory_textField.getText(),
			ResourceManager.get_instance().get( "plugin.grid2.log.directory.selector.dialog.title"),
			( Frame)getOwner(), this, _receiver.get(), _parameters._username, _parameters._password,
			_root_directory, _parameters._local_username);
		if ( null == directory)
			return;
	...
	private void setup_model_builder_memory_size_numberSpinner() {
		...
		_labels[ 4] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.model.builder.memory.size"), SwingConstants.RIGHT);
		panel.add( _labels[ 4]);
	...
	private void setup_simulation_checkBox() {
		...
		_simulation_checkBox = new JCheckBox(
			ResourceManager.get_instance().get( "plugin.grid2.dialog.simulation"),
	...
	private void setup_number_of_times_numberSpinner() {
		...
		_labels[ 5] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.number.of.times"), SwingConstants.RIGHT);
		panel.add( _labels[ 5]);
	...
	private void setup_log_analysis_checkBox() {
		...
		_log_analysis_checkBox = new JCheckBox(
			ResourceManager.get_instance().get( "plugin.grid2.dialog.log.analysis"),
	...
	private void setup_log_analysis_condition_filename_textField() {
		...
		_labels[ 6] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.log.analysis.condition.filename"), SwingConstants.RIGHT);
		...
		_log_analysis_condition_file_selector_button = new JButton( ResourceManager.get_instance().get( "plugin.grid2.dialog.log.analysis.condition.filename.reference"));
	...
	protected void on_log_analysis_condition_file_selector_button_actionPerformed(ActionEvent actionEvent) {
		...
		File file = Tool.get_file( _log_analysis_condition_filename_textField.getText(),
			ResourceManager.get_instance().get( "plugin.grid2.log.analysis.condition.file.selector.dialog.title"),
			new String[] { "groovy"},
	...
	private void setup_transfer_log_checkBox() {
		...
		_transfer_log_checkBox = new JCheckBox(
			ResourceManager.get_instance().get( "plugin.grid2.dialog.log.transfer"),
	...
	private void setup_local_log_directory_textField() {
		...
		_labels[ 7] = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.local.log.directory"), SwingConstants.RIGHT);
		panel.add( _labels[ 7]);
		...
		_local_log_directory_selector_button = new JButton( ResourceManager.get_instance().get( "plugin.grid2.dialog.local.log.reference"));
	...
	protected void on_local_log_directory_selector_button_actionPerformed(ActionEvent actionEvent) {
		File directory = Tool.get_directory( _local_log_directory_textField.getText(),
			ResourceManager.get_instance().get( "plugin.grid2.local.log.directory.selector.dialog.title"), this);
		if ( null == directory)
			return;
	...
	private void setup_error_log_textArea() {
		...
		JLabel label = new JLabel( ResourceManager.get_instance().get( "plugin.grid2.dialog.error.log"));
		panel.add( label);
	...
	private boolean edit_experimentManager() {
		...
		if ( !Reflection.execute_class_method( _frame, "edit_experimentManager",
			...
			new Object[] { _experimentManager, ResourceManager.get_instance().get( "plugin.grid2.dialog.title"), this},
			resultList)) {
	...
	private boolean clear(String script_directory, String log_directory, String local_log_directory) {
		if ( Environment.get_instance().get( Environment._simulation_key, "false").equals( "true")) {
			SshClient sshClient = SshTool.getSshClient( _receiver.get(), _parameters._username, _parameters._password);
			if ( null == sshClient)
				return false;
