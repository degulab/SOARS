...
//	list.add( "-Djava.endorsed.dirs=" + currentDirectoryName + "/../" + Constant._xercesDirectory);
		...
		list.add( "-D" + CommonEnvironment._localeKey + "=" + Locale.getDefault().getLanguage());
...
//		list.add( currentDirectoryName + "/" + Constant._soarsEngineJarFilename);
	 ...
	 */
	public static boolean run(ScriptFile scriptFile, MonitorPropertyPage monitorPropertyPage, File currentDirectory, String currentDirectoryName, boolean toFile, boolean keepUserDataFile) {
		String propertyDirectoryName = System.getProperty( Constant._soarsProperty);
		String memorySize = CommonEnvironment.get_instance().get_memory_size();
		String osname = System.getProperty( "os.name");

		String[] cmdarray = null;
		if ( !toFile && !keepUserDataFile)
			cmdarray = get_cmdarray( scriptFile._path, currentDirectory, currentDirectoryName, propertyDirectoryName, memorySize, osname, null, null);
		else {
			Parameters parameters = new Parameters();
			...
			if ( toFile) {
				if ( !LogInformationFileMaker.make( scriptFile._logFolderPath))
					return false;
			}

			cmdarray = get_cmdarray( scriptFile._path, currentDirectory, currentDirectoryName, propertyDirectoryName, memorySize, osname, parameters.get_user_data_directory(), parameters.get_gaming_runner_file());
		}
...
//		list,.add( "-Djava.endorsed.dirs=" + currentDirectoryName + "/../" + Constant._xercesDirectory);
	 ...
	 * @param keepUserDataFile 
	 */
	public static boolean run(ScriptFile[] scriptFiles, MonitorPropertyPage monitorPropertyPage, File currentDirectory, String currentDirectoryName, boolean toFile, boolean keepUserDataFile) {
		if ( toFile || keepUserDataFile) {
					...
					if ( line.startsWith( "pleaseprint"))
						...
						if ( !ignore)
							...
							if ( line.startsWith( "Execution Time:") || line.startsWith( "Caused by:"))
								...
								monitorPropertyPage.update( line);
							//else
							//	monitorPropertyPage.update( "This is a test !\n");
						}
					}
