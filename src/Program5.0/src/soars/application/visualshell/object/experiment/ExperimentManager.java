...
//			_column_widths.add( Integer.valueOf( integer.intValue()));
//		}
...
//			put( "name1", new InitialValueMap());
	...
	private void update_column_widths(String alias) {
		...
		_columnWidths.insertElementAt( Integer.valueOf( _defaultExperimentTableColumnWidth), index);
	}
	...
	public boolean export(File file, boolean toDisplay, boolean toFile) {
		...
		while ( iterator.hasNext()) {
			...
			if ( !Exporter.execute_on_model_builder( newFile, initialValueMap, null, experimentName, toDisplay, toFile, false, false, false))
				return false;
		}
		return true;
	}
	 ...
	 */
	public ScriptFile[] export(/*boolean animator, */boolean toDisplay, boolean toFile, boolean userLogFile, boolean monitor) {
//		if ( !animator) {
			...
			for ( int i = 0; i < _numberOfTimes; ++i) {
				...
				while ( iterator.hasNext()) {
					...
					if ( !Exporter.execute_on_model_builder( file, initialValueMap, workDirectoryName, name, toDisplay, toFile, userLogFile, monitor, false)) {
						file.delete();
						break;
					}
...
//		}
	...
	public List<ScriptFile> export(File localLogDirectory, JTextField numberOfScriptsTextField, JProgressBar progressBar) {
		...
		while ( iterator.hasNext()) {
			...
			if ( !Exporter.execute_on_model_builder( file, initialValueMap, localLogDirectoryName, experimentName, false, true, false, false, true)) {
				file.delete();
				break;
			}
