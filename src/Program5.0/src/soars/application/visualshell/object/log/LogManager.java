...
public class LogManager extends HashMap<String, Map<String, LogOptionManager>> {
	 ...
	 * @return
	 */
	public Map<String, Map<String, List<String>>> get_entityNamesMap() {
		// TODO Auto-generated method stub
		Map<String, Map<String, List<String>>> entityNamesMap = new HashMap<>();
		Set<String> keys1 = keySet();
		for ( String key1:keys1) {
			Map<String, LogOptionManager> map = get( key1);
			Set<String> keys2 = map.keySet();
			for ( String key2:keys2) {
				LogOptionManager logOptionManager = map.get( key2);
				//System.out.println( logOptionManager._type + ", " + logOptionManager._kind);
				for ( LogOption logOption:logOptionManager) {
					if ( logOption._flag) {
						List<String> entities = new ArrayList<>();
						LayerManager.get_instance().get_entities_have_this_object( logOptionManager._type, logOption._name, entities);
						if ( logOptionManager._type.equals( "agent"))
							append( entityNamesMap, "agents", logOption._name, entities);
						else if ( logOptionManager._type.equals( "spot"))
							append( entityNamesMap, "spos", logOption._name, entities);
					}
					//System.out.println( logOption._name + ", " + logOption._flag);
				}
			}
		}
	...
	/**
	 * @param entityNamesMap
	 * @param key
	 * @param name
	 * @param entities
	 */
	private void append(Map<String, Map<String, List<String>>> entityNamesMap, String key, String name, List<String> entities) {
		// TODO Auto-generated method stub
		Map<String, List<String>> map = entityNamesMap.get( key);
		if ( null == map) {
			map = new HashMap<String, List<String>>();
			entityNamesMap.put( key, map);
		}
		map.put( name, entities);
	}
