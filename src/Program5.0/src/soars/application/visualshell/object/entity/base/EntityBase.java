...
public class EntityBase extends DrawObject {
	...
	/**
	 * Exceptional class variable names.
	 */
	static private final String[] _exceptionalClassVariableNames = new String[] {
		Constant._javaMethodCallClassVariableName,
		Constant._userLogFileClassVariableName,
		Constant._monitorClassVariableName
	};
...
//				if ( indicesMap.containsKey( roleName)) {
	 ...
	 * @return
	 */
	private boolean contains_role_name(Map<ObjectBase, Vector<int[]>> indicesMap, String roleName) {
		...
		if ( !_initialRole.equals( "")) {
			if ( this instanceof AgentObject) {
				...
				if ( null != role)
					...
					// TODO for monitor or user log file
					if ( _initialRole.equals( Constant._monitorRoleName) || _initialRole.equals( Constant._userLogFileRoleName))
						script += ( "\t<>startRule " + _initialRole);
				}
			} else
				return "";
	...
	private String get_script(String kind, TreeMap<String, Object> treeMap, int index, IntBuffer counter, InitialValueMap initialValueMap) {
		...
		while ( iterator.hasNext()) {
			...
			Map.Entry entry = ( Map.Entry)object;
			Map<ObjectBase, Vector<int[]>> indicesMap = ( Map<ObjectBase, Vector<int[]>>)entry.getValue();
			script += get_script( kind, index, indicesMap, counter, initialValueMap);
		}
	 ...
	 * @return
	 */
	private String get_script(String kind, int index, Map<ObjectBase, Vector<int[]>> indicesMap, IntBuffer counter, InitialValueMap initialValueMap) {
		String script = "";
		Iterator iterator = indicesMap.entrySet().iterator();
		while ( iterator.hasNext()) {
			Object object = iterator.next();
			Map.Entry entry = ( Map.Entry)object;
			ObjectBase objectBase = ( ObjectBase)entry.getKey();
			if ( kind.equals( "class variable") && !Environment.get_instance().is_java_method_call_enable() && !Environment.get_instance().is_functional_object_enable()) {
				if ( !Arrays.asList( _exceptionalClassVariableNames).contains( objectBase._name))
					continue;
			}
			Vector<int[]> indices = ( Vector<int[]>)entry.getValue();
	...
	@Override
	public boolean has_this_object(String name) {
		// TODO Auto-generated method stub
		Set<String> keys1 = _objectMapMap.keySet();
		for ( String key1:keys1) {
			//System.out.println( key1);
			Map<String, Object> map = _objectMapMap.get( key1);
			Set<String> keys2 = map.keySet();
			for ( String key2:keys2) {
				//System.out.println( key2 + ", " + map.get( key2).toString());
				if ( name.equals( ( ( ObjectBase)map.get( key2))._name))
					return true;
			}
		}
		return false;
