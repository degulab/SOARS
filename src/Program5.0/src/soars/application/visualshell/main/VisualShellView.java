...
public class VisualShellView extends View implements DropTargetListener, IMessageCallback, IIntMessageCallback, IObjectsMessageCallback {
	...
	public boolean on_file_export(File file, boolean toDisplay, boolean toFile) {
		return MessageDlg.execute( MainFrame.get_instance(), ResourceManager.get_instance().get( "application.title"), true,
			...
			new Object[] { file, Boolean.valueOf( toDisplay), Boolean.valueOf( toFile), Boolean.valueOf( false)}, this, MainFrame.get_instance());
	}
	 ...
	 */
	public ScriptFile[] on_file_export(/*boolean animator, */boolean toDisplay, boolean toFile, boolean userLogFile, boolean monitor) {
		return ( ScriptFile[])ObjectsMessageDlg.execute( MainFrame.get_instance(), ResourceManager.get_instance().get( "application.title"), true,
			"on_file_export", ResourceManager.get_instance().get( "file.export.show.message"),
			new Object[] { /*Boolean.valueOf( animator), */Boolean.valueOf( toDisplay), Boolean.valueOf( toFile), Boolean.valueOf( userLogFile), Boolean.valueOf( monitor)}, ( IObjectsMessageCallback)this, MainFrame.get_instance());
	}
	 ...
	 * @return the ModelBuilder scripts 
	 */
	public ScriptFile[] on_file_export(/*boolean animator, */boolean toDisplay, boolean toFile, boolean userLogFile, boolean monitor, ExperimentManager experimentManager) {
		return ( ScriptFile[])ObjectsMessageDlg.execute( MainFrame.get_instance(), ResourceManager.get_instance().get( "application.title"), true,
			"on_experiment_file_export", ResourceManager.get_instance().get( "file.export.show.message"),
			new Object[] { /*Boolean.valueOf( animator), */Boolean.valueOf( toDisplay), Boolean.valueOf( toFile), Boolean.valueOf( userLogFile), Boolean.valueOf( monitor), experimentManager}, ( IObjectsMessageCallback)this, MainFrame.get_instance());
	}
	...
	public Object[] objects_message_callback(String id, Object[] objects, ObjectsMessageDlg objectsMessageDlg) {
		if ( id.equals( "on_file_export"))
			return LayerManager.get_instance().export_data(
				...
				( ( Boolean)objects[ 1]).booleanValue(),
				( ( Boolean)objects[ 2]).booleanValue(),
				( ( Boolean)objects[ 3]).booleanValue());
//				( ( Boolean)objects[ 0]).booleanValue(),
//				( ( Boolean)objects[ 1]).booleanValue(),
//				( ( Boolean)objects[ 2]).booleanValue());
...
//		if ( 0 == ExperimentManager.get_instance().get_initial_value_count()) {
		if ( !ExperimentManager.get_instance().initial_value_exists()) {
			EditExportSettingDlg editExportSettingDlg = new EditExportSettingDlg( MainFrame.get_instance(),
				ResourceManager.get_instance().get( "file.export.to.soars.engine.menu"), true, true, false, false);
...
//		if ( 0 == ExperimentManager.get_instance().get_initial_value_count()) {
		if ( !ExperimentManager.get_instance().initial_value_exists()) {
			...
			EditExportSettingDlg editExportSettingDlg = new EditExportSettingDlg( MainFrame.get_instance(),
				ResourceManager.get_instance().get( "run.model.builder.menu"), true, true, true, true);
			if ( !editExportSettingDlg.do_modal( MainFrame.get_instance()))
				return;

			scriptFiles = on_file_export( /*false, */false,
				Environment.get_instance().get( Environment._editExportSettingDialogToFileKey, "false").equals( "true"),
				Environment.get_instance().get( Environment._editExportSettingDialogUserLogFileKey, "false").equals( "true"),
				true);
			if ( null == scriptFiles || 0 == scriptFiles.length)
				return;
		...
		if ( 1 < scriptFiles.length && experimentManager._parallel) {
			for ( int i = 0; i < scriptFiles.length; ++i) {
				...
				ParallelModelBuilder.execute( scriptFiles[ i],
					monitorPropertyPage,
					Environment.get_instance().get( Environment._editExportSettingDialogToFileKey, "false").equals( "true"),
					Environment.get_instance().get( Environment._editExportSettingDialogKeepUserDataFileKey, "false").equals( "true"));
			}
		} else {
			MonitorPropertyPage monitorPropertyPage = monitorFrame.append( get_title(), "");
			SerialModelBuilder.execute( scriptFiles,
				monitorPropertyPage,
				Environment.get_instance().get( Environment._editExportSettingDialogToFileKey, "false").equals( "true"),
				Environment.get_instance().get( Environment._editExportSettingDialogKeepUserDataFileKey, "false").equals( "true"));
...
//		if ( 0 == ExperimentManager.get_instance().get_initial_value_count()) {
		if ( !ExperimentManager.get_instance().initial_value_exists()) {
			EditExportSettingDlg editExportSettingDlg = new EditExportSettingDlg( MainFrame.get_instance(),
				ResourceManager.get_instance().get( "run.simulator.menu"), true, true, false, false);
			if ( !editExportSettingDlg.do_modal( MainFrame.get_instance()))
				return;
		...
		for ( int i = 0; i < scriptFiles.length; ++i) {
			...
			ParallelSimulator.execute( scriptFiles[ i], parameters,
				Environment.get_instance().get( Environment._editExportSettingDialogToFileKey, "false").equals( "true"),
				direct);
		}
	}
