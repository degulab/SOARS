...
public class EditExportSettingDlg extends Dialog {
	 ...
	 * 
	 */
	private JCheckBox _userLogFileCheckBox = null;
	 ...
	 * 
	 */
	private boolean _enableToFileCheckBox = false;

	/**
	 * 
	 */
	private boolean _enableUserLogFileCheckBox = false;

	/**
	 * 
	 */
	private boolean _enableKeepUserDataFileCheckBox = false;
	 ...
	 */
	public EditExportSettingDlg(Frame arg0, String arg1, boolean arg2, boolean enableToFileCheckBox, boolean enableUserLogFileCheckBox, boolean enableKeepUserDataFileCheckBox) {
		super(arg0, arg1, arg2);
		_enableToFileCheckBox = enableToFileCheckBox;
		_enableUserLogFileCheckBox = enableUserLogFileCheckBox;
		_enableKeepUserDataFileCheckBox = enableKeepUserDataFileCheckBox;
	}
	...
	protected boolean on_init_dialog() {
		...
		if ( !setup_user_log_file_checkBox( centerPanel))
			return false;
	 ...
	 * @return
	 */
	private boolean setup_user_log_file_checkBox(JPanel parent) {
		if ( !_enableUserLogFileCheckBox)
			return true;

		JPanel panel = new JPanel();
		panel.setLayout( new FlowLayout( FlowLayout.LEFT, 5, 0));

		_userLogFileCheckBox = new JCheckBox(
			ResourceManager.get_instance().get( "edit.export.setting.dialog.check.box.user.log.file.name"));
		_userLogFileCheckBox.setSelected( Environment.get_instance().get( Environment._editExportSettingDialogUserLogFileKey, "false").equals( "true"));
		panel.add( _userLogFileCheckBox);
		parent.add( panel);
	...
	private boolean setup_keep_user_data_file_checkBox(JPanel parent) {
		if ( !_enableKeepUserDataFileCheckBox)
			return true;
	...
	protected void on_ok(ActionEvent actionEvent) {
		...
		if ( null != _userLogFileCheckBox)
			Environment.get_instance().set( Environment._editExportSettingDialogUserLogFileKey, String.valueOf( _userLogFileCheckBox.isSelected()));

		if ( null != _keepUserDataFileCheckBox)
