...
public class Exporter {
	 ...
	 * @param userLogFile whether to make the user log files
	 * @param monitor whether to show the current time
	 * @param ga
	 * @return true for exporting the ModelBuilder script to the specified file in CSV format successfully
	 */
	public static boolean execute_on_model_builder(File file, InitialValueMap initialValueMap, String workDirectoryName, String experimentName, boolean toDisplay, boolean toFile, boolean userLogFile, boolean monitor, boolean ga) {
		String script = get_script_on_model_builder( initialValueMap, workDirectoryName, experimentName, toDisplay, toFile, userLogFile, monitor, ga);
		if ( null == script)
			return false;
...
//			writer.close();
	...
	public static void execute(InitialValueMap initialValueMap, String experimentName, boolean toDisplay, boolean toFile) {
		String script = get_script_on_model_builder( initialValueMap, null, experimentName, toDisplay, toFile, false, false, false);
		if ( null == script)
			return;
	 ...
	 * @return
	 */
	private static String get_script_on_model_builder(InitialValueMap initialValueMap, String workDirectoryName, String experimentName, boolean toDisplay, boolean toFile, boolean userLogFile, boolean monitor, boolean ga) {
...
//		script += OtherScriptsManager.get_instance().get_script( "", false);
		String script = OtherScriptsManager.get_instance().get_script( "", "", userLogFile, monitor, false, ga);
		script += get_role_script( userLogFile, monitor, false, ga);
		script += get_spot_script( initialValueMap, experimentName, userLogFile, monitor, false, ga);
		...
		script += StageManager.get_instance().get_script( userLogFile, monitor, false, ga);
		script += get_rule_script( initialValueMap, experimentName, userLogFile, monitor, false, ga, false);
...
//			writer.close();
	...
	private static String get_script_on_docker(InitialValueMap initialValueMap, String workDirectoryName, String experimentName, boolean toFile) {
		...
		script += get_role_script( false, false, true, false);
		script += get_spot_script( initialValueMap, "", false, false, true, false);
		script += get_agent_script( initialValueMap, true, false);
		script += get_spot_variable_script( initialValueMap);
		script += get_agent_variable_script( initialValueMap);
		script += StageManager.get_instance().get_script( false, false, true, false);
		script += get_rule_script( initialValueMap, experimentName, false, false, true, false, false);
...
//		script += OtherScriptsManager.get_instance().get_script( program_directory, true);
		String script = OtherScriptsManager.get_instance().get_script( scriptDirectory, programDirectory, false, false, true, false);
		script += get_role_script( false, false, true, false);
		script += get_spot_script( initialValueMap, "", false, false, true, false);
		script += get_agent_script( initialValueMap, true, false);
		script += get_spot_variable_script( initialValueMap);
...
//		String script = CommentManager.get_instance().get_script();
//		script += OtherScriptsManager.get_instance().get_script( "", false);
		String script = OtherScriptsManager.get_instance().get_script( "", "", false, false, false, false);
		script += get_role_script( false, false, false, false);
		script += get_spot_script( null, "", false, false, false, false);
		script += get_agent_script( null, false, false);
		script += get_spot_variable_script( null);
		script += get_agent_variable_script( null);
		script += StageManager.get_instance().get_script( false, false, false, false);
		script += get_rule_script( initialValueMap, experimentName, false, false, false, false, true);
		script += LogManager.get_instance().get_script_on_model_builder( workDirectoryName, toDisplay, toFile);
		script += SimulationManager.get_instance().get_script( initialValueMap);
	 ...
	 * @return
	 */
	private static String get_role_script(boolean userLogFile, boolean monitor, boolean grid, boolean ga) {
		...
		// TODO for user log
		if ( monitor)
			script += ( Constant._userLogFileRoleName + "\t" + "adapter.Rules" + Constant._lineSeparator);

		// TODO for monitor
		if ( monitor)
			script += ( Constant._monitorRoleName + "\t" + "adapter.Rules" + Constant._lineSeparator);
	 ...
	 * @return
	 */
	private static String get_spot_script(InitialValueMap initialValueMap, String experimentName, boolean userLogFile, boolean monitor, boolean grid, boolean ga) {
		...
		spots.add( get_javaMethodCallSpot());	// TODO シミュレーション実行時にだけ使われるJavaメソッド呼出スポットを作成して追加する
		spots.add( get_userRuleSpot());				// TODO シミュレーション実行時にだけ使われるユーザ定義ルールスポットを作成して追加する
		if ( userLogFile) {
			DrawObject drawObject = get_userLogFileSpot( experimentName);
			if ( null == drawObject)
				return "";
			spots.add( drawObject);							// TODO シミュレーション実行時にだけ使われるユーザログファイルスポットを作成して追加する
		}
		if ( monitor)
			spots.add( get_monitorSpot());			// TODO シミュレーション実行時にだけ使われるモニタスポットを作成して追加する
	 ...
	 * @return
	 */
	private static DrawObject get_javaMethodCallSpot() {
		// TODO Auto-generated method stub
		SpotObject spotObject = new SpotObject( Constant._javaMethodCallSpotName);
		spotObject._objectMapMap.get( "class variable").put( Constant._javaMethodCallClassVariableName, new ClassVariableObject( Constant._javaMethodCallClassVariableName, Constant._javaMethodCallJar, Constant._javaMethodCallClassName));
		return spotObject;
	}
	 ...
	 * @return
	 */
	private static DrawObject get_userLogFileSpot(String experimentName) {
		// TODO Auto-generated method stub
		String userLogFileFolderPath = make_properties( experimentName);
		if ( null == userLogFileFolderPath) {
			WarningManager.get_instance().add( new String[] { ResourceManager.get_instance().get( "edit.export.setting.dialog.user.log.file.folder.error.message")});
			return null;
		}

		if ( !LogInformationFileMaker.make( userLogFileFolderPath))
			return null;

		SpotObject spotObject = new SpotObject( Constant._userLogFileSpotName);
		spotObject._objectMapMap.get( "class variable").put( Constant._userLogFileClassVariableName, new ClassVariableObject( Constant._userLogFileClassVariableName, Constant._userLogFileJar, Constant._userLogFileClassName));
		spotObject._objectMapMap.get( "keyword").put( Constant._userLogFileStartMethodParameterKeywordName, new KeywordObject( Constant._userLogFileStartMethodParameterKeywordName, userLogFileFolderPath));
		spotObject._initialRole = Constant._userLogFileRoleName;
		return spotObject;
	}
	 ...
	 * @return
	 */
	private static String make_properties(String experimentName) {
		// TODO Auto-generated method stub
		File userLogFileFolder = make_userLogFileFolder();
		if ( null == userLogFileFolder)
			return null;

		LogProperties logProperties = new LogProperties();
		logProperties._entityNamesMap = LogManager.get_instance().get_entityNamesMap();
		if ( null == logProperties._entityNamesMap)
			return null;

		logProperties._title = CommentManager.get_instance()._title;
		logProperties._experimentName = experimentName;

		File soarsFile = LayerManager.get_instance().get_current_file();
		logProperties._filePath = ( null != soarsFile) ? soarsFile.getAbsolutePath().replaceAll( "\\\\", "/") : "";

		File file = new File( userLogFileFolder, "properties.json");
		if ( !JacksonUtility.write_json( file, logProperties))
			return null;

		return userLogFileFolder.getAbsolutePath().replaceAll( "\\\\", "/");
	}

	/**
	 * @return
	 */
	private static File make_userLogFileFolder() {
		// TODO Auto-generated method stub
		File parentFolder = ( null == LayerManager.get_instance().get_current_file())
			? new File( System.getProperty( Constant._soarsHome) + "/../log")
			: LayerManager.get_instance().get_current_file().getParentFile();

		int index = 1;
		while ( true) {
			File folder = new File( parentFolder, "log" + String.valueOf( index));
			if ( !folder.exists()) {
				if ( !folder.mkdirs())
					return null;
	 ...
	 * @return
	 */
	private static DrawObject get_monitorSpot() {
		// TODO Auto-generated method stub
		SpotObject spotObject = new SpotObject( Constant._monitorSpotName);
		spotObject._objectMapMap.get( "class variable").put( Constant._monitorClassVariableName, new ClassVariableObject( Constant._monitorClassVariableName, Constant._monitorJar, Constant._monitorClassName));
		spotObject._objectMapMap.get( "keyword").put( Constant._monitorMethodParameterKeywordName, new KeywordObject( Constant._monitorMethodParameterKeywordName,
			SimulationManager.get_instance()._exportEndTime ? ( SimulationManager.get_instance()._endTime[ 0] + "/" + SimulationManager.get_instance()._endTime[ 1] + ":" + SimulationManager.get_instance()._endTime[ 2]) : ""));
		spotObject._initialRole = Constant._monitorRoleName;
		return spotObject;
	}
	 ...
	 * @return
	 */
	private static String get_rule_script(InitialValueMap initialValueMap, String experimentName, boolean userLogFile, boolean monitor, boolean grid, boolean ga, boolean demo) {
		Vector<DrawObject> rs = new Vector<DrawObject>();
		LayerManager.get_instance().get_roles( rs);
		...
		// TODO for monitor
		if ( monitor) {
			script3 += get_monitor_rule_script( Constant._monitorInitialStageName, ruleCount.get( 0));
			script3 += get_monitor_rule_script( Constant._monitorMainStageName, ruleCount.get( 0));
			script3 += get_monitor_rule_script( Constant._monitorTerminalStageName, ruleCount.get( 0));
		}
		...
		// TODO for user log file
		if ( userLogFile) {
			script3 += get_user_log_file_initial_stage_rule_script( ruleCount.get( 0));
			script3 += get_user_log_file_main_stage_rule_script( ruleCount.get( 0));
			script3 += get_user_log_file_terminal_stage_rule_script( ruleCount.get( 0));
		}
	 ...
	 * @return
	 */
	private static String get_monitor_rule_script(String stageName, int ruleCount) {
		// TODO Auto-generated method stub
		String script = ( Constant._monitorRoleName + "\t" + stageName);

		for ( int i = 0; i < ruleCount - 1; ++i)
			script += "\t";

		return script
			+ "<" + Constant._javaMethodCallSpotName + ">setEquip " + Constant._javaMethodCallParameterListName + "=java.util.LinkedList ; "
			+ "<" + Constant._javaMethodCallSpotName + ">logEquip " + Constant._javaMethodCallParameterListName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastSpot " + Constant._javaMethodCallParameterListName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=execute ; "	// ここは任意の文字列。空文字列でも可。
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._monitorClassName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._monitorSpotName + "." + Constant._monitorClassVariableName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._monitorMethodName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=java.lang.String ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=__self." + Constant._monitorMethodParameterKeywordName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addParam " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallParameterListName + "=java.util.LinkedList ; "
			+ "<" + Constant._javaMethodCallSpotName + ">invokeClass " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallCommandMethodName + " ; "
			+ "TRUE\tMonitor" + Constant._lineSeparator;
	}
	 ...
	 * @return
	 */
	private static String get_user_log_file_initial_stage_rule_script(int ruleCount) {
		// TODO Auto-generated method stub
		String script = ( Constant._userLogFileRoleName + "\t" + Constant._userLogFileInitialStageName);
		...
		return script
			...
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=execute ; "	// ここは任意の文字列。空文字列でも可。
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._userLogFileClassName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._userLogFileSpotName + "." + Constant._userLogFileClassVariableName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._userLogFileStartMethodName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=java.lang.String ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=__self." + Constant._userLogFileStartMethodParameterKeywordName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addParam " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallParameterListName + "=java.util.LinkedList ; "
			+ "<" + Constant._javaMethodCallSpotName + ">invokeClass " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallCommandMethodName + " ; "
			+ "TRUE\tUser log file initial" + Constant._lineSeparator;
	}
	 ...
	 * @return
	 */
	private static String get_user_log_file_main_stage_rule_script(int ruleCount) {
		// TODO Auto-generated method stub
		String script = ( Constant._userLogFileRoleName + "\t" + Constant._userLogFileMainStageName);
		...
		return script
			...
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._userLogFileExecuteMethodName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addParam " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallParameterListName + "=java.util.LinkedList ; "
			+ "<" + Constant._javaMethodCallSpotName + ">invokeClass " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallCommandMethodName + " ; "
			+ "TRUE\tUser log file main" + Constant._lineSeparator;
	}
	 ...
	 * @return
	 */
	private static String get_user_log_file_terminal_stage_rule_script(int ruleCount) {
		// TODO Auto-generated method stub
		String script = ( Constant._userLogFileRoleName + "\t" + Constant._userLogFileTerminalStageName);
		...
		return script
			...
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "= ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addLastString " + Constant._javaMethodCallParameterListName + "=" + Constant._userLogFileTerminateMethodName + " ; "
			+ "<" + Constant._javaMethodCallSpotName + ">addParam " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallParameterListName + "=java.util.LinkedList ; "
			+ "<" + Constant._javaMethodCallSpotName + ">invokeClass " + Constant._javaMethodCallClassVariableName + "=" + Constant._javaMethodCallCommandMethodName + " ; "
			+ "TRUE\tUser log file terminal" + Constant._lineSeparator;
	}
}
