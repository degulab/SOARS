...
//		_lastTime = CommonTool.time_to_double( _last);
	...
	public boolean load(Graphics2D graphics2D) {
		if ( !open()) {
			close();
			return false;
		}
	...
	public boolean read(int position, int size, Vector<String> times, AgentTransitionManager agentTransitionManager, SpotTransitionManager spotTransitionManager, Graphics2D graphics2D) {
		if ( !open()) {
			close();
			return false;
		}
