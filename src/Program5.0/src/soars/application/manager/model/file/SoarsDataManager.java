...
public class SoarsDataManager {
	 ...
	 * @return
	 */
	public static File create(File folder, String title, String date, String author, String email, String comment, Component mainPanel, boolean visualshell4) {
		SoarsDataManager soarsDataManager = new SoarsDataManager( folder, new VisualShell( title, date, author, email, comment));
		return soarsDataManager.make( visualshell4, mainPanel);
	}
	 ...
	 * @return
	 */
	private File make(boolean visualshell4, Component mainPanel) {
		if ( !setup_work_directory())
			return null;

		if ( !_visualShell.make( visualshell4, new File( _rootDirectory, Constant._visualShellZipFileName), mainPanel)) {
			FileUtility.delete( _parentDirectory, true);
			return null;
		}

		if ( visualshell4) {
			if ( !FileUtility.write_text_to_file( new File( _rootDirectory, Constant._visualShell4IdentifyFileName), "", "UTF8")) {
				FileUtility.delete( _parentDirectory, true);
				return null;
			}
		}
